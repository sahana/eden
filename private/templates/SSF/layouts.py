...
# =============================================================================
class S3MenuLogin(S3NavigationItem):

    @staticmethod
    def layout(item):

        # Dropdown Login Form in Menu
        auth = current.auth

        dropdown_form = UL(auth.login(inline=True, lost_pw_link=False),
                           _class="dropdown-menu",
                           )
        _attr = {"data-toggle": "dropdown"}
        return LI(A(I(_class="icon-signin"),
                    B(_class="caret"),
                    _href="#",
