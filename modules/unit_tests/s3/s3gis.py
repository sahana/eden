...
class S3LocationTreeTests(unittest.TestCase):
    ...
    def testULT3_update_location_tree_all_locations_no_infinite_recursion(self):
        ...
        # The name of the logger is dependent on the application path, e.g.
        # application.eden.modules.s3log
        #logger = logging.getLogger(current.log.name)
        logger = logging.getLogger(current.log.get_name())
        buffer = StringIO()
        ...
        # Remove the log handler and retrieve the messages.
        logger.removeHandler(handler)
        handler.flush()
        log_messages = buffer.getvalue()
        handler.close()
        buffer.close()
