        ...
        if recipient_filters is not None:
            ...
            if subscription_id:
                ...
                pr_row = db(stable.id == subscription_id).select(stable.pe_id,
                                                        stable.owned_by_group,
                                                        limitby=(0, 1)).first()
                pe_id = pr_row.pe_id
                group_id=pr_row.owned_by_group
...
'''
                ...
                if group_ids is not None and \
                   ...
                   len(user_ids) > 0:
                    ...
                    for group_id in group_ids:
                        row = db(pr_group_table.id.belongs([group_id])).select(\
                                                        pr_group_table.pe_id,
                                                        limitby=(0, 1)).first()
                        s_row = db(stable.pe_id.belongs([row.pe_id])).select(\
                                                        stable.id,
                                                        stable.filter_id,
                                                        limitby=(0, 1)).first()
                        if s_row:
                            ...
                            success_subscription = update_admin_subscription(\
                                                    ...
                                                    row.pe_id,
                                                    group_id=group_id,
                                                    filter_id=s_row.filter_id,
                                                    subscription_id=s_row.id,
                                                    )
                        else:
                            # Insert
                            success_subscription = update_admin_subscription(\
                    ...
                    for user_id in user_ids:
                        pe_id = user_pe_id(int(user_id))
                        query = (stable.pe_id.belongs([pe_id])) & \
                                (stable.owned_by_user != user_id)
                        # Check if that particular pe_id has subscription
                        st_row = db(query).select(stable.id,
                                                  stable.filter_id,
                                                  limitby=(0, 1)).first()
                        if st_row:
                            # Update
                            success_subscription = update_admin_subscription(\
                                                    ...
                                                    pe_id,
                                                    user_id=int(user_id),
                                                    filter_id=st_row.filter_id,
                                                    subscription_id=st_row.id,
                                                    )
                        else:
                    ...
                    subscription_id = request.get_vars.get("subscription_id")
                    if subscription_id:
                        query = base_query & (stable.id.belongs([subscription_id]))
                        row = db(query).select(stable.pe_id,
                                               stable.filter_id,
                                               stable.owned_by_group,
                                               limitby=(0, 1)).first()
                        if str(row.owned_by_group) in group_ids:
                            # Update
                            success_subscription = update_admin_subscription(\
                                                subscription,
                                                row.pe_id,
                                                group_id=row.owned_by_group,
                                                filter_id=row.filter_id,
                                                subscription_id=subscription_id,
                                                )
                            group_ids.remove(str(row.owned_by_group))
                        else:
                    ...
                    rows = db(query).select(stable.owned_by_group)
                    subscribed_group_ids = [int(row.owned_by_group) for row in rows]
                    if len(group_ids) > 0:
                        for group_id in group_ids:
                            if int(group_id) in subscribed_group_ids:
                                # Get the pe_id
                                row = db(stable.owned_by_group.belongs([group_id])).\
                                                    select(
                                                           stable.id,
                                                           stable.pe_id,
                                                           stable.filter_id,
                                ...
                                row = db(pr_group_table.id.belongs([group_id])).\
                                                    select(
                                                        pr_group_table.pe_id,
                                                        limitby=(0, 1)).first()
                                # Create
                    ...
                    if len(pe_ids) > 0:
                        base_query &= ((stable.owned_by_group != None) | 
                                       (stable.comments != None))
                        for pe_id in pe_ids:
                            user_id = get_user_id(pe_id=pe_id)
                            query = base_query & (stable.pe_id.belongs([pe_id]))
                    ...
                    subscription_id = request.get_vars.get("subscription_id")
                    if subscription_id:
                        row = db(stable.id.belongs([subscription_id])).select(\
                                                        stable.filter_id,
                                                        limitby=(0, 1)).first()
                        define_resource("pr_subscription",
                                        id=subscription_id).delete()
    ...
    # -------------------------------------------------------------------------
    def _get_recipients_and_groups(self, pe_id=None, group_id=None, caller=None):
        ...
        if pe_id is not None:
            ...
            row = current.db(query).select(stable.comments,
                                           ...
                                           limitby=(0, 1)).first() 
            if row is not None:
                if caller == "recipients" and row.comments is not None:
                    filters = json.loads(row.comments)
                elif caller == "groups" and row.owned_by_group is not None:
                    filters = [['id__belongs', "%s" % str(row.owned_by_group)]]
                else:
                    filters = None
    ...
    def _update_admin_subscription(self,
        ...
        if user_id is not None:
            recipient_filter = [["id__belongs", "%d" % user_id]]
