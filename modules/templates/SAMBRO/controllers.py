    ...
    def _manage_subscriptions(self, resources, filters):
        ...
        if ftp_rows:
            ...
            if current.auth.s3_has_permission("update", "sync_repository", 
                ...
                repository_id.comment = S3PopupLink(c="sync", f="repository",
                                                    m="update",
...
$('input#s3-groupedopts-option-0-1').change(function(){
    ...
    def _update_sync(self, resource, filters, selected_repository_id, properties):
        ...
        # [["event_type_id__belongs","24,3"],["priority__belongs",null]]
        ...
        if len(filters)>0:           
            ...
            for filter in filters:
                # Get the query
                query_part = str(filter[0].split("__belongs",1)[0]).strip("[]")
                # Get the value for the query
                value_ = str(filter[1])
                if query_part in ["event_type_id", "priority", "language"]:
                    tablename = "cap_info"
                else:
            ...
            if (len(removed_filter_ids) > 0):
                query = sync_resource_filter_table.id.belongs(removed_filter_ids)
                resource = s3db.resource("sync_resource_filter", filter=query)
                resource.delete()
        else:
            query = sync_resource_filter_table.owned_by_user == user_id
            resource = s3db.resource("sync_resource_filter", filter=query)
            resource.delete()
        ...
        if properties and \
           "async_task_id" in json.loads(properties):
            ...
            # First Run: Initialize, don't start
            # Runned in onapprove cap_alert 
            async_task_id = current.s3task.schedule_task("sync_synchronize",
                                                args = [selected_repository_id],
        ...
        if async_task_id is not None:
            properties["async_task_id"] = async_task_id
        ...
        if resource_filter_id_list is not None:
            properties["resource_filter_ids"] = resource_filter_id_list
    ...
    def _remove_sync(self, properties):
        ...
        if properties:
            ...
            if "sync_task_id" in properties:
                sync_task_table = s3db.sync_task
                query = (sync_task_table.deleted != True) &\
                        (sync_task_table.owned_by_user == current.auth.user.id) & \
                        (sync_task_table.repository_id == properties["repository_id"])
                resource = s3db.resource("sync_task", filter=query)
                resource.delete(cascade=True)
                
            if "async_task_id" in properties:
                sync_job_table = s3db.sync_job
                repo_id = properties["repository_id"]
                query = (sync_job_table.deleted != True) &\
                        (sync_job_table.repository_id == repo_id)
                # Remove the scheduler_task
                row = db(query).select(sync_job_table.scheduler_task_id,
                                       limitby=(0, 1)).first()
                scheduler_table = db.scheduler_task
                query = (scheduler_table.id == row.scheduler_task_id)
                db(query).delete()
                # Delete the sync_job record
                job_resource = s3db.resource("sync_job", filter=query)
                job_resource.delete()              
