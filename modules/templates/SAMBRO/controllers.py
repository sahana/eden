...
'''
                ...
                if len(user_ids) > 0:
                    ...
                    if unselected_pe_ids:
                        for unselected_pe_id in unselected_pe_ids:
                            # Check and delete
                            user_id = get_user_id(pe_id=unselected_pe_id)
                            ...
                            # Alert Editor
                            r_ids = [row.id for row in rows 
                                     if row.owned_by_group == group_row.id]
                            # Delete these IDs
                    ...
                    if pe_ids:
                        ...
                        for pe_id in pe_ids:
                            query = (stable.owned_by_group == group_row.id) & \
                                    (stable.pe_id == pe_id) & \
                                    (stable.deleted != True)
    ...
    def _get_admin_subscription(self):
        ...
        row_ = db(agtable.role == "Alert Editor").select(agtable.id,
                                                         limitby=(0, 1)).first()
        query = (stable.owned_by_group == row_.id) & \
                (stable.deleted != True)
        left = ftable.on(ftable.id == stable.filter_id)
