...
<xsl:stylesheet
    ...
    <!-- **********************************************************************
         Indicator Data - CSV Import Stylesheet
         ...
         Comments.......................optional.....indicator_data.comments
         Approved.......................optional.....indicator_data.approved_by
                                                     Set to 'false' to not approve records.
                                                     Note this only works for prepop or users with acl.APPROVE rights

        Not currently enabled in model but likely to be in future & supported by .xsl:
    ...
    *********************************************************************** -->
    ...
    <xsl:template match="/">
        ...
        <s3xml>
            ...
            <!-- Indicators (multi/row) -->
            <xsl:for-each select="//row[1]/col[starts-with(@field, 'Ind:')]">
                <xsl:call-template name="IndicatorMulti"/>
            </xsl:for-each>
    ...
    <xsl:template match="row">
        ...
        <xsl:choose>
            ...
            <xsl:otherwise>
                ...
                <xsl:for-each select="col[starts-with(@field, 'Ind:')]">
                    <xsl:variable name="Indicator" select="normalize-space(substring-after(@field, ':'))"/>
    ...
    <xsl:template name="IndicatorData">
        ...
        <resource name="project_indicator_data">
            ...
            <reference field="parameter_id" resource="project_indicator">
                <xsl:attribute name="tuid">
                    <xsl:value-of select="concat('project_indicator/',$indicator)"/>
                </xsl:attribute>
            </reference>
    ...
    <xsl:template name="Indicator">
        ...
        <resource name="project_indicator">
            <xsl:attribute name="tuid">
                <xsl:value-of select="concat('project_indicator/',$name)"/>
            </xsl:attribute>
    ...
    <!-- ****************************************************************** -->
    <xsl:template name="IndicatorMulti">
