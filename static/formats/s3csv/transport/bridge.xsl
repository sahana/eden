...
<xsl:stylesheet version="1.0"
    ...
    *********************************************************************** -->
    ...
    <xsl:template match="row">
        ...
        <xsl:variable name="BridgeName" select="col[@field='Name']/text()"/>

        <resource name="transport_bridge">
            <xsl:attribute name="tuid">
                <xsl:value-of select="$BridgeName"/>
            </xsl:attribute>

            <!-- Link to Location -->
            <reference field="location_id" resource="gis_location">
                <xsl:attribute name="tuid">
                    <xsl:value-of select="concat($LocationPrefix, $BridgeName)"/>
                </xsl:attribute>
            </reference>

            <!-- Bridge data -->
            <data field="name"><xsl:value-of select="$BridgeName"/></data>
