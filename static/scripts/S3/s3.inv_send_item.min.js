...
t=$('<div id="sub_defaultsend_bin_defaultsend_bin_i_quantity_edit_0-warning" class="alert alert-warning" style="padding-left:36px;">'+message+'<button type="button" class="close" data-dismiss="alert">\u00d7</button></div>'),E.val(f).parent().append(t).undelegate(".s3").delegate(".alert","click.s3",function(){$(this).fadeOut("slow").remove();return!1}))}});Q.on("click.s3",".inline-edt",function(){1<r&&(c=E.val())&&(c=parseFloat(c),m-=c*h/n)});ea.click(function(){if(1<r){if(c=E.val())c=parseFloat(c),
m+=c*h/n;x.change()}});Q.on("rowAdded",function(u,A){if(1<r){if(c=A.quantity.value)c=parseFloat(c),m+=c*h/n;$("#sub_defaultsend_bin_defaultsend_bin_i_quantity_edit_none-warning").remove()}});Q.on("rowRemoved",function(u,A){1<r&&(c=A.quantity.value)&&(c=parseFloat(c),m-=c*h/n)})}});
