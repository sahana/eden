...
p=$('<div id="inv_req_item_quantity_reserved-warning" class="alert alert-warning" style="padding-left:36px;">'+e+'<button type="button" class="close" data-dismiss="alert">\u00d7</button></div>'),t.val(b).parent().append(p).undelegate(".s3").delegate(".alert","click.s3",function(){$(this).fadeOut("slow").remove();return!1}));z?I?(d=l[f].b[g]+k,a=Math.min(b,d),updateQuantity=function(h){h.quantity.value=a;h.quantity.text=a.toString().replace(Q,"$1,")},x.inlinecomponent("updateRows",updateQuantity),
$("#edt-defaultreq_item_inv-0").hide(),$("#rmv-defaultreq_item_inv-0").hide()):(c=b-k,d=l[f].b[g],a=Math.min(c,d),m.val(a)):b<k&&(b=k,e="Quantity Reserved increased to Quantity in Bins",p=$('<div id="inv_req_item_quantity_reserved-warning" class="alert alert-warning" style="padding-left:36px;">'+e+'<button type="button" class="close" data-dismiss="alert">\u00d7</button></div>'),t.val(b).parent().append(p).undelegate(".s3").delegate(".alert","click.s3",function(){$(this).fadeOut("slow").remove();return!1}))});
N.change(function(){y=(y=N.val())?parseFloat(y):0;b>y&&t.change()});m.change(function(){if(a=m.val())c=b-k,a=parseFloat(a),e=null,(f=F.val())?(g=K.val())?(d=l[f].b[g],c>=d?a>d&&(e="Bin Quantity reduced to Quantity in Bin"):a>c&&(e="Bin Quantity reduced to Quantity Reserved")):(d=l[f].q,c>=d?a>d&&(e="Bin Quantity reduced to Quantity of Stock Item"):a>c&&(e="Bin Quantity reduced to Quantity Reserved")):a>c&&(e="Bin Quantity reduced to Quantity Reserved"),e&&(p=$('<div id="sub_defaultreq_item_inv_defaultreq_item_inv_i_quantity_edit_none-warning" class="alert alert-warning" style="padding-left:36px;">'+
e+'<button type="button" class="close" data-dismiss="alert">\u00d7</button></div>'),m.val(c).parent().append(p).undelegate(".s3").delegate(".alert","click.s3",function(){$(this).fadeOut("slow").remove();return!1}))});F.change(function(){if(f=F.val()){D=l[f].b;n=[];for(g in D)n.push(g);E=n.length;1==E?I?($("#add-row-defaultreq_item_inv").hide(),$("#read-row-defaultreq_item_inv-0 > .subform-action").hide(),x.off("click.inlinecomponent",".read-row"),B=function(){}):(g=n[0],B=function(){G.hierarchicalopts("set",
[g]);$(".s3-hierarchy-button").attr("disabled","disabled");z?(c=b-k,d=l[f].b[g],m.val(Math.min(c,d)).attr("disabled",!0),$("#add-row-defaultreq_item_inv > .subform-action").hide()):(a=m.val())?m.change():(c=b-k,d=l[f].b[g],m.val(Math.min(c,d)))}):(z=!1,B=function(){$(".s3-hierarchy-button").removeAttr("disabled");G.hierarchicalopts("show",n,!0);m.change()});if(G.is(":data('s3-hierarchicalopts')"))B();else G.find(".s3-hierarchy-tree").first().on("ready.jstree",function(){B()})}});K.change(function(){m.change()});
