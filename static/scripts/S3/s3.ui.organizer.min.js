...
calendar:{text:"",click:function(){n.datepicker("show")}}},header:{left:k,center:"title",right:"calendar today prev,next"},defaultView:g,eventRender:function(a,b){l._eventRender(a,b)},eventDestroy:function(a,b){l._eventDestroy(a,b)},select:function(a,b,c){l._selectDate(a,b,c)},unselect:function(){d(l.element).qtip("destroy",!0)},unselectCancel:".s3-organizer-create",locale:a.locale,timezone:"local"});this.reloadButton=d(".fc-reload-button").html('<i class="fa fa-refresh">');e=d(".fc-calendar-button").html('<i class="fa fa-calendar">');
...
var h=f.columns,k=a.description;h&&k&&h.forEach(function(a){var c=a[0];a=a[1];k[c]!==r&&(a&&d("<label>").text(a).appendTo(b),d("<p>").html(k[c]).appendTo(b))});var g=d(this.element).attr("id");h=this.eventNamespace;var l=this,n=[],m=f.baseURL;if(m){if(f.editable&&!1!==a.editable){var p=document.createElement("a");p.href=m;p.pathname+="/"+a.id+"/update.popup";p.search=p.search?p.search+("&refresh="+g):"?refresh="+g;g=d('<a class="action-btn s3_modal">').text(e.labelEdit).attr("href",p.href);g.on("click"+
h,function(){c.hide()});n.push(g)}f.deletable&&!1!==a.deletable&&(g=d('<a class="action-btn delete-btn-ajax">').text(e.labelDelete),g.on("click"+h,function(){confirm(e.deleteConfirmation)&&(c.hide(),l._deleteItem(a,function(){c.destroy()}));return!1}),n.push(g))}n.length&&d("<div>").append(n).appendTo(b);return b},_selectDate:function(a,c,b){var e=this;d(this.element).qtip({content:{text:function(b,d){return e._selectResource(a,c,b,d)}},position:{target:"mouse",at:"center right",my:"left center",
effect:!1,viewport:d(window),adjust:{mouse:!1,method:"flip shift"}},show:{event:"click",solo:!0},hide:{event:"mouseleave",delay:800,fixed:!0},events:{visible:function(){S3.addModals()}}});d(this.element).qtip("show",b)},_selectResource:function(a,c,b,e){e.set("style.classes","s3-organizer-create");b=this.options.resources;var f=this.eventNamespace,h=d(this.element).attr("id"),k=d("<div>");b.forEach(function(b){if(b.insertable){var g=d('<a class="action-btn s3_modal">'),n=b.labelCreate;if((b=b.baseURL)&&
n){b+="/create.popup";var m=[];h&&m.push("refresh="+encodeURIComponent(h));var p=a.toISOString()+"--"+moment(c).subtract(1,"seconds").toISOString();m.push("organizer="+encodeURIComponent(p));b+="?"+m.join("&");g.attr("href",b).text(n).appendTo(k).on("click"+f,function(){e.hide()})}}});return k},_fetchItems:function(a,c,b,e,f){if(e=a._cache.retrieve(c,b))f(e);else{e=this.options;this._showThrobber();var h;a.filterForm?h=d("#"+a.filterForm):e.filterForm&&(h=d("#"+e.filterForm));var k=S3.search.getCurrentFilters(h).filter(function(b){b=
b[0].split("__")[0];return b!==a.start&&b!==a.end});if(h=a.ajaxURL){h=S3.search.filterURL(h,k);k=encodeURIComponent(c.toISOString()+"--"+b.toISOString());h=-1!=h.indexOf("?")?h+("&$interval="+k):h+("?$interval="+k);k=a.timeout;var g=d.ajaxS3;k===r&&(k=e.timeout);d.searchS3!==r&&(g=d.searchS3);if(e=a.openRequest)e.onreadystatechange=null,e.abort();var l=this;a.openRequest=g({timeout:k,url:h,dataType:"json",type:"GET",success:function(d){d=l._decodeServerData(a,d);l._hideThrobber();a._cache.store(c,
b,d);f(d)},error:function(a,b,c){l._hideThrobber();console.log("UNAUTHORIZED"==c?i18n.gis_requires_login:a.responseText)}})}}},_decodeServerData:function(a,c){var b=c.c;c=c.r;var e=[],f=0,h=a.colors;b&&b.constructor===Array&&(f=b.length);c.forEach(function(c){var g={},l=c.d;if(f&&l&&l.constructor===Array){var k=l.length;if(k<=f)for(var m=0;m<k;m++)g[b[m]]=l[m]}(l=c.e)&&(l=a.useTime?moment(l).add(1,"seconds").toISOString():moment(l).add(1,"days").startOf("day").toISOString());k=c.t;g={id:c.id,title:d("<div>").html(k).text(),
popupTitle:k,start:c.s,end:l,description:g};c.pe||(g.editable=!1);c.pd||(g.deletable=!1);h&&c.c&&(c=h[c.c],c!==r&&(g.color=c));e.push(g)});return e},_updateItem:function(a,c){var b=this.resources[a.source.id],d=this,f={id:a.id,s:a.start.toISOString()};b.end&&(f.e=b.useTime?moment(a.end).subtract(1,"seconds").toISOString():moment(a.end).subtract(1,"days").endOf("day").toISOString());this._sendItems(b,{u:[f]},function(){b.reloadOnUpdate?d.reload():b._cache.updateItem(a.id,{start:a.start,end:a.end})},
c)},_deleteItem:function(a,c){var b=this.resources[a.source.id],e={id:a.id},f=d(this.element);this._sendItems(b,{d:[e]},function(){f.fullCalendar("removeEvents",function(b){return b.source.id==a.source.id&&b.id==a.id});b._cache.deleteItem(a.id);"function"===typeof c&&c()})},_sendItems:function(a,c,b,e){var f=d('input[name="_formkey"]',this.element).val();c=JSON.stringify(d.extend({k:f},c));this._showThrobber();var h=this;d.ajaxS3({type:"POST",url:a.ajaxURL,data:c,dataType:"json",retryLimit:0,contentType:"application/json; charset=utf-8",
success:function(){"function"===typeof b&&b();h._hideThrobber()},error:function(){"function"===typeof e&&e();h._hideThrobber()}})},reload:function(){this.resources.forEach(function(a){a._cache.clear()});d(this.element).fullCalendar("refetchEvents")},_showThrobber:function(){this.reloadButton.prop("disabled",!0);this.throbber.css({visibility:"visible"})},_hideThrobber:function(){this.throbber.css({visibility:"hidden"});this.reloadButton.prop("disabled",!1)},_bindEvents:function(){return!0},_unbindEvents:function(){return!0}})})(jQuery);
