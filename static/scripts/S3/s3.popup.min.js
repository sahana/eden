...
self.parent.S3.popup_remove();else if(s3_debug("Caller: ",k),d=c.person_id,"undefined"!=typeof d)self.parent.$("#"+k).val(d).change(),self.parent.S3.popup_remove();else{d=new RegExp(".*\\"+S3.Ap+"\\/");var b=c.child,q;"undefined"===typeof b?(a=(new String(self.location)).replace(d,""),a=a.split("?")[0].split("/"),q=a[1]+"_id"):q=b;s3_debug("child_resource",q);var u=c.parent;"undefined"===typeof u?(b=k.replace("_"+q,""),s3_debug("parent_field",b),a=b.replace(/_.*/,""),b=b.replace(a+"_",""),a=new String(self.parent.location),
a=a.replace(d,""),a=a.split("?")[0].split("/"),c=null,d=a[0],g=a[1],2<a.length&&(null!==a[2].match(/\d*/)?3<a.length&&(c=a[3]):c=a[2]),null!==c&&b!=g&&b==c&&(b=g+"/"+c)):(b=u,a=new String(self.parent.location),a=a.replace(d,""),a=a.split("?")[0].split("/"),d=a[0]);s3_debug("parent_resource",b);s3_debug("caller_prefix",d);d=S3.Ap.concat("/"+d+"/"+b+"/options.s3json?field="+q);f=self.parent.$("#"+k);s3_debug("selector",f);var B="sub_"==k.substring(0,4),x=self.parent.$("#dummy_"+k),y=void 0!=x.val();
s3_debug("has_dummy",y);var v=f.hasClass("checkboxes-widget-s3"),z=f.hasClass("s3-hierarchy-input"),n;if(v){var C=self.parent.$("#"+k+" tbody tr:first").children().length;n=[]}else{var D=self.parent.$("#"+k+" >option"),w="select"==f.prop("tagName").toLowerCase();w?n=[]:d=z?d+"&hierarchy=1&only_last=1":d+"&only_last=1"}var r=1,A="";$.getJSONS3(d,function(a){var c,b,d,e=0;$.each(a.option,function(){c=this["@value"];b=this.$;"undefined"===typeof b&&(b="");w?n.push(["<option value='",c,"'>",b,"</option>"].join("")):
v?(d="id_"+q+"-"+e,n.push(["<td><input id='",d,"' name='",q,"' value='",c,"' type='checkbox'><label for='",d,"'>",b,"</label></td>"].join("")),e++):z&&(u=this["@parent"],f.parent().hierarchicalopts("addNode",u,c,b,!0));numeric_value=+c;numeric_value>r&&(r=numeric_value,A=b)});y&&(x.val(A),f.val(r).change());if(w){if(B){var g=["_0","_none","_default"],h,m=k.split("_").slice(0,-1).join("_");for(a=0;a<g.length;a++)h=g[a],self.parent.$("#"+m+h).empty().append(n.join(""))}else D.remove(),f.append(n.join("")).val(r).change();
f.val(r).change();f.prop("disabled",!1);if(f.hasClass("multiselect-widget"))try{f.multiselect("refresh")}catch(p){}}else if(v){var l=[];self.parent.$("#"+k+" input").each(function(a){$(this).prop("checked")&&l.push($(this).val())});g=[];for(a=e=0;a<n.length;a++)0===e?(g.push("<tr>"),g.push(n[a]),e++):e==C-1?(g.push(n[a]),g.push("</tr>"),e=0):(g.push(n[a]),e++);f.html(g.join(""));l.push(r);for(a=0;a<l.length;a++)self.parent.$("#"+k+' input[value="'+l[a]+'"]').prop("checked",!0)}self.parent.S3.popup_remove()})}}}
