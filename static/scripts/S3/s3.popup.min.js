...
g.append(n.join("")).val(r).change();g.val(r).change();g.prop("disabled",!1);if(g.hasClass("multiselect-widget"))try{g.multiselect("refresh")}catch(p){}}else if(v){var l=[];self.parent.$("#"+k+" input").each(function(a){$(this).prop("checked")&&l.push($(this).val())});f=[];for(a=e=0;a<n.length;a++)0===e?(f.push("<tr>"),f.push(n[a]),e++):e==C-1?(f.push(n[a]),f.push("</tr>"),e=0):(f.push(n[a]),e++);g.html(f.join(""));l.push(r);for(a=0;a<l.length;a++)self.parent.$("#"+k+' input[value="'+l[a]+'"]').prop("checked",
!0)}self.parent.S3.popup_remove()})}}}function getQueryParams(a){var d={};if(a=a.substring(1)){a=a.split("&");for(var g=[],c=0;c<a.length;c++)g=a[c].split("="),d[decodeURIComponent(g[0])]=decodeURIComponent(g[1])}return d};
