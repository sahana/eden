...
b,"' type='checkbox'><label for='",e,"'>",c,"</label></td>"].join("")),d++);numeric_value=+b;numeric_value>s&&(s=numeric_value,x=c)});w&&(v.val(x),g.val(s).change());if(u){if(y){var f=["_0","_none","_default"],k,l=n.split("_").slice(0,-1).join("_");for(a=0;a<f.length;a++)k=f[a],self.parent.$("#"+l+k).empty().append(p.join(""))}else A.remove(),g.append(p.join("")).val(s).change();g.val(s).change();g.prop("disabled",!1);if(g.hasClass("multiselect-widget"))try{g.multiselect("refresh")}catch(m){}}else if(t){var h=
[];self.parent.$("#"+n+" input").each(function(a){$(this).prop("checked")&&h.push($(this).val())});f=[];for(a=d=0;a<p.length;a++)0===d?(f.push("<tr>"),f.push(p[a]),d++):d==z-1?(f.push(p[a]),f.push("</tr>"),d=0):(f.push(p[a]),d++);g.html(f.join(""));h.push(s);for(a=0;a<h.length;a++)self.parent.$("#"+n+' input[value="'+h[a]+'"]').prop("checked",!0)}self.parent.S3.popup_remove()})}}}
