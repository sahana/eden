...
a;a--)addGPXLayer(S3.gis.layers_gpx[a-1]);if(S3.gis.layers_arcrest)for(a=S3.gis.layers_arcrest.length;0<a;a--)addArcRESTLayer(S3.gis.layers_arcrest[a-1]);S3.gis.CoordinateGrid&&addCoordinateGrid();if(S3.gis.layers_georss)for(a=S3.gis.layers_georss.length;0<a;a--)addGeoJSONLayer(S3.gis.layers_georss[a-1]);if(S3.gis.layers_kml){S3.gis.format_kml=new OpenLayers.Format.KML({extractStyles:!0,extractAttributes:!0,maxDepth:2});for(a=S3.gis.layers_kml.length;0<a;a--)addKMLLayer(S3.gis.layers_kml[a-1])}if(S3.gis.layers_wfs)for(a=
S3.gis.layers_wfs.length;0<a;a--)addWFSLayer(S3.gis.layers_wfs[a-1]);if(S3.gis.layers_feature_queries)for(a=S3.gis.layers_feature_queries.length;0<a;a--)addGeoJSONLayer(S3.gis.layers_feature_queries[a-1]);if(S3.gis.layers_feature_resources)for(a=S3.gis.layers_feature_resources.length;0<a;a--)addGeoJSONLayer(S3.gis.layers_feature_resources[a-1]);if(S3.gis.layers_features)for(a=S3.gis.layers_features.length;0<a;a--)addGeoJSONLayer(S3.gis.layers_features[a-1]);(S3.gis.features||S3.gis.draw_feature||
S3.gis.draw_polygon||navigator.geolocation)&&addDraftLayer();if(S3.gis.features)for(a=0;a<S3.gis.features.length;a++){var d=new OpenLayers.Geometry.Point(S3.gis.features[a].lon,S3.gis.features[a].lat);d.transform(S3.gis.proj4326,S3.gis.projection_current);S3.gis.draftLayer.addFeatures(new OpenLayers.Feature.Vector(d))}}
