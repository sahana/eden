...
c.transform(S3.gis.projection_current,S3.gis.proj4326);b.val(c.lon);$("#gis_location_lat").val(c.lat);$("#gis_location_wkt").val("")}S3.gis.lastDraftFeature=a}});a?(S3.gis.pointButton=new GeoExt.Action({control:c,handler:function(){S3.gis.pointButton.items[0].pressed?$(".olMapViewport").addClass("crosshair"):$(".olMapViewport").removeClass("crosshair")},map:map,iconCls:"drawpoint-off",tooltip:i18n.gis_draw_feature,allowDepress:!0,enableToggle:!0,toggleGroup:"controls",pressed:b}),a.add(S3.gis.pointButton)):
