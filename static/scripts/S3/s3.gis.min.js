...
"active")var e=false,f=true,g=false;else if(S3.gis.draw_polygon=="active"){f=e=false;g=true}else{e=true;g=f=false}S3.gis.panButton=new GeoExt.Action({control:new OpenLayers.Control.Navigation,map:map,iconCls:"pan-off",tooltip:S3.i18n.gis_pan,toggleGroup:"controls",allowDepress:true,pressed:e});a.add(b);navigator.geolocation&&addGeolocateControl(a);if(void 0===S3.gis.loc_select){a.add(c);a.add(d);a.add(S3.gis.panButton);a.addSeparator()}void 0===S3.gis.loc_select&&addNavigationControl(a);void 0===
S3.gis.loc_select&&S3.auth&&addSaveButton(a);a.addSeparator();addMeasureControls(a);S3.gis.mgrs_url&&addPdfControl(a);if(S3.gis.draw_feature||S3.gis.draw_polygon){a.addSeparator();S3.gis.draw_feature&&addPointControl(a,f);S3.gis.draw_polygon&&addPolygonControl(a,g)}S3.i18n.gis_get_feature_info&&addWMSGetFeatureInfoControl(a);S3.gis.osm_oauth&&addPotlatchButton(a);S3.gis.Google&&S3.gis.Google.StreetviewButton&&addGoogleStreetviewControl(a);try{S3.gis.Google.Earth&&google&addGoogleEarthControl(a)}catch(h){}if(S3.i18n.gis_search){b=
...
S3.gis.cluster_threshold,n=OpenLayers.Util.extend({},OpenLayers.Feature.Vector.style["default"]);if(g){n.graphicOpacity=o;n.graphicWidth=f;n.graphicHeight=e;n.graphicXOffset=-(f/2);n.graphicYOffset=-e;n.externalGraphic=d}else n.externalGraphic="";n.strokeColor="blue";n.strokeWidth=6;n.strokeOpacity=o;a=new OpenLayers.Layer.Vector(b,{dir:m,projection:S3.gis.proj4326,strategies:[new OpenLayers.Strategy.Fixed,new OpenLayers.Strategy.Cluster({distance:k,threshold:p})],layer_id:a.id,legendURL:d,style:n,
...
interval:g*1E3})],layer_id:a.id,legendURL:d,style:k,protocol:new OpenLayers.Protocol.HTTP({url:c,format:S3.gis.format_kml})});a.title=e;a.body=f;a.setVisibility(h);a.events.on({featureselected:onKmlFeatureSelect,featureunselected:onFeatureUnselect,loadstart:showThrobber,loadend:hideThrobber,loadcancel:hideThrobber});map.addLayer(a);S3.gis.layers_all.push(a)}
...
function onKmlFeatureSelect(a){s3_gis_tooltipUnselect(a);for(var a=a.feature,b=S3.uid(),c=a.geometry.getBounds().getCenterLonLat(),d=a.layer.title,e=a.attributes,f=typeof e[d],d="object"==f?e[d].value:e[d],g=a.layer.body.split(" "),h="",j=0;j<g.length;j++){f=typeof e[g[j]];if("object"==f){f=e[g[j]].displayName;f==""&&(f=g[j]);f="<b>"+f+"</b>: "+e[g[j]].value+"<br />"}else f=e[g[j]]+"<br />";h=h+f}h.search("<script")!=-1&&(h="Content contained Javascript! Escaped content below.<br />"+h.replace(/</g,
...
function onWfsFeatureSelect(a){s3_gis_tooltipUnselect(a);var a=a.feature,b=S3.uid(),c=a.geometry.getBounds().getCenterLonLat(),d=a.layer.title;if(a.cluster){for(var e,f=S3.i18n.gis_cluster_multiple+":<ul>",g=Math.min(a.cluster.length,9),h=0;h<g;h++){e=a.cluster[h].attributes[d];f=f+("<li>"+e+"</li>")}f=f+"</ul>"+("<div align='center'><a href='javascript:s3_gis_zoomToSelectedFeature("+c.lon+","+c.lat+", 3)'>Zoom in</a></div>")}else{e=a.attributes;var d=e[d],j="";$.each(e,function(a,b){j=j+("<b>"+a+
