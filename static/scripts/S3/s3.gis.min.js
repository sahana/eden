...
function addBingLayers(){var a=S3.gis.Bing,b=a.ApiKey,c;if(a.Aerial){c=new OpenLayers.Layer.Bing({key:b,type:"Aerial",name:a.Aerial});map.addLayer(c)}if(a.Road){c=new OpenLayers.Layer.Bing({key:b,type:"Road",name:a.Road});map.addLayer(c)}if(a.Hybrid){c=new OpenLayers.Layer.Bing({key:b,type:"AerialWithLabels",name:a.Hybrid});map.addLayer(c)}}
...
a.opacity?a.opacity:1;if(void 0!=a.legendURL)var r=a.legendURL;a=new OpenLayers.Layer.WMS(b,c,{layers:d},{dir:f,wrapDateLine:true,isBaseLayer:g,layer_id:a.id,queryable:void 0!=a.queryable?a.queryable:1,visibility:e});if(m)a.params.MAP=m;if(j)a.params.FORMAT=j;if(h)a.params.TRANSPARENT=true;if(l)a.params.VERSION=l;if(o)a.params.STYLES=o;if(k)a.params.BGCOLOR=k;if(n){a.params.TILED=true;a.params.TILESORIGIN=[map.maxExtent.left,map.maxExtent.bottom]}if(!g){a.opacity=q;a.buffer=p?p:0}if(r)a.legendURL=
r;map.addLayer(a)}function showThrobber(){$("#layer_throbber").show().removeClass("hidden")}function hideThrobber(){$("#layer_throbber").hide().addClass("hidden")}function s3_gis_loadDetails(a,b,c){$.ajax({url:a,success:function(a){$("#"+b).html(a);c.updateSize()},error:function(a,e,f){msg=f=="UNAUTHORIZED"?S3.i18n.gis_requires_login:a.responseText;$("#"+b+"_contentDiv").html(msg);c.updateSize()},dataType:"html"})}
...
function addSaveButton(a){var b=new Ext.Toolbar.Button({iconCls:"save",tooltip:S3.i18n.gis_save,handler:function(){var a=getState(),b=Ext.util.JSON.encode(a.layers),e=Ext.util.JSON.encode(a.plugins),f=S3.gis.config_id?S3.Ap.concat("/gis/config/"+S3.gis.config_id+".url/update"):S3.Ap.concat("/gis/config.url/create");Ext.Ajax.request({url:f,method:"POST",success:function(a){if(a=Ext.decode(a.responseText).message.split("=",2)[1])S3.gis.config_id=a},params:{lat:a.lat,lon:a.lon,zoom:a.zoom,layers:b,plugins:e}})}});
a.addSeparator();a.addButton(b)}function getState(){var a={},b=map.getCenter();b.transform(map.getProjectionObject(),S3.gis.proj4326);a.lon=b.lon;a.lat=b.lat;a.zoom=map.getZoom();var c=[],d;Ext.iterate(map.layers,function(a){d={id:a.options.layer_id,visible:a.visibility};c.push(d)});a.layers=c;var e=[];Ext.iterate(S3.gis.plugins,function(a){a.getState&&e.push(a.getState())});a.plugins=e;return a}
