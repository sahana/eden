...
a.CLASS_NAME){a.activate();a.features=b.features;a.recluster();break}b.events.un({loadend:M})};S3.gis.search_layer_loadend=M;S3.gis.refreshLayer=function(b,a){var c=S3.gis.maps,d,e,f;for(d in c){var g=c[d];var h=g.layers;var l=0;for(e=h.length;l<e;l++)if((f=h[l])&&f.s3_layer_id==b){if(a.length){var k=f.protocol.url;k=S3.search.filterURL(k,a);k={data:[],type:"GET",url:k};S3.search.searchRewriteAjaxOptions(k,"form");var m=f.protocol.options;m.params=k.data;m.readWithPOST=!0;m.url=k.url}if(g.s3.mapWin.isVisible()){f.events.on({loadend:M});
