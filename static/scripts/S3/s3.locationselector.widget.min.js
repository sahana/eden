...
$("ul.ui-autocomplete").hide();s3_gis_ac_search_selected(b.item);return!1}}).data("autocomplete")._renderItem=function(a,b){var c;c=b.name&&b.addr_street?"<a>"+b.name+",  "+b.addr_street.split(",")[0].split("\n")[0]+"</a>":b.name?"<a>"+b.name+"</a>":"<a>"+b.addr_street.split(",")[0].split("\n")[0]+"</a>";return $("<li></li>").data("item.autocomplete",b).append(c).appendTo(a)})}
...
function s3_gis_lat_lon_converter(){function a(a){return!isNaN(parseFloat(a))&&isFinite(a)}function b(a){return a.parents(".gis_coord_wrap").eq(0)}function c(a){return function(b){a.val(b||"");"undefined"==typeof b&&a.focus()}}function d(a){var b=Math.abs(a),b=60*(b-parseInt(b,10));if(1.0E-10>Math.abs(b-Math.round(b)))b=Math.round(b),c=0;else{var c=60*(b-parseInt(b,10));1.0E-10>Math.abs(c-Math.round(c))&&(c=Math.round(c))}return{d:parseInt(a,10),m:parseInt(b,10),s:c}}function e(b){var e=$(".degrees",
b).val()||0,f=$(".minutes",b).val()||0,i=$(".seconds",b).val()||0,j=c($(".degrees",b)),k=c($(".minutes",b)),l=c($(".seconds",b)),m=c($(".decimal",b)),b="gis_location_lat"==$(".decimal",b).attr("id");a(e)?(e=Number(e),Math.abs(e)>(b?90:180)?(alert(h.degrees[b?"lat":"lon"]),j()):a(f)?(f=Math.abs(f),60<f?(alert(h.minutes),k()):a(i)?(i=Math.abs(i),60<=i?(alert(h.seconds),l()):(e=(0>e?-1:1)*(Math.abs(e)+f/60+i/3600),Math.abs(e)>(b?90:180)?alert(h.decimal[b?"lat":"lon"]):(f=d(e),m(""+e),j(f.d||"0"),k(f.m||
"0"),l(f.s||"0")))):(alert(g.seconds),l())):(alert(g.minutes),k())):(alert(g.degrees),j())}function f(b){var c=$(".decimal",b),e=c.val(),f="gis_location_lat"==$(".decimal",b).attr("id");""!==e&&(a(e)?(e=Number(e),Math.abs(e)>(f?90:180)?(alert(h.decimal[f?"lat":"lon"]),c.focus()):(c=d(e),$(".degrees",b).val(c.d||"0"),$(".minutes",b).val(c.m||"0"),$(".seconds",b).val(c.s||"0"))):(alert(g.decimal),c.val("").focus()))}var g=i18n.gis_only_numbers,h=i18n.gis_range_error;$(".gis_coord_dms input").blur(function(){e(b($(this)))}).keypress(function(a){13==
...
function(b,c){if(c==google.maps.GeocoderStatus.OK){var n=b[0].geometry.location,i=n.lat(),n=n.lng(),j=new OpenLayers.LonLat(n,i);j.transform(S3.gis.proj4326,S3.gis.projection_current);var k=b[0].geometry.viewport;if(k){var l=k.getNorthEast(),m=k.getSouthWest(),k=m.lng(),m=m.lat(),r=l.lng(),l=l.lat();s3_gis_zoomMap(k,m,r,l)}else S3.gis.mapWin.rendered?map.setCenter(j):S3.gis.mapPanel.center=j;if(a){$("#gis_location_lat").val(i);$("#gis_location_lon").val(n);var o=b[0].address_components;$.each(o,function(a){var b=
o[a];$.each(b.types,function(a,c){"postal_code"==c&&!d?$("#gis_location_postcode").val(b.long_name):"administrative_area_level_1"==c&&!h?$("#gis_location_L1_ac").val(b.long_name):"administrative_area_level_2"==c&&!g?$("#gis_location_L2_ac").val(b.long_name):"administrative_area_level_3"==c&&!f?$("#gis_location_L3_ac").val(b.long_name):"administrative_area_level_4"==c&&!e&&$("#gis_location_L4_ac").val(b.long_name)})});i=new OpenLayers.Feature.Vector(new OpenLayers.Geometry.Point(j.lon,j.lat));S3.gis.draftLayer.addFeatures([i])}}else s3_debug("Geocode was not successful for the following reason",
c)})}
function s3_gis_save_locations(){$("#"+S3.gis.location_id).val()||$("#"+S3.gis.location_id).val("dummy");var a;if(S3.gis.site){var b="#"+S3.gis.site+"_name";if(a=$(b).val())$("#gis_location_name").val()||$("#gis_location_name").val(a);else return void 0==$("#name__error").val()&&$(b).after('<div id="name__error" class="error" style="display: block;">'+i18n.gis_name_required+"</div>"),$(b).focus(),S3.navigate_away_confirm&&S3SetNavigateAwayConfirm(),!1}var b=$("#gis_location_L1").val(),c=$("#gis_location_L2").val(),d=
$("#gis_location_L3").val(),e=$("#gis_location_L4").val(),f=$("#gis_location_L5").val();if(!$("#gis_location_L0").val()){a=S3.gis.site?"":$("#gis_location_name").val();var g=$("#gis_location_street").val(),h=$("#gis_location_postcode").val(),p=$("#gis_location_lat").val(),q=$("#gis_location_lon").val();if(b||c||d||e||f||a||g||h||p||q)return void 0==$("#country__error").val()&&$("#gis_location_L0").after('<div id="country__error" class="error" style="display: block;">'+i18n.gis_country_required+"</div>"),
$("#gis_location_L0").focus(),S3.navigate_away_confirm&&S3SetNavigateAwayConfirm(),!1}b||(b=$("#gis_location_L1_ac").val())&&$("#gis_location_L1").val(b);c||(c=$("#gis_location_L2_ac").val())&&$("#gis_location_L2").val(c);d||(d=$("#gis_location_L3_ac").val())&&$("#gis_location_L3").val(d);e||(e=$("#gis_location_L4_ac").val())&&$("#gis_location_L4").val(e);f||(f=$("#gis_location_L5_ac").val())&&$("#gis_location_L5").val(f);$("#gis_location_L0").prop("disabled",!1);$("#gis_location_name").prop("disabled",
