...
function s3_gis_lat_lon_converter(){function a(a){return!isNaN(parseFloat(a))&&isFinite(a)}function b(a){return a.parents(".gis_coord_wrap").eq(0)}function c(a){return function(c){a.val(c||"");"undefined"==typeof c&&a.focus()}}function d(a){var c=Math.abs(a),c=60*(c-parseInt(c));if(1.0E-10>Math.abs(c-Math.round(c)))c=Math.round(c),b=0;else{var b=60*(c-parseInt(c));1.0E-10>Math.abs(b-Math.round(b))&&(b=Math.round(b))}return{d:parseInt(a),m:parseInt(c),s:b}}function e(b){var e=$(".degrees",b).val()||
...
function s3_gis_l0_select(){var a=$("#gis_location_L0").val(),b=S3.gis.url.replace("gis/location","gis/l0")+"/"+a;$.ajax({async:!0,url:b,dataType:"json",success:function(b){if(b.id==a){S3.gis.country=b.code;for(level=1;6>level;level++){var d="L"+level;if(b[d]){$("#gis_location_"+d+"_label__row label").text(b[d]+":");s3_gis_show_level(level);var e=$("#gis_location_"+d+"__row div.tooltip"),f=e.attr("title").split("|");e.attr("title",b[d]+"|"+f[1]+"|"+f[2]);e.cluetip({activation:"hover",sticky:!1,splitTitle:"|"})}else s3_gis_hide_level(level)}if(!S3.gis.no_map&&
(d=$("#gis_location_lat").val(),e=$("#gis_location_lon").val(),!d&&!e)){var d=b.lon_min,e=b.lat_min,f=b.lon_max,g=b.lat_max;d&&e&&f&&g?s3_gis_zoomMap(d,e,f,g):(d=b.lat,e=b.lon,d&&e&&(b=new OpenLayers.LonLat(e,d),b.transform(S3.gis.proj4326,S3.gis.projection_current),S3.gis.mapWin.rendered?map.setCenter(b):S3.gis.mapPanel.center=b))}}}});$("#gis_location_L1_ac").autocomplete("option","source",s3_gis_ac_set_source(1));$("#gis_location_L2_ac").autocomplete("option","source",s3_gis_ac_set_source(2));
