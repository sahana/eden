...
$("#gis_location_expand").addClass("expanded"),$("#gis_location_expand").removeClass("expand")):(s3_gis_show_tab("add"),$("#gis_location_expand").addClass("expand"),$("#gis_location_expand").removeClass("expanded"));a.preventDefault()});$("#gis_location_search_select-btn").click(function(a){s3_gis_select_search_result();a.preventDefault()});$("form").submit(function(){S3ClearNavigateAwayConfirm();return s3_gis_save_locations()?!0:!1})}var b=$("#gis_location_map-btn");if(b){var c=b.attr("map");void 0==
c&&(c="default_map");$.when(function(){var a=new jQuery.Deferred;setTimeout(function f(){void 0!=S3.gis.maps?a.resolve("loaded"):"pending"===a.state()&&(a.notify("waiting for JS to load..."),setTimeout(f,500))},1);return a.promise()}()).then(function(a){var d=S3.gis.maps[c];d.s3.pointPlaced=function(a){a=a.geometry.getBounds().getCenterLonLat();a.transform(d.getProjectionObject(),S3.gis.proj4326);$("#gis_location_lon").val(a.lon);$("#gis_location_lat").val(a.lat);$("#gis_location_wkt").val("")};b.click(function(){d.s3.mapWin.show();
if(d.s3.polygonButton)$("#gis_location_wkt").val()||($(".olMapViewport").addClass("crosshair"),d.s3.polygonButton.control.activate());else{var a=$("#gis_location_lat").val(),c=$("#gis_location_lon").val();a&&c||($(".olMapViewport").addClass("crosshair"),d.s3.pointButton.control.activate())}})})}}function s3_gis_autocompletes(){s3_gis_autocomplete(1);s3_gis_autocomplete(2);s3_gis_autocomplete(3);s3_gis_autocomplete(4);s3_gis_autocomplete(5);s3_gis_autocomplete_search()}
function s3_gis_autocomplete(a){var b=$("#gis_location_L"+a+"_ac");if(void 0!=b.val()){var c=$("#gis_location_L"+a),e=$("#gis_location_L"+a+"_throbber");b.autocomplete({delay:500,minLength:2,source:function(c,b){$.ajax({url:s3_gis_ac_set_source(a),data:{term:c.term}}).done(function(a){if(0==a.length){var c=i18n.no_matching_records;a.push({id:0,value:"",label:c,name:c})}b(a)})},search:function(a,b){e.removeClass("hide").show();c.val("");return!0},response:function(a,c,b){e.hide();return b},focus:function(a,
c){c.item.id&&b.val(c.item.name);return!1},select:function(a,e){var g=e.item;g.id?(b.val(g.name),c.val(g.id),"L1"==g.level&&(g.parent&&""===$("#gis_location_L0").val())&&$("#gis_location_L0").val(g.parent),$("ul.ui-autocomplete").hide(),s3_gis_autocomplete(parseInt(g.level.replace("L",""),10)+1)):c.val("").change();return!1}}).data("ui-autocomplete")._renderItem=function(a,c){return $("<li></li>").data("item.autocomplete",c).append("<a>"+c.name+"</a>").appendTo(a)}}b.change(function(){for(var c=a+
1;5>=c;c++)$("#gis_location_L"+c+", #gis_location_L"+c+"_ac").val("")})}
function s3_gis_autocomplete_search(){var a=$("#gis_location_search_ac");if(void 0!=a.val()){var b=$("#gis_location_search_throbber");a.autocomplete({delay:500,minLength:2,source:function(a,b){$.ajax({url:s3_gis_ac_set_search_source(),data:{term:a.term}}).done(function(a){if(0==a.length){var c=i18n.no_matching_records;a.push({id:0,value:"",label:c,name:c})}b(a)})},search:function(a,e){b.removeClass("hide").show();$("#gis_location_search_select-btn").hide();return!0},response:function(a,e,d){b.hide();
return d},focus:function(c,b){b.item.id&&a.val(b.item.name);return!1},select:function(c,b){var d=b.item;d.id?(a.val(d.name),$("ul.ui-autocomplete").hide(),s3_gis_ac_search_selected(d)):a.val("");return!1}}).data("ui-autocomplete")._renderItem=function(a,b){var d;d=b.name&&b.addr_street?"<a>"+b.name+",  "+b.addr_street.split(",")[0].split("\n")[0]+"</a>":b.name?"<a>"+b.name+"</a>":"<a>"+b.addr_street.split(",")[0].split("\n")[0]+"</a>";return $("<li></li>").data("item.autocomplete",b).append(d).appendTo(a)}}}
