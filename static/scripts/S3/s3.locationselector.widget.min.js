...
c){c.item.id&&b.val(c.item.name);return!1},select:function(a,c){var g=c.item;g.id?(b.val(g.name),d.val(g.id),"L1"==g.level&&g.parent&&""===$("#gis_location_L0").val()&&$("#gis_location_L0").val(g.parent),$("ul.ui-autocomplete").hide(),s3_gis_autocomplete(parseInt(g.level.replace("L",""),10)+1)):d.val("").change();return!1}}).data("ui-autocomplete")._renderItem=function(a,c){return $("<li></li>").data("item.autocomplete",c).append("<a>"+c.name+"</a>").appendTo(a)}}b.change(function(){for(var c=a+1;5>=
...
return e},focus:function(b,c){c.item.id&&a.val(c.item.name);return!1},select:function(b,c){var e=c.item;e.id?(a.val(e.name),$("ul.ui-autocomplete").hide(),s3_gis_ac_search_selected(e)):a.val("");return!1}}).data("ui-autocomplete")._renderItem=function(a,c){var b;b=c.name&&c.addr_street?"<a>"+c.name+",  "+c.addr_street.split(",")[0].split("\n")[0]+"</a>":c.name?"<a>"+c.name+"</a>":"<a>"+c.addr_street.split(",")[0].split("\n")[0]+"</a>";return $("<li></li>").data("item.autocomplete",c).append(b).appendTo(a)}}}
function s3_gis_ac_set_source(a){var b=$("#gis_location_L"+(a-1)).val(),d;""===b&&1<a&&(d=$("#gis_location_L"+(a-2)).val(),""===d&&2<a&&(d=$("#gis_location_L"+(a-2)).val(),""===d&&3<a&&(d=$("#gis_location_L"+(a-3)).val(),""===d&&4<a&&(d=$("#gis_location_L"+(a-4)).val(),""===d&&5<a&&(d=$("#gis_location_L"+(a-5)).val())))));return b?S3.gis.url+"/search_ac?loc_select=1&level=L"+a+"&parent="+b:d?S3.gis.url+"/search_ac?loc_select=1&level=L"+a+"&children="+d:S3.gis.url+"/search_ac?loc_select=1&level=L"+
...
function s3_gis_l0_select(){var a=$("#gis_location_L0").val(),b=S3.gis.url.replace("gis/location","gis/l0")+"/"+a;$.ajax({async:!0,url:b,dataType:"json"}).done(function(b){if(b.id==a){S3.gis.country=b.code;for(level=1;5>=level;level++){var c="L"+level;b[c]?($("#gis_location_"+c+"_label__row label").text(b[c]+":"),s3_gis_show_level(level)):s3_gis_hide_level(level);$("#gis_location_"+c+", #gis_location_"+c+"_ac").val("")}if(!S3.gis.no_map){var c=$("#gis_location_lat").val(),e=$("#gis_location_lon").val();
if(!c&&!e){var c=b.lon_min,e=b.lat_min,f=b.lon_max,g=b.lat_max;c&&e&&f&&g?s3_gis_zoomMap(c,e,f,g):(c=b.lat,e=b.lon,c&&e&&(b=S3.gis.maps.default_map,c=new OpenLayers.LonLat(e,c),c.transform(S3.gis.proj4326,b.getProjectionObject()),b.s3.mapWin.rendered?b.setCenter(c):b.s3.mapPanel.center=c))}}}})}
...
c=$("#gis_location_L3").val(),e=$("#gis_location_L4").val(),f=$("#gis_location_L5").val();if(!$("#gis_location_L0").val()){a=S3.gis.site?"":$("#gis_location_name").val();var g=$("#gis_location_street").val(),h=$("#gis_location_postcode").val(),u=$("#gis_location_lat").val(),v=$("#gis_location_lon").val();if(b||d||c||e||f||a||g||h||u||v)return void 0==$("#country__error").val()&&$("#gis_location_L0").after('<div id="country__error" class="error" style="display: block;">'+i18n.gis_country_required+
