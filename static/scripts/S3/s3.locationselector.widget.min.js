...
function s3_gis_lat_lon_converter(){function a(a){return!isNaN(parseFloat(a))&&isFinite(a)}function b(a){return a.parents(".gis_coord_wrap").eq(0)}function c(a){return function(b){a.val(b||"");"undefined"==typeof b&&a.focus()}}function d(a){var b=Math.abs(a),b=60*(b-parseInt(b));if(1.0E-10>Math.abs(b-Math.round(b)))b=Math.round(b),c=0;else{var c=60*(b-parseInt(b));1.0E-10>Math.abs(c-Math.round(c))&&(c=Math.round(c))}return{d:parseInt(a),m:parseInt(b),s:c}}function e(b){var e=$(".degrees",b).val()||
0,f=$(".minutes",b).val()||0,i=$(".seconds",b).val()||0,j=c($(".degrees",b)),k=c($(".minutes",b)),l=c($(".seconds",b)),m=c($(".decimal",b)),b="gis_location_lat"==$(".decimal",b).attr("id");a(e)?(e=Number(e),Math.abs(e)>(b?90:180)?(alert(h.degrees[b?"lat":"lon"]),j()):a(f)?(f=Math.abs(f),60<f?(alert(h.minutes),k()):a(i)?(i=Math.abs(i),60<=i?(alert(h.seconds),l()):(e=(0>e?-1:1)*(Math.abs(e)+f/60+i/3600),Math.abs(e)>(b?90:180)?alert(h.decimal[b?"lat":"lon"]):(f=d(e),m(""+e),j(f.d||"0"),k(f.m||"0"),l(f.s||
"0")))):(alert(g.seconds),l())):(alert(g.minutes),k())):(alert(g.degrees),j())}function f(b){var c=$(".decimal",b),e=c.val(),f="gis_location_lat"==$(".decimal",b).attr("id");""!=e&&(a(e)?(e=Number(e),Math.abs(e)>(f?90:180)?(alert(h.decimal[f?"lat":"lon"]),c.focus()):(c=d(e),$(".degrees",b).val(c.d||"0"),$(".minutes",b).val(c.m||"0"),$(".seconds",b).val(c.s||"0"))):(alert(g.decimal),c.val("").focus()))}var g=i18n.gis_only_numbers,h=i18n.gis_range_error;$(".gis_coord_dms input").blur(function(){e(b($(this)))}).keypress(function(a){13==
...
($("#gis_location_L1_ac").attr("disabled",!0),$("#gis_location_L1__row").show());$("#gis_location_map-btn").html(i18n.gis_view_on_map);$("#gis_location_map_button_row").show();$("#gis_location_geocoder-btn").hide();$("#gis_loc_view_tab").removeClass("tab_other").addClass("tab_here");$("#gis_loc_edit_tab").removeClass("tab_here").addClass("tab_last")}
...
$("#gis_location_L3").val(),e=$("#gis_location_L4").val(),f=$("#gis_location_L5").val();if(!$("#gis_location_L0").val()){var b=S3.gis.site?"":$("#gis_location_name").val(),g=$("#gis_location_street").val(),h=$("#gis_location_postcode").val(),p=$("#gis_location_lat").val(),q=$("#gis_location_lon").val();if(a||c||d||e||f||b||g||h||p||q)return void 0==$("#country__error").val()&&$("#gis_location_L0").after('<div id="country__error" class="error" style="display: block;">'+i18n.gis_country_required+"</div>"),
