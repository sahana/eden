...
$("#gis_location_expand").addClass("expanded"),$("#gis_location_expand").removeClass("expand")):(s3_gis_show_tab("add"),$("#gis_location_expand").addClass("expand"),$("#gis_location_expand").removeClass("expanded"));a.preventDefault()});$("#gis_location_search_select-btn").click(function(a){s3_gis_select_search_result();a.preventDefault()});$("form").submit(function(){S3ClearNavigateAwayConfirm();return s3_gis_save_locations()?!0:!1})}var b=Ext.get("gis_location_map-btn");if(b)b.on("click",function(){var a=
$("#gis_location_map-btn").attr("map");void 0==a&&(a="default_map");a=S3.gis.maps[a];a.s3.mapWin.show();if(a.s3.polygonButton)$("#gis_location_wkt").val()||($(".olMapViewport").addClass("crosshair"),a.s3.polygonButton.control.activate());else{var b=$("#gis_location_lat").val(),d=$("#gis_location_lon").val();b&&d||($(".olMapViewport").addClass("crosshair"),a.s3.pointButton.control.activate())}})}
...
function s3_gis_autocomplete(a){var b=$("#gis_location_L"+a+"_ac");if(void 0!=b.val()){var c=$("#gis_location_L"+a),e=$("#gis_location_L"+a+"_throbber");b.autocomplete({delay:500,minLength:2,source:function(b,c){$.ajax({url:s3_gis_ac_set_source(a),data:{term:b.term}}).done(function(a){if(0==a.length){var b=i18n.no_matching_records;a.push({id:0,value:"",label:b,name:b})}c(a)})},search:function(a,f){b.hide();e.removeClass("hide").show();c.val("");return!0},response:function(a,c,g){e.hide();b.show();
return g},focus:function(a,c){b.val(c.item.name);return!1},select:function(a,e){var g=e.item;g.id?(b.val(g.name),c.val(g.id),"L1"==g.level&&(g.parent&&""===$("#gis_location_L0").val())&&$("#gis_location_L0").val(g.parent),$("ul.ui-autocomplete").hide(),s3_gis_autocomplete(parseInt(g.level.replace("L",""),10)+1)):(b.val(""),c.val("").change());return!1}}).data("ui-autocomplete")._renderItem=function(a,b){return $("<li></li>").data("item.autocomplete",b).append("<a>"+b.name+"</a>").appendTo(a)}}b.change(function(){for(var b=
a+1;5>=b;b++)$("#gis_location_L"+b+", #gis_location_L"+b+"_ac").val("")})}
function s3_gis_autocomplete_search(){var a=$("#gis_location_search_ac");if(void 0!=a.val()){var b=$("#gis_location_search_throbber");a.autocomplete({delay:500,minLength:2,source:function(a,b){$.ajax({url:s3_gis_ac_set_search_source(),data:{term:a.term}}).done(function(a){if(0==a.length){var c=i18n.no_matching_records;a.push({id:0,value:"",label:c,name:c})}b(a)})},search:function(c,e){a.hide();b.removeClass("hide").show();$("#gis_location_search_select-btn").hide();return!0},response:function(c,e,
d){b.hide();a.show();return d},focus:function(b,e){a.val(e.item.name);return!1},select:function(b,e){var d=e.item;d.id?(a.val(d.name),$("ul.ui-autocomplete").hide(),s3_gis_ac_search_selected(d)):(a.val(""),real_input.val("").change());return!1}}).data("ui-autocomplete")._renderItem=function(a,b){var d;d=b.name&&b.addr_street?"<a>"+b.name+",  "+b.addr_street.split(",")[0].split("\n")[0]+"</a>":b.name?"<a>"+b.name+"</a>":"<a>"+b.addr_street.split(",")[0].split("\n")[0]+"</a>";return $("<li></li>").data("item.autocomplete",
b).append(d).appendTo(a)}}}
function s3_gis_ac_set_source(a){var b=$("#gis_location_L"+(a-1)).val(),c;""===b&&1<a&&(c=$("#gis_location_L"+(a-2)).val(),""===c&&2<a&&(c=$("#gis_location_L"+(a-2)).val(),""===c&&3<a&&(c=$("#gis_location_L"+(a-3)).val(),""===c&&4<a&&(c=$("#gis_location_L"+(a-4)).val(),""===c&&5<a&&(c=$("#gis_location_L"+(a-5)).val())))));return b?S3.gis.url+"/search_ac?filter=~&field=name&loc_select=1&level=L"+a+"&parent="+b:c?S3.gis.url+"/search_ac?filter=~&field=name&loc_select=1&level=L"+a+"&children="+c:S3.gis.url+
"/search_ac?filter=~&field=name&loc_select=1&level=L"+a}function s3_gis_ac_set_search_source(){return S3.gis.url+"/search_ac?filter=~&field=name&loc_select=1&field2=addr_street&level=nullnone"}
function s3_gis_ac_search_selected(a){var b;for(b=0;5>=b;b++)$("#gis_location_L"+b+"_search").val(""),$("#gis_location_L"+b+"_label__row").hide(),$("#gis_location_L"+b+"_search__row").hide();var c=a.path,e=a.parent;if(c){var d=c.slice(0,c.toString().length+1).split("/"),f=d.length-1;if(0===f)$("#gis_location_search_throbber").hide(),$("#gis_location_search_select-btn").removeClass("hide").show();else for(b=0;b<=f;b++)b==f-1?s3_gis_search_hierarchy(d[b],!1,!0):s3_gis_search_hierarchy(d[b],!1,!1)}else e&&
s3_gis_search_hierarchy(e,!0,!0);a.name?($("#gis_location_name_label__row").removeClass("hide").show(),$("#gis_location_name_search").val(a.name),$("#gis_location_name_search__row").removeClass("hide").show()):($("#gis_location_name_label__row").hide(),$("#gis_location_name_search").val(""),$("#gis_location_name_search__row").hide());a.addr_street?($("#gis_location_street_label__row").removeClass("hide").show(),$("#gis_location_street_search").val(a.addr_street),$("#gis_location_street_search__row").removeClass("hide").show()):
...
$("#gis_location_lon_search").val(""),$("#gis_location_lon_search__row").hide()));$("body").data("id",a.id);c||e||$("#gis_location_search_select-btn").removeClass("hide").show()}
function s3_gis_lat_lon_converter(){function a(a){return!isNaN(parseFloat(a))&&isFinite(a)}function b(a){return a.parents(".gis_coord_wrap").eq(0)}function c(a){return function(b){a.val(b||"");"undefined"==typeof b&&a.focus()}}function e(a){var b=Math.abs(a),b=60*(b-parseInt(b,10));if(1E-10>Math.abs(b-Math.round(b)))b=Math.round(b),c=0;else{var c=60*(b-parseInt(b,10));1E-10>Math.abs(c-Math.round(c))&&(c=Math.round(c))}return{d:parseInt(a,10),m:parseInt(b,10),s:c}}function d(b){var d=$(".degrees",
b).val()||0,f=$(".minutes",b).val()||0,k=$(".seconds",b).val()||0,n=c($(".degrees",b)),l=c($(".minutes",b)),m=c($(".seconds",b)),p=c($(".decimal",b));b="gis_location_lat"==$(".decimal",b).attr("id");a(d)?(d=Number(d),Math.abs(d)>(b?90:180)?(alert(h.degrees[b?"lat":"lon"]),n()):a(f)?(f=Math.abs(f),60<f?(alert(h.minutes),l()):a(k)?(k=Math.abs(k),60<=k?(alert(h.seconds),m()):(d=(0>d?-1:1)*(Math.abs(d)+f/60+k/3600),Math.abs(d)>(b?90:180)?alert(h.decimal[b?"lat":"lon"]):(f=e(d),p(""+d),n(f.d||"0"),l(f.m||
"0"),m(f.s||"0")))):(alert(g.seconds),m())):(alert(g.minutes),l())):(alert(g.degrees),n())}function f(b){var c=$(".decimal",b),d=c.val(),f="gis_location_lat"==$(".decimal",b).attr("id");""!==d&&(a(d)?(d=Number(d),Math.abs(d)>(f?90:180)?(alert(h.decimal[f?"lat":"lon"]),c.focus()):(c=e(d),$(".degrees",b).val(c.d||"0"),$(".minutes",b).val(c.m||"0"),$(".seconds",b).val(c.s||"0"))):(alert(g.decimal),c.val("").focus()))}var g=i18n.gis_only_numbers,h=i18n.gis_range_error;$(".gis_coord_dms input").blur(function(){d(b($(this)))}).keypress(function(a){13==
a.which&&a.preventDefault()});$(".gis_coord_decimal input").blur(function(){f(b($(this)))}).keypress(function(a){13==a.which&&a.preventDefault()});$(".gis_coord_switch_dms").click(function(a){$(".gis_coord_dms").show();$(".gis_coord_decimal").hide();$(".gis_coord_wrap").each(function(){f($(this))});a.preventDefault()});$(".gis_coord_switch_decimal").click(function(a){$(".gis_coord_decimal").show();$(".gis_coord_dms").hide();$(".gis_coord_wrap").each(function(){d($(this))});a.preventDefault()});$(".gis_coord_wrap").each(function(){f($(this))})}
function s3_gis_search_hierarchy(a,b,c){$("#gis_location_search_throbber").removeClass("hide").show();$.ajax({async:!0,url:S3.gis.url+"/"+a+".json",dataType:"json"}).done(function(e){if(e[0].id==a){var d=e[0].name,f=e[0].level;e=e[0].parent;$("body").data(f,a);$("#gis_location_"+f+"_search").val(d);$("#gis_location_"+f+"_label__row").removeClass("hide").show();$("#gis_location_"+f+"_search__row").removeClass("hide").show();b&&e?s3_gis_search_hierarchy(e,!0,!0):c&&($("#gis_location_search_throbber").hide(),
...
function s3_gis_l0_select(){var a=$("#gis_location_L0").val(),b=S3.gis.url.replace("gis/location","gis/l0")+"/"+a;$.ajax({async:!0,url:b,dataType:"json"}).done(function(b){if(b.id==a){S3.gis.country=b.code;for(level=1;5>=level;level++){var e="L"+level;b[e]?($("#gis_location_"+e+"_label__row label").text(b[e]+":"),s3_gis_show_level(level)):s3_gis_hide_level(level);$("#gis_location_"+e+", #gis_location_"+e+"_ac").val("")}if(!S3.gis.no_map){var e=$("#gis_location_lat").val(),d=$("#gis_location_lon").val();
if(!e&&!d){var e=b.lon_min,d=b.lat_min,f=b.lon_max,g=b.lat_max;e&&d&&f&&g?s3_gis_zoomMap(e,d,f,g):(e=b.lat,d=b.lon,e&&d&&(b=S3.gis.maps.default_map,e=new OpenLayers.LonLat(d,e),e.transform(S3.gis.proj4326,b.getProjectionObject()),b.s3.mapWin.rendered?b.setCenter(e):b.s3.mapPanel.center=e))}}}})}
function s3_gis_zoomMap(a,b,c,e){if(S3.gis.maps){var d=S3.gis.maps.default_map,f=S3.gis.proj4326,g=d.getProjectionObject();d.s3.mapWin.rendered&&(b=new OpenLayers.LonLat(a,b),e=new OpenLayers.LonLat(c,e),b.transform(f,g),e.transform(f,g),a=b.lon,b=b.lat,c=e.lon,e=e.lat);c=OpenLayers.Bounds.fromArray([a,b,c,e]);a=c.getCenterLonLat();c=d.getZoomForExtent(c,!0);16<c&&(c=16);d.s3.mapWin.rendered?(d.setCenter(a),d.zoomTo(c)):(a.transform(f,g),d=d.s3.mapPanel,d.center=a,d.zoom=c)}}
...
function s3_gis_geocode(a){var b=$("#gis_location_street").val();if(b)var c=b.search(/[A-z]/),b=b.substr(c);var e=$("#gis_location_postcode").val();e&&(b+=", "+e);(c=$("#gis_location_L5_ac").val())&&(b+=", "+c);var d=$("#gis_location_L4_ac").val();d&&(b+=", "+d);var f=$("#gis_location_L3_ac").val();f&&(b+=", "+f);var g=$("#gis_location_L2_ac").val();g&&(b+=", "+g);var h=$("#gis_location_L1_ac").val();h&&(b+=", "+h);b={address:b};S3.gis.country&&(b.region=S3.gis.country.toLowerCase());S3.gis.geocoder.geocode(b,
function(b,c){if(c==google.maps.GeocoderStatus.OK){var q=S3.gis.maps.default_map,k=b[0].geometry.location,n=k.lat(),k=k.lng(),l=new OpenLayers.LonLat(k,n);l.transform(S3.gis.proj4326,q.getProjectionObject());var m=b[0].geometry.viewport;if(m){var p=m.getNorthEast(),r=m.getSouthWest(),m=r.lng(),r=r.lat(),v=p.lng(),p=p.lat();s3_gis_zoomMap(m,r,v,p)}else q.s3.mapWin.rendered?q.setCenter(l):q.s3.mapPanel.center=l;if(a){$("#gis_location_lat").val(n);$("#gis_location_lon").val(k);var s=b[0].address_components;
$.each(s,function(a,b){var c=s[a];$.each(c.types,function(a,b){"postal_code"!=b||e?"administrative_area_level_1"!=b||h?"administrative_area_level_2"!=b||g?"administrative_area_level_3"!=b||f?"administrative_area_level_4"!=b||d||$("#gis_location_L4_ac").val(c.long_name):$("#gis_location_L3_ac").val(c.long_name):$("#gis_location_L2_ac").val(c.long_name):$("#gis_location_L1_ac").val(c.long_name):$("#gis_location_postcode").val(c.long_name)})});n=new OpenLayers.Feature.Vector(new OpenLayers.Geometry.Point(l.lon,
l.lat));q.s3.draftLayer.addFeatures([n])}}else s3_debug("Geocode was not successful for the following reason",c)})}
...
e=$("#gis_location_L3").val(),d=$("#gis_location_L4").val(),f=$("#gis_location_L5").val();if(!$("#gis_location_L0").val()){a=S3.gis.site?"":$("#gis_location_name").val();var g=$("#gis_location_street").val(),h=$("#gis_location_postcode").val(),t=$("#gis_location_lat").val(),u=$("#gis_location_lon").val();if(b||c||e||d||f||a||g||h||t||u)return void 0==$("#country__error").val()&&$("#gis_location_L0").after('<div id="country__error" class="error" style="display: block;">'+i18n.gis_country_required+
"</div>"),$("#gis_location_L0").focus(),S3.navigate_away_confirm&&S3SetNavigateAwayConfirm(),!1}b||(b=$("#gis_location_L1_ac").val())&&$("#gis_location_L1").val(b);c||(c=$("#gis_location_L2_ac").val())&&$("#gis_location_L2").val(c);e||(e=$("#gis_location_L3_ac").val())&&$("#gis_location_L3").val(e);d||(d=$("#gis_location_L4_ac").val())&&$("#gis_location_L4").val(d);f||(f=$("#gis_location_L5_ac").val())&&$("#gis_location_L5").val(f);$("#gis_location_L0").prop("disabled",!1);$("#gis_location_name").prop("disabled",
