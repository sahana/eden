...
function s3_gis_ac_search_selected(a){var b;for(b=0;b<=5;b++)$("#gis_location_L"+b+"_search").val(""),$("#gis_location_L"+b+"_label__row").hide(),$("#gis_location_L"+b+"_search__row").hide();var c=a.path,f=a.parent;if(c){var d=c.slice(0,c.toString().length+1).split("/"),e=d.length-1;if(e==0)$("#gis_location_search_throbber").hide(),$("#gis_location_search_select-btn").removeClass("hide").show();else for(b=0;b<=e;b++)b==e-1?s3_gis_search_hierarchy(d[b],!1,!0):s3_gis_search_hierarchy(d[b],!1,!1)}else f&&
...
function s3_gis_lat_lon_converter(){function a(a){return!isNaN(parseFloat(a))&&isFinite(a)}function b(a){return a.parents(".gis_coord_wrap").eq(0)}function c(a){return function(c){a.val(c||"");typeof c=="undefined"&&a.focus()}}function f(a){var c=Math.abs(a),c=(c-parseInt(c))*60;if(Math.abs(c-Math.round(c))<1.0E-10)c=Math.round(c),b=0;else{var b=(c-parseInt(c))*60;Math.abs(b-Math.round(b))<1.0E-10&&(b=Math.round(b))}return{d:parseInt(a),m:parseInt(c),s:b}}function d(b){var d=$(".degrees",b).val()||
0,e=$(".minutes",b).val()||0,i=$(".seconds",b).val()||0,j=c($(".degrees",b)),k=c($(".minutes",b)),l=c($(".seconds",b)),o=c($(".decimal",b)),b=$(".decimal",b).attr("id")=="gis_location_lat";a(d)?(d=Number(d),Math.abs(d)>(b?90:180)?(alert(h.degrees[b?"lat":"lon"]),j()):a(e)?(e=Math.abs(e),e>60?(alert(h.minutes),k()):a(i)?(i=Math.abs(i),i>=60?(alert(h.seconds),l()):(d=(d<0?-1:1)*(Math.abs(d)+e/60+i/3600),Math.abs(d)>(b?90:180)?alert(h.decimal[b?"lat":"lon"]):(e=f(d),o(""+d),j(e.d||"0"),k(e.m||"0"),l(e.s||
"0")))):(alert(g.seconds),l())):(alert(g.minutes),k())):(alert(g.degrees),j())}function e(c){var b=$(".decimal",c),d=b.val(),e=$(".decimal",c).attr("id")=="gis_location_lat";d!=""&&(a(d)?(d=Number(d),Math.abs(d)>(e?90:180)?(alert(h.decimal[e?"lat":"lon"]),b.focus()):(b=f(d),$(".degrees",c).val(b.d||"0"),$(".minutes",c).val(b.m||"0"),$(".seconds",c).val(b.s||"0"))):(alert(g.decimal),b.val("").focus()))}var g=S3.i18n.gis_only_numbers,h=S3.i18n.gis_range_error;$(".gis_coord_dms input").blur(function(){d(b($(this)))}).keypress(function(a){a.which==
...
function s3_gis_show_tab(a){$("#gis_location_tabs_row").removeClass("hide").show();if(a=="search")s3_gis_search_tab();else if(a=="add")s3_gis_add_tab();else if(a=="edit")s3_gis_edit_tab();else if(a=="view")s3_gis_view_tab();else return!1}
