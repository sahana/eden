...
function s3_gis_lat_lon_converter(){function a(a){return!isNaN(parseFloat(a))&&isFinite(a)}function b(a){return a.parents(".gis_coord_wrap").eq(0)}function c(a){return function(c){a.val(c||"");"undefined"==typeof c&&a.focus()}}function f(a){var c=Math.abs(a),c=60*(c-parseInt(c));if(1.0E-10>Math.abs(c-Math.round(c)))c=Math.round(c),b=0;else{var b=60*(c-parseInt(c));1.0E-10>Math.abs(b-Math.round(b))&&(b=Math.round(b))}return{d:parseInt(a),m:parseInt(c),s:b}}function d(b){var d=$(".degrees",b).val()||
0,e=$(".minutes",b).val()||0,j=$(".seconds",b).val()||0,l=c($(".degrees",b)),m=c($(".minutes",b)),n=c($(".seconds",b)),o=c($(".decimal",b)),b="gis_location_lat"==$(".decimal",b).attr("id");a(d)?(d=Number(d),Math.abs(d)>(b?90:180)?(alert(h.degrees[b?"lat":"lon"]),l()):a(e)?(e=Math.abs(e),60<e?(alert(h.minutes),m()):a(j)?(j=Math.abs(j),60<=j?(alert(h.seconds),n()):(d=(0>d?-1:1)*(Math.abs(d)+e/60+j/3600),Math.abs(d)>(b?90:180)?alert(h.decimal[b?"lat":"lon"]):(e=f(d),o(""+d),l(e.d||"0"),m(e.m||"0"),n(e.s||
...
function s3_gis_l0_select(){var a=$("#gis_location_L0").val(),b=S3.gis.url.replace("gis/location","gis/l0")+"/"+a;$.ajax({async:!0,url:b,dataType:"json",success:function(c){if(c.id==a){S3.gis.country=c.code;for(level=1;6>level;level++){var b="L"+level;if(c[b]){$("#gis_location_"+b+"_label__row label").text(c[b]+":");s3_gis_show_level(level);var d=$("#gis_location_"+b+"__row div.tooltip"),e=d.attr("title").split("|");d.attr("title",c[b]+"|"+e[1]+"|"+e[2]);d.cluetip({activation:"hover",sticky:!1,splitTitle:"|"})}else s3_gis_hide_level(level)}if(!S3.gis.no_map&&
(b=$("#gis_location_lat").val(),d=$("#gis_location_lon").val(),!b&&!d)){var b=c.lon_min,d=c.lat_min,e=c.lon_max,g=c.lat_max;b&&d&&e&&g?s3_gis_zoomMap(b,d,e,g):(b=c.lat,d=c.lon,b&&d&&(c=new OpenLayers.LonLat(d,b),c.transform(S3.gis.proj4326,S3.gis.projection_current),S3.gis.mapWin.rendered?map.setCenter(c):S3.gis.mapPanel.center=c))}}}});$("#gis_location_L1_ac").autocomplete("option","source",s3_gis_ac_set_source(1));$("#gis_location_L2_ac").autocomplete("option","source",s3_gis_ac_set_source(2));
...
$("#gis_location_map-btn").html(i18n.gis_place_on_map);$("#gis_location_map_button_row").show();$("#gis_loc_edit_tab").removeClass("tab_other").addClass("tab_here");$("#gis_loc_view_tab").removeClass("tab_here").addClass("tab_last")}
...
($("#gis_location_L1_ac").attr("disabled",!0),$("#gis_location_L1__row").show());$("#gis_location_map-btn").html(i18n.gis_view_on_map);$("#gis_location_map_button_row").show();$("#gis_loc_view_tab").removeClass("tab_other").addClass("tab_here");$("#gis_loc_edit_tab").removeClass("tab_here").addClass("tab_last")}
...
function s3_gis_initGeocoder(){try{google&&($("#gis_location_geocoder-btn").click(function(){var a=s3_gis_geocode();$("#gis_location_lat").val(a[0]);$("#gis_location_lon").val(a[1])}),$("#gis_location_street").blur(function(){var a=$("#gis_location_lat").val(),c=$("#gis_location_lon").val();!a&&!c&&s3_gis_geocode()}),$("#gis_location_postcode").blur(function(){var a=$("#gis_location_lat").val(),c=$("#gis_location_lon").val();!a&&!c&&s3_gis_geocode()}),$("#gis_location_L3_ac").blur(function(){var a=
$("#gis_location_lat").val(),c=$("#gis_location_lon").val();!a&&!c&&s3_gis_geocode()}))}catch(a){$("#gis_location_geocoder-btn").hide()}}
function s3_gis_geocode(){var a=$("#gis_location_street").val();if(a)var b=a.search(/[A-z]/),a=a.substr(b);(b=$("#gis_location_postcode").val())&&(a+=", "+b);(b=$("#gis_location_L5_ac").val())&&(a+=", "+b);(b=$("#gis_location_L4_ac").val())&&(a+=", "+b);(b=$("#gis_location_L3_ac").val())&&(a+=", "+b);(b=$("#gis_location_L2_ac").val())&&(a+=", "+b);(b=$("#gis_location_L1_ac").val())&&(a+=", "+b);a={address:a};S3.gis.country&&(a.region=S3.gis.country);(new google.maps.Geocoder).geocode(a,function(a,
b){if(b==google.maps.GeocoderStatus.OK){var d=a[0].geometry.location,e=d.lat(),d=d.lng(),g=new OpenLayers.LonLat(d,e),h=a[0].geometry.viewport;if(h){var g=h.getNorthEast(),i=h.getSouthWest(),h=i.lng(),i=i.lat(),k=g.lng(),g=g.lat();s3_gis_zoomMap(h,i,k,g)}else if(S3.gis.mapWin.rendered){g.transform(S3.gis.proj4326,S3.gis.projection_current);map.setCenter(g)}else S3.gis.mapPanel.center=g;return[e,d]}s3_debug("Geocode was not successful for the following reason",b)})}
...
f=$("#gis_location_L3").val(),d=$("#gis_location_L4").val(),e=$("#gis_location_L5").val();if(!$("#gis_location_L0").val()){var b=S3.gis.site?"":$("#gis_location_name").val(),g=$("#gis_location_street").val(),h=$("#gis_location_postcode").val(),i=$("#gis_location_lat").val(),k=$("#gis_location_lon").val();if(a||c||f||d||e||b||g||h||i||k)return void 0==$("#country__error").val()&&$("#gis_location_L0").after('<div id="country__error" class="error" style="display: block;">'+i18n.gis_country_required+
