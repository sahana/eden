...
$("#gis_location_expand").removeClass("expand")):(s3_gis_show_tab("add"),$("#gis_location_expand").addClass("expand"),$("#gis_location_expand").removeClass("expanded"));a.preventDefault()});$("#gis_location_search_select-btn").click(function(a){s3_gis_select_search_result();a.preventDefault()});$("form").submit(function(){S3ClearNavigateAwayConfirm();return s3_gis_save_locations()?!0:!1})}var a=$("#gis_location_map-btn");if(a){var b=a.attr("map");void 0==b&&(b="default_map");$.when(function(){var a=
new jQuery.Deferred;setTimeout(function e(){void 0!=S3.gis.maps?a.resolve("loaded"):"pending"===a.state()&&(a.notify("waiting for JS to load..."),setTimeout(e,500))},1);return a.promise()}()).then(function(d){var c=S3.gis.maps[b];c.s3.pointPlaced=function(a){a=a.geometry.getBounds().getCenterLonLat();a.transform(c.getProjectionObject(),S3.gis.proj4326);$("#gis_location_lon").val(a.lon);$("#gis_location_lat").val(a.lat);$("#gis_location_wkt").val("")};a.click(function(){c.s3.mapWin.show();if(c.s3.polygonButton)$("#gis_location_wkt").val()||
($(".olMapViewport").addClass("crosshair"),c.s3.polygonButton.control.activate());else{var a=$("#gis_location_lat").val(),b=$("#gis_location_lon").val();a&&b||($(".olMapViewport").addClass("crosshair"),c.s3.pointButton.control.activate())}})})}}function s3_gis_autocompletes(){s3_gis_autocomplete(1);s3_gis_autocomplete(2);s3_gis_autocomplete(3);s3_gis_autocomplete(4);s3_gis_autocomplete(5);s3_gis_autocomplete_search()}
function s3_gis_autocomplete(a){var b=$("#gis_location_L"+a+"_ac");if(void 0!=b.val()){var d=$("#gis_location_L"+a),c=$("#gis_location_L"+a+"_throbber");b.autocomplete({delay:500,minLength:2,source:function(c,b){$.ajax({url:s3_gis_ac_set_source(a),data:{term:c.term}}).done(function(a){if(0==a.length){var c=i18n.no_matching_records;a.push({id:0,value:"",label:c,name:c})}b(a)})},search:function(a,b){c.removeClass("hide").show();d.val("");return!0},response:function(a,b,d){c.hide();return d},focus:function(a,
c){c.item.id&&b.val(c.item.name);return!1},select:function(a,c){var e=c.item;e.id?(b.val(e.name),d.val(e.id),"L1"==e.level&&e.parent&&""===$("#gis_location_L0").val()&&$("#gis_location_L0").val(e.parent),$("ul.ui-autocomplete").hide(),s3_gis_autocomplete(parseInt(e.level.replace("L",""),10)+1)):d.val("").change();return!1}}).data("ui-autocomplete")._renderItem=function(a,c){return $("<li></li>").data("item.autocomplete",c).append("<a>"+c.name+"</a>").appendTo(a)}}b.change(function(){for(var c=a+1;5>=
c;c++)$("#gis_location_L"+c+", #gis_location_L"+c+"_ac").val("")})}
function s3_gis_autocomplete_search(){var a=$("#gis_location_search_ac");if(void 0!=a.val()){var b=$("#gis_location_search_throbber");a.autocomplete({delay:500,minLength:2,source:function(a,c){$.ajax({url:s3_gis_ac_set_search_source(),data:{term:a.term}}).done(function(a){if(0==a.length){var b=i18n.no_matching_records;a.push({id:0,value:"",label:b,name:b})}c(a)})},search:function(a,c){b.removeClass("hide").show();$("#gis_location_search_select-btn").hide();return!0},response:function(a,c,e){b.hide();
return e},focus:function(b,c){c.item.id&&a.val(c.item.name);return!1},select:function(b,c){var d=c.item;d.id?(a.val(d.name),$("ul.ui-autocomplete").hide(),s3_gis_ac_search_selected(d)):a.val("");return!1}}).data("ui-autocomplete")._renderItem=function(a,c){var b=c.name&&c.addr_street?"<a>"+c.name+",  "+c.addr_street.split(",")[0].split("\n")[0]+"</a>":c.name?"<a>"+c.name+"</a>":"<a>"+c.addr_street.split(",")[0].split("\n")[0]+"</a>";return $("<li></li>").data("item.autocomplete",c).append(b).appendTo(a)}}}
function s3_gis_ac_set_source(a){var b=$("#gis_location_L"+(a-1)).val();if(""===b&&1<a){var d=$("#gis_location_L"+(a-2)).val();""===d&&2<a&&(d=$("#gis_location_L"+(a-2)).val(),""===d&&3<a&&(d=$("#gis_location_L"+(a-3)).val(),""===d&&4<a&&(d=$("#gis_location_L"+(a-4)).val(),""===d&&5<a&&(d=$("#gis_location_L"+(a-5)).val()))))}return b?S3.gis.url+"/search_ac?loc_select=1&level=L"+a+"&parent="+b:d?S3.gis.url+"/search_ac?loc_select=1&level=L"+a+"&children="+d:S3.gis.url+"/search_ac?loc_select=1&level=L"+
a}function s3_gis_ac_set_search_source(){return S3.gis.url+"/search_ac?loc_select=1&field2=addr_street&level=nullnone"}
function s3_gis_ac_search_selected(a){var b;for(b=0;5>=b;b++)$("#gis_location_L"+b+"_search").val(""),$("#gis_location_L"+b+"_label__row").hide(),$("#gis_location_L"+b+"_search__row").hide();var d=a.path,c=a.parent;if(d){var e=d.slice(0,d.toString().length+1).split("/"),f=e.length-1;if(0===f)$("#gis_location_search_throbber").hide(),$("#gis_location_search_select-btn").removeClass("hide").show();else for(b=0;b<=f;b++)b==f-1?s3_gis_search_hierarchy(e[b],!1,!0):s3_gis_search_hierarchy(e[b],!1,!1)}else c&&
s3_gis_search_hierarchy(c,!0,!0);a.name?($("#gis_location_name_label__row").removeClass("hide").show(),$("#gis_location_name_search").val(a.name),$("#gis_location_name_search__row").removeClass("hide").show()):($("#gis_location_name_label__row").hide(),$("#gis_location_name_search").val(""),$("#gis_location_name_search__row").hide());a.addr_street?($("#gis_location_street_label__row").removeClass("hide").show(),$("#gis_location_street_search").val(a.addr_street),$("#gis_location_street_search__row").removeClass("hide").show()):
...
$("#gis_location_lon_search").val(""),$("#gis_location_lon_search__row").hide()));$("body").data("id",a.id);d||c||$("#gis_location_search_select-btn").removeClass("hide").show()}
function s3_gis_search_hierarchy(a,b,d){$("#gis_location_search_throbber").removeClass("hide").show();$.ajax({async:!0,url:S3.gis.url+"/"+a+".json",dataType:"json"}).done(function(c){if(c[0].id==a){var e=c[0].name,f=c[0].level;c=c[0].parent;$("body").data(f,a);$("#gis_location_"+f+"_search").val(e);$("#gis_location_"+f+"_label__row").removeClass("hide").show();$("#gis_location_"+f+"_search__row").removeClass("hide").show();b&&c?s3_gis_search_hierarchy(c,!0,!0):d&&($("#gis_location_search_throbber").hide(),
...
function s3_gis_l0_select(){var a=$("#gis_location_L0").val(),b=S3.gis.url.replace("gis/location","gis/l0")+"/"+a;$.ajax({async:!0,url:b,dataType:"json"}).done(function(b){if(b.id==a){S3.gis.country=b.code;for(level=1;5>=level;level++){var c="L"+level;b[c]?($("#gis_location_"+c+"_label__row label").text(b[c]+":"),s3_gis_show_level(level)):s3_gis_hide_level(level);$("#gis_location_"+c+", #gis_location_"+c+"_ac").val("")}if(!S3.gis.no_map){var c=$("#gis_location_lat").val(),d=$("#gis_location_lon").val();
if(!c&&!d){var c=b.lon_min,d=b.lat_min,f=b.lon_max,g=b.lat_max;c&&d&&f&&g?s3_gis_zoomMap(c,d,f,g):(c=b.lat,d=b.lon,c&&d&&(b=S3.gis.maps.default_map,c=new OpenLayers.LonLat(d,c),c.transform(S3.gis.proj4326,b.getProjectionObject()),b.s3.mapWin.rendered?b.setCenter(c):b.s3.mapPanel.center=c))}}}})}
function s3_gis_zoomMap(a,b,d,c){if(S3.gis.maps){var e=S3.gis.maps.default_map,f=S3.gis.proj4326,g=e.getProjectionObject();e.s3.mapWin.rendered&&(b=new OpenLayers.LonLat(a,b),c=new OpenLayers.LonLat(d,c),b.transform(f,g),c.transform(f,g),a=b.lon,b=b.lat,d=c.lon,c=c.lat);d=OpenLayers.Bounds.fromArray([a,b,d,c]);a=d.getCenterLonLat();d=e.getZoomForExtent(d,!0);16<d&&(d=16);e.s3.mapWin.rendered?(e.setCenter(a),e.zoomTo(d)):(a.transform(f,g),e=e.s3.mapPanel,e.center=a,e.zoom=d)}}
...
function s3_gis_initGeocoder(){try{google&&(S3.gis.geocoder=new google.maps.Geocoder,$("#gis_location_geocoder-btn").click(function(){s3_gis_geocode(!0)}),$("#gis_location_street").blur(function(){var a=$("#gis_location_lat").val(),b=$("#gis_location_lon").val();a||b||s3_gis_geocode()}),$("#gis_location_postcode").blur(function(){var a=$("#gis_location_lat").val(),b=$("#gis_location_lon").val();a||b||s3_gis_geocode()}),$("#gis_location_L3_ac").blur(function(){var a=$("#gis_location_lat").val(),b=
$("#gis_location_lon").val();a||b||s3_gis_geocode()}))}catch(a){$("#gis_location_geocoder-btn").hide()}}
function s3_gis_geocode(a){var b=$("#gis_location_street").val();if(b)var d=b.search(/[A-z]/),b=b.substr(d);var c=$("#gis_location_postcode").val();c&&(b+=", "+c);(d=$("#gis_location_L5_ac").val())&&(b+=", "+d);var e=$("#gis_location_L4_ac").val();e&&(b+=", "+e);var f=$("#gis_location_L3_ac").val();f&&(b+=", "+f);var g=$("#gis_location_L2_ac").val();g&&(b+=", "+g);var h=$("#gis_location_L1_ac").val();h&&(b+=", "+h);b={address:b};S3.gis.country&&(b.region=S3.gis.country.toLowerCase());S3.gis.geocoder.geocode(b,
function(b,d){if(d==google.maps.GeocoderStatus.OK){var k=S3.gis.maps.default_map,l=b[0].geometry.location,p=l.lat(),l=l.lng(),m=new OpenLayers.LonLat(l,p);m.transform(S3.gis.proj4326,k.getProjectionObject());var n=b[0].geometry.viewport;if(n){var q=n.getNorthEast(),r=n.getSouthWest(),n=r.lng(),r=r.lat(),t=q.lng(),q=q.lat();s3_gis_zoomMap(n,r,t,q)}else k.s3.mapWin.rendered?k.setCenter(m):k.s3.mapPanel.center=m;if(a){$("#gis_location_lat").val(p);$("#gis_location_lon").val(l);var u=b[0].address_components;
$.each(u,function(a,b){var d=u[a];$.each(d.types,function(a,b){"postal_code"!=b||c?"administrative_area_level_1"!=b||h?"administrative_area_level_2"!=b||g?"administrative_area_level_3"!=b||f?"administrative_area_level_4"!=b||e||$("#gis_location_L4_ac").val(d.long_name):$("#gis_location_L3_ac").val(d.long_name):$("#gis_location_L2_ac").val(d.long_name):$("#gis_location_L1_ac").val(d.long_name):$("#gis_location_postcode").val(d.long_name)})});p=new OpenLayers.Feature.Vector(new OpenLayers.Geometry.Point(m.lon,
m.lat));k.s3.draftLayer.addFeatures([p])}}else s3_debug("Geocode was not successful for the following reason",d)})}
function s3_gis_save_locations(){$("#"+S3.gis.location_id).val()||$("#"+S3.gis.location_id).val("dummy");var a;if(S3.gis.site){var b="#"+S3.gis.site+"_name";if(a=$(b).val())$("#gis_location_name").val()||$("#gis_location_name").val(a);else return void 0==$("#name__error").val()&&$(b).after('<div id="name__error" class="error" style="display: block;">'+i18n.gis_name_required+"</div>"),$(b).focus(),S3.navigate_away_confirm&&S3SetNavigateAwayConfirm(),!1}var b=$("#gis_location_L1").val(),d=$("#gis_location_L2").val(),
c=$("#gis_location_L3").val(),e=$("#gis_location_L4").val(),f=$("#gis_location_L5").val();if(!$("#gis_location_L0").val()){a=S3.gis.site?"":$("#gis_location_name").val();var g=$("#gis_location_street").val(),h=$("#gis_location_postcode").val(),t=$("#gis_location_lat").val(),v=$("#gis_location_lon").val();if(b||d||c||e||f||a||g||h||t||v)return void 0==$("#country__error").val()&&$("#gis_location_L0").after('<div id="country__error" class="error" style="display: block;">'+i18n.gis_country_required+
"</div>"),$("#gis_location_L0").focus(),S3.navigate_away_confirm&&S3SetNavigateAwayConfirm(),!1}b||(b=$("#gis_location_L1_ac").val())&&$("#gis_location_L1").val(b);d||(d=$("#gis_location_L2_ac").val())&&$("#gis_location_L2").val(d);c||(c=$("#gis_location_L3_ac").val())&&$("#gis_location_L3").val(c);e||(e=$("#gis_location_L4_ac").val())&&$("#gis_location_L4").val(e);f||(f=$("#gis_location_L5_ac").val())&&$("#gis_location_L5").val(f);$("#gis_location_L0").prop("disabled",!1);$("#gis_location_name").prop("disabled",
