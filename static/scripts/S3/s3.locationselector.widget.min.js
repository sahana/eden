...
function s3_gis_autocomplete(a){void 0!=$("#gis_location_L"+a+"_ac").val()&&($("#gis_location_L"+a+"_ac").autocomplete({source:s3_gis_ac_set_source(a),delay:500,minLength:2,search:function(b,c){$("#gis_location_L"+a+"_throbber").removeClass("hide").show();$("#gis_location_L"+a).val("");return!0},response:function(b,c,e){$("#gis_location_L"+a+"_throbber").hide();return e},focus:function(b,c){$("#gis_location_L"+a+"_ac").val(c.item.name);return!1},select:function(b,c){$("#gis_location_L"+a+"_ac").val(c.item.name);
$("#gis_location_L"+a).val(c.item.id);"L1"==c.item.level&&(c.item.parent&&""===$("#gis_location_L0").val())&&$("#gis_location_L0").val(c.item.parent);$("ul.ui-autocomplete").hide();s3_gis_autocomplete(parseInt(c.item.level.replace("L",""),10)+1);return!1}}).data("ui-autocomplete")._renderItem=function(a,c){return $("<li></li>").data("item.autocomplete",c).append("<a>"+c.name+"</a>").appendTo(a)});$("#gis_location_L"+a+"_ac").change(function(){for(var b=a+1;5>=b;b++)$("#gis_location_L"+b+", #gis_location_L"+
b+"_ac").val("")})}
function s3_gis_autocomplete_search(){void 0!=$("#gis_location_search_ac").val()&&($("#gis_location_search_ac").autocomplete({source:s3_gis_ac_set_search_source(),delay:500,minLength:2,search:function(a,b){$("#gis_location_search_throbber").removeClass("hide").show();$("#gis_location_search_select-btn").hide();return!0},response:function(a,b,c){$("#gis_location_search_throbber").hide();return c},focus:function(a,b){$("#gis_location_search_ac").val(b.item.name);return!1},select:function(a,b){$("#gis_location_search_ac").val(b.item.name);
$("ul.ui-autocomplete").hide();s3_gis_ac_search_selected(b.item);return!1}}).data("ui-autocomplete")._renderItem=function(a,b){var c;c=b.name&&b.addr_street?"<a>"+b.name+",  "+b.addr_street.split(",")[0].split("\n")[0]+"</a>":b.name?"<a>"+b.name+"</a>":"<a>"+b.addr_street.split(",")[0].split("\n")[0]+"</a>";return $("<li></li>").data("item.autocomplete",b).append(c).appendTo(a)})}
...
b).val()||0,f=$(".minutes",b).val()||0,h=$(".seconds",b).val()||0,n=c($(".degrees",b)),l=c($(".minutes",b)),m=c($(".seconds",b)),p=c($(".decimal",b));b="gis_location_lat"==$(".decimal",b).attr("id");a(d)?(d=Number(d),Math.abs(d)>(b?90:180)?(alert(k.degrees[b?"lat":"lon"]),n()):a(f)?(f=Math.abs(f),60<f?(alert(k.minutes),l()):a(h)?(h=Math.abs(h),60<=h?(alert(k.seconds),m()):(d=(0>d?-1:1)*(Math.abs(d)+f/60+h/3600),Math.abs(d)>(b?90:180)?alert(k.decimal[b?"lat":"lon"]):(f=e(d),p(""+d),n(f.d||"0"),l(f.m||
"0"),m(f.s||"0")))):(alert(g.seconds),m())):(alert(g.minutes),l())):(alert(g.degrees),n())}function f(b){var c=$(".decimal",b),d=c.val(),f="gis_location_lat"==$(".decimal",b).attr("id");""!==d&&(a(d)?(d=Number(d),Math.abs(d)>(f?90:180)?(alert(k.decimal[f?"lat":"lon"]),c.focus()):(c=e(d),$(".degrees",b).val(c.d||"0"),$(".minutes",b).val(c.m||"0"),$(".seconds",b).val(c.s||"0"))):(alert(g.decimal),c.val("").focus()))}var g=i18n.gis_only_numbers,k=i18n.gis_range_error;$(".gis_coord_dms input").blur(function(){d(b($(this)))}).keypress(function(a){13==
...
if(!e&&!d){var e=b.lon_min,d=b.lat_min,f=b.lon_max,g=b.lat_max;e&&d&&f&&g?s3_gis_zoomMap(e,d,f,g):(e=b.lat,d=b.lon,e&&d&&(b=S3.gis.maps.default_map,e=new OpenLayers.LonLat(d,e),e.transform(S3.gis.proj4326,b.getProjectionObject()),b.s3.mapWin.rendered?b.setCenter(e):b.s3.mapPanel.center=e))}}}}});$("#gis_location_L1_ac").autocomplete("option","source",s3_gis_ac_set_source(1));$("#gis_location_L2_ac").autocomplete("option","source",s3_gis_ac_set_source(2));$("#gis_location_L3_ac").autocomplete("option",
"source",s3_gis_ac_set_source(3));$("#gis_location_L4_ac").autocomplete("option","source",s3_gis_ac_set_source(4));$("#gis_location_L5_ac").autocomplete("option","source",s3_gis_ac_set_source(5))}
function s3_gis_zoomMap(a,b,c,e){var d=S3.gis.maps.default_map,f=S3.gis.proj4326,g=d.getProjectionObject();d.s3.mapWin.rendered&&(b=new OpenLayers.LonLat(a,b),e=new OpenLayers.LonLat(c,e),b.transform(f,g),e.transform(f,g),a=b.lon,b=b.lat,c=e.lon,e=e.lat);c=OpenLayers.Bounds.fromArray([a,b,c,e]);a=c.getCenterLonLat();c=d.getZoomForExtent(c,!0);16<c&&(c=16);d.s3.mapWin.rendered?(d.setCenter(a),d.zoomTo(c)):(a.transform(f,g),d=d.s3.mapPanel,d.center=a,d.zoom=c)}
...
c=$("#gis_location_lon").val();!a&&!c&&s3_gis_geocode()}))}catch(a){$("#gis_location_geocoder-btn").hide()}}
function s3_gis_geocode(a){var b=$("#gis_location_street").val();if(b)var c=b.search(/[A-z]/),b=b.substr(c);var e=$("#gis_location_postcode").val();e&&(b+=", "+e);(c=$("#gis_location_L5_ac").val())&&(b+=", "+c);var d=$("#gis_location_L4_ac").val();d&&(b+=", "+d);var f=$("#gis_location_L3_ac").val();f&&(b+=", "+f);var g=$("#gis_location_L2_ac").val();g&&(b+=", "+g);var k=$("#gis_location_L1_ac").val();k&&(b+=", "+k);b={address:b};S3.gis.country&&(b.region=S3.gis.country.toLowerCase());S3.gis.geocoder.geocode(b,
function(b,c){if(c==google.maps.GeocoderStatus.OK){var q=S3.gis.maps.default_map,h=b[0].geometry.location,n=h.lat(),h=h.lng(),l=new OpenLayers.LonLat(h,n);l.transform(S3.gis.proj4326,q.getProjectionObject());var m=b[0].geometry.viewport;if(m){var p=m.getNorthEast(),r=m.getSouthWest(),m=r.lng(),r=r.lat(),v=p.lng(),p=p.lat();s3_gis_zoomMap(m,r,v,p)}else q.s3.mapWin.rendered?q.setCenter(l):q.s3.mapPanel.center=l;if(a){$("#gis_location_lat").val(n);$("#gis_location_lon").val(h);var s=b[0].address_components;
$.each(s,function(a,b){var c=s[a];$.each(c.types,function(a,b){"postal_code"==b&&!e?$("#gis_location_postcode").val(c.long_name):"administrative_area_level_1"==b&&!k?$("#gis_location_L1_ac").val(c.long_name):"administrative_area_level_2"==b&&!g?$("#gis_location_L2_ac").val(c.long_name):"administrative_area_level_3"==b&&!f?$("#gis_location_L3_ac").val(c.long_name):"administrative_area_level_4"==b&&!d&&$("#gis_location_L4_ac").val(c.long_name)})});n=new OpenLayers.Feature.Vector(new OpenLayers.Geometry.Point(l.lon,
...
e=$("#gis_location_L3").val(),d=$("#gis_location_L4").val(),f=$("#gis_location_L5").val();if(!$("#gis_location_L0").val()){a=S3.gis.site?"":$("#gis_location_name").val();var g=$("#gis_location_street").val(),k=$("#gis_location_postcode").val(),t=$("#gis_location_lat").val(),u=$("#gis_location_lon").val();if(b||c||e||d||f||a||g||k||t||u)return void 0==$("#country__error").val()&&$("#gis_location_L0").after('<div id="country__error" class="error" style="display: block;">'+i18n.gis_country_required+
