...
function s3_gis_lat_lon_converter(){function a(a){return!isNaN(parseFloat(a))&&isFinite(a)}function b(a){return a.parents(".gis_coord_wrap").eq(0)}function c(a){return function(b){a.val(b||"");"undefined"==typeof b&&a.focus()}}function d(a){var b=Math.abs(a),b=60*(b-parseInt(b,10));if(1E-10>Math.abs(b-Math.round(b)))b=Math.round(b),c=0;else{var c=60*(b-parseInt(b,10));1E-10>Math.abs(c-Math.round(c))&&(c=Math.round(c))}return{d:parseInt(a,10),m:parseInt(b,10),s:c}}function e(b){var e=$(".degrees",
b).val()||0,f=$(".minutes",b).val()||0,k=$(".seconds",b).val()||0,l=c($(".degrees",b)),m=c($(".minutes",b)),n=c($(".seconds",b)),p=c($(".decimal",b));b="gis_location_lat"==$(".decimal",b).attr("id");a(e)?(e=Number(e),Math.abs(e)>(b?90:180)?(alert(h.degrees[b?"lat":"lon"]),l()):a(f)?(f=Math.abs(f),60<f?(alert(h.minutes),m()):a(k)?(k=Math.abs(k),60<=k?(alert(h.seconds),n()):(e=(0>e?-1:1)*(Math.abs(e)+f/60+k/3600),Math.abs(e)>(b?90:180)?alert(h.decimal[b?"lat":"lon"]):(f=d(e),p(""+e),l(f.d||"0"),m(f.m||
"0"),n(f.s||"0")))):(alert(g.seconds),n())):(alert(g.minutes),m())):(alert(g.degrees),l())}function f(b){var c=$(".decimal",b),e=c.val(),f="gis_location_lat"==$(".decimal",b).attr("id");""!==e&&(a(e)?(e=Number(e),Math.abs(e)>(f?90:180)?(alert(h.decimal[f?"lat":"lon"]),c.focus()):(c=d(e),$(".degrees",b).val(c.d||"0"),$(".minutes",b).val(c.m||"0"),$(".seconds",b).val(c.s||"0"))):(alert(g.decimal),c.val("").focus()))}var g=i18n.gis_only_numbers,h=i18n.gis_range_error;$(".gis_coord_dms input").blur(function(){e(b($(this)))}).keypress(function(a){13==
a.which&&a.preventDefault()});$(".gis_coord_decimal input").blur(function(){f(b($(this)))}).keypress(function(a){13==a.which&&a.preventDefault()});$(".gis_coord_switch_dms").click(function(a){$(".gis_coord_dms").show();$(".gis_coord_decimal").hide();$(".gis_coord_wrap").each(function(){f($(this))});a.preventDefault()});$(".gis_coord_switch_decimal").click(function(a){$(".gis_coord_decimal").show();$(".gis_coord_dms").hide();$(".gis_coord_wrap").each(function(){e($(this))});a.preventDefault()});$(".gis_coord_wrap").each(function(){f($(this))})}
function s3_gis_search_hierarchy(a,b,c){$("#gis_location_search_throbber").removeClass("hide").show();$.ajax({async:!0,url:S3.gis.url+"/"+a+".json",dataType:"json",success:function(d){if(d[0].id==a){var e=d[0].name,f=d[0].level;d=d[0].parent;$("body").data(f,a);$("#gis_location_"+f+"_search").val(e);$("#gis_location_"+f+"_label__row").removeClass("hide").show();$("#gis_location_"+f+"_search__row").removeClass("hide").show();b&&d?s3_gis_search_hierarchy(d,!0,!0):c&&($("#gis_location_search_throbber").hide(),
...
if(!d&&!e){var d=b.lon_min,e=b.lat_min,f=b.lon_max,g=b.lat_max;d&&e&&f&&g?s3_gis_zoomMap(d,e,f,g):(d=b.lat,e=b.lon,d&&e&&(b=new OpenLayers.LonLat(e,d),b.transform(S3.gis.proj4326,S3.gis.projection_current),S3.gis.mapWin.rendered?map.setCenter(b):S3.gis.mapPanel.center=b))}}}}});$("#gis_location_L1_ac").autocomplete("option","source",s3_gis_ac_set_source(1));$("#gis_location_L2_ac").autocomplete("option","source",s3_gis_ac_set_source(2));$("#gis_location_L3_ac").autocomplete("option","source",s3_gis_ac_set_source(3));
$("#gis_location_L4_ac").autocomplete("option","source",s3_gis_ac_set_source(4));$("#gis_location_L5_ac").autocomplete("option","source",s3_gis_ac_set_source(5))}
function s3_gis_zoomMap(a,b,c,d){S3.gis.mapWin.rendered&&(b=new OpenLayers.LonLat(a,b),d=new OpenLayers.LonLat(c,d),b.transform(S3.gis.proj4326,S3.gis.projection_current),d.transform(S3.gis.proj4326,S3.gis.projection_current),a=b.lon,b=b.lat,c=d.lon,d=d.lat);c=OpenLayers.Bounds.fromArray([a,b,c,d]);a=c.getCenterLonLat();c=map.getZoomForExtent(c,!0);16<c&&(c=16);S3.gis.mapWin.rendered?(map.setCenter(a),map.zoomTo(c)):(a.transform(S3.gis.proj4326,S3.gis.projection_current),S3.gis.mapPanel.center=a,
...
function s3_gis_show_tab(a){$("#gis_location_tabs_row").removeClass("hide").show();"search"==a?s3_gis_search_tab():"add"==a?s3_gis_add_tab():"edit"==a?s3_gis_edit_tab():"view"==a&&s3_gis_view_tab()}
...
function s3_gis_edit_tab(){$("#gis_location_L0").prop("disabled",!1);$("#gis_location_name").prop("disabled",!1);$("#gis_location_street").prop("disabled",!1);$("#gis_location_postcode").prop("disabled",!1);$("#gis_location_lat").prop("disabled",!1);$("#gis_location_lon").prop("disabled",!1);$("#gis_location_L5_ac").prop("disabled",!1);$("#gis_location_L4_ac").prop("disabled",!1);$("#gis_location_L3_ac").prop("disabled",!1);$("#gis_location_L2_ac").prop("disabled",!1);$("#gis_location_L1_ac").prop("disabled",
!1);$("#gis_location_L0_label__row").removeClass("hide").show();$("#gis_location_L0__row").removeClass("hide").show();$("#gis_location_name_label__row").removeClass("hide").show();$("#gis_location_name__row").removeClass("hide").show();$("#gis_location_street_label__row").removeClass("hide").show();$("#gis_location_street__row").removeClass("hide").show();$("#gis_location_postcode_label__row").removeClass("hide").show();$("#gis_location_postcode__row").removeClass("hide").show();$("#gis_location_L5_ac").hasClass("hide")?
...
$("#gis_location_L3__row").hide()):($("#gis_location_L3_label__row").removeClass("hide").show(),$("#gis_location_L3__row").removeClass("hide").show());$("#gis_location_L2_ac").hasClass("hide")?($("#gis_location_L2_label__row").hide(),$("#gis_location_L2__row").hide()):($("#gis_location_L2_label__row").removeClass("hide").show(),$("#gis_location_L2__row").removeClass("hide").show());$("#gis_location_L1_ac").hasClass("hide")?($("#gis_location_L1_label__row").hide(),$("#gis_location_L1__row").hide()):
($("#gis_location_L1_label__row").removeClass("hide").show(),$("#gis_location_L1__row").removeClass("hide").show());S3.gis.no_latlon||($("#gis_location_lat_label__row").removeClass("hide").show(),$("#gis_location_lat__row").removeClass("hide").show(),$("#gis_location_lon_label__row").removeClass("hide").show(),$("#gis_location_lon__row").removeClass("hide").show());$("#gis_location_wkt_label__row").removeClass("hide").show();$("#gis_location_wkt__row").removeClass("hide").show();$("#gis_location_map-btn").html(i18n.gis_place_on_map);
$("#gis_location_map_button_row").show();$("#gis_location_geocoder-btn").removeClass("hide").show();$("#gis_loc_edit_tab").removeClass("tab_other").addClass("tab_here");$("#gis_loc_view_tab").removeClass("tab_here").addClass("tab_last")}
...
function(b,c){if(c==google.maps.GeocoderStatus.OK){var q=b[0].geometry.location,k=q.lat(),q=q.lng(),l=new OpenLayers.LonLat(q,k);l.transform(S3.gis.proj4326,S3.gis.projection_current);var m=b[0].geometry.viewport;if(m){var n=m.getNorthEast(),p=m.getSouthWest(),m=p.lng(),p=p.lat(),u=n.lng(),n=n.lat();s3_gis_zoomMap(m,p,u,n)}else S3.gis.mapWin.rendered?map.setCenter(l):S3.gis.mapPanel.center=l;if(a){$("#gis_location_lat").val(k);$("#gis_location_lon").val(q);var r=b[0].address_components;$.each(r,function(a,
b){var c=r[a];$.each(c.types,function(a,b){"postal_code"==b&&!d?$("#gis_location_postcode").val(c.long_name):"administrative_area_level_1"==b&&!h?$("#gis_location_L1_ac").val(c.long_name):"administrative_area_level_2"==b&&!g?$("#gis_location_L2_ac").val(c.long_name):"administrative_area_level_3"==b&&!f?$("#gis_location_L3_ac").val(c.long_name):"administrative_area_level_4"==b&&!e&&$("#gis_location_L4_ac").val(c.long_name)})});k=new OpenLayers.Feature.Vector(new OpenLayers.Geometry.Point(l.lon,l.lat));
S3.gis.draftLayer.addFeatures([k])}}else s3_debug("Geocode was not successful for the following reason",c)})}
...
d=$("#gis_location_L3").val(),e=$("#gis_location_L4").val(),f=$("#gis_location_L5").val();if(!$("#gis_location_L0").val()){a=S3.gis.site?"":$("#gis_location_name").val();var g=$("#gis_location_street").val(),h=$("#gis_location_postcode").val(),s=$("#gis_location_lat").val(),t=$("#gis_location_lon").val();if(b||c||d||e||f||a||g||h||s||t)return void 0==$("#country__error").val()&&$("#gis_location_L0").after('<div id="country__error" class="error" style="display: block;">'+i18n.gis_country_required+
