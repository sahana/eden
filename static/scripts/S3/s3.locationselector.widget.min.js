...
$("#gis_location_L"+a).val(c.item.id);"L1"==c.item.level&&(c.item.parent&&""==$("#gis_location_L0").val())&&$("#gis_location_L0").val(c.item.parent);$("ul.ui-autocomplete").hide();s3_gis_autocomplete(parseInt(c.item.level.replace("L",""))+1);return!1}}).data("autocomplete")._renderItem=function(a,c){return $("<li></li>").data("item.autocomplete",c).append("<a>"+c.name+"</a>").appendTo(a)});$("#gis_location_L"+a+"_ac").change(function(){for(var b=a+1;5>=b;b++)$("#gis_location_L"+b+", #gis_location_L"+
b+"_ac").val("")})}
...
function s3_gis_lat_lon_converter(){function a(a){return!isNaN(parseFloat(a))&&isFinite(a)}function b(a){return a.parents(".gis_coord_wrap").eq(0)}function c(a){return function(b){a.val(b||"");"undefined"==typeof b&&a.focus()}}function d(a){var b=Math.abs(a),b=60*(b-parseInt(b));if(1E-10>Math.abs(b-Math.round(b)))b=Math.round(b),c=0;else{var c=60*(b-parseInt(b));1E-10>Math.abs(c-Math.round(c))&&(c=Math.round(c))}return{d:parseInt(a),m:parseInt(b),s:c}}function e(b){var e=$(".degrees",b).val()||0,
f=$(".minutes",b).val()||0,j=$(".seconds",b).val()||0,k=c($(".degrees",b)),l=c($(".minutes",b)),m=c($(".seconds",b)),n=c($(".decimal",b));b="gis_location_lat"==$(".decimal",b).attr("id");a(e)?(e=Number(e),Math.abs(e)>(b?90:180)?(alert(h.degrees[b?"lat":"lon"]),k()):a(f)?(f=Math.abs(f),60<f?(alert(h.minutes),l()):a(j)?(j=Math.abs(j),60<=j?(alert(h.seconds),m()):(e=(0>e?-1:1)*(Math.abs(e)+f/60+j/3600),Math.abs(e)>(b?90:180)?alert(h.decimal[b?"lat":"lon"]):(f=d(e),n(""+e),k(f.d||"0"),l(f.m||"0"),m(f.s||
"0")))):(alert(g.seconds),m())):(alert(g.minutes),l())):(alert(g.degrees),k())}function f(b){var c=$(".decimal",b),e=c.val(),f="gis_location_lat"==$(".decimal",b).attr("id");""!=e&&(a(e)?(e=Number(e),Math.abs(e)>(f?90:180)?(alert(h.decimal[f?"lat":"lon"]),c.focus()):(c=d(e),$(".degrees",b).val(c.d||"0"),$(".minutes",b).val(c.m||"0"),$(".seconds",b).val(c.s||"0"))):(alert(g.decimal),c.val("").focus()))}var g=i18n.gis_only_numbers,h=i18n.gis_range_error;$(".gis_coord_dms input").blur(function(){e(b($(this)))}).keypress(function(a){13==
...
!0),$("#gis_location_L1__row").show());$("#gis_location_map-btn").html(i18n.gis_view_on_map);$("#gis_location_map_button_row").show();$("#gis_location_geocoder-btn").hide();$("#gis_loc_view_tab").removeClass("tab_other").addClass("tab_here");$("#gis_loc_edit_tab").removeClass("tab_here").addClass("tab_last")}
...
function(b,c){if(c==google.maps.GeocoderStatus.OK){var p=b[0].geometry.location,j=p.lat(),p=p.lng(),k=new OpenLayers.LonLat(p,j);k.transform(S3.gis.proj4326,S3.gis.projection_current);var l=b[0].geometry.viewport;if(l){var m=l.getNorthEast(),n=l.getSouthWest(),l=n.lng(),n=n.lat(),t=m.lng(),m=m.lat();s3_gis_zoomMap(l,n,t,m)}else S3.gis.mapWin.rendered?map.setCenter(k):S3.gis.mapPanel.center=k;if(a){$("#gis_location_lat").val(j);$("#gis_location_lon").val(p);var q=b[0].address_components;$.each(q,function(a){var b=
q[a];$.each(b.types,function(a,c){"postal_code"==c&&!d?$("#gis_location_postcode").val(b.long_name):"administrative_area_level_1"==c&&!h?$("#gis_location_L1_ac").val(b.long_name):"administrative_area_level_2"==c&&!g?$("#gis_location_L2_ac").val(b.long_name):"administrative_area_level_3"==c&&!f?$("#gis_location_L3_ac").val(b.long_name):"administrative_area_level_4"==c&&!e&&$("#gis_location_L4_ac").val(b.long_name)})});j=new OpenLayers.Feature.Vector(new OpenLayers.Geometry.Point(k.lon,k.lat));S3.gis.draftLayer.addFeatures([j])}}else s3_debug("Geocode was not successful for the following reason",
c)})}
function s3_gis_save_locations(){$("#"+S3.gis.location_id).val()||$("#"+S3.gis.location_id).val("dummy");if(S3.gis.site){var a="#"+S3.gis.site+"_name",b=$(a).val();if(b)$("#gis_location_name").val()||$("#gis_location_name").val(b);else return void 0==$("#name__error").val()&&$(a).after('<div id="name__error" class="error" style="display: block;">'+i18n.gis_name_required+"</div>"),$(a).focus(),S3.navigate_away_confirm&&S3SetNavigateAwayConfirm(),!1}var a=$("#gis_location_L1").val(),c=$("#gis_location_L2").val(),d=
$("#gis_location_L3").val(),e=$("#gis_location_L4").val(),f=$("#gis_location_L5").val();if(!$("#gis_location_L0").val()){var b=S3.gis.site?"":$("#gis_location_name").val(),g=$("#gis_location_street").val(),h=$("#gis_location_postcode").val(),r=$("#gis_location_lat").val(),s=$("#gis_location_lon").val();if(a||c||d||e||f||b||g||h||r||s)return void 0==$("#country__error").val()&&$("#gis_location_L0").after('<div id="country__error" class="error" style="display: block;">'+i18n.gis_country_required+"</div>"),
$("#gis_location_L0").focus(),S3.navigate_away_confirm&&S3SetNavigateAwayConfirm(),!1}a||(a=$("#gis_location_L1_ac").val())&&$("#gis_location_L1").val(a);c||(c=$("#gis_location_L2_ac").val())&&$("#gis_location_L2").val(c);d||(d=$("#gis_location_L3_ac").val())&&$("#gis_location_L3").val(d);e||(e=$("#gis_location_L4_ac").val())&&$("#gis_location_L4").val(e);f||(f=$("#gis_location_L5_ac").val())&&$("#gis_location_L5").val(f);$("#gis_location_L0").removeAttr("disabled");$("#gis_location_name").removeAttr("disabled");
