(function(){S3.addPersonWidget=function(b,d){var a="#"+b,e=$(a),c=$(a+"__row"),f=$(a+"_title__row"),g=e.next(".error_wrapper"),l=c.hasClass("control-group")||c.hasClass("form-row");if(l){var h=$(a+"_organisation_id__row"),k=$(a+"_full_name__row"),m=$(a+"_first_name__row"),n=$(a+"_middle_name__row"),p=$(a+"_last_name__row"),q=$(a+"_father_name__row"),t=$(a+"_grandfather_name__row"),u=$(a+"_date_of_birth__row"),v=$(a+"_year_of_birth__row"),w=$(a+"_gender__row"),x=$(a+"_occupation__row"),y=$(a+"_mobile_phone__row"),
z=$(a+"_home_phone__row"),A=$(a+"_email__row"),B=$(a+"_box_bottom");m.length?void 0==$(a+"_first_name").attr("data-c")?c.hide().after(B).after(A).after(z).after(y).after(x).after(w).after(u).after(v).after(t).after(q).after(m).after(n).after(p).after(h).after(g).after(f):c.hide().after(B).after(A).after(z).after(y).after(x).after(w).after(u).after(v).after(t).after(q).after(p).after(n).after(m).after(h).after(g).after(f):c.hide().after(B).after(A).after(z).after(y).after(x).after(w).after(u).after(v).after(t).after(q).after(p).after(n).after(m).after(k).after(h).after(g).after(f);
f.removeClass("hide").show();h.removeClass("hide").show();k.removeClass("hide").show();m.removeClass("hide").show();n.removeClass("hide").show();p.removeClass("hide").show();q.removeClass("hide").show();t.removeClass("hide").show();u.removeClass("hide").show();v.removeClass("hide").show();w.removeClass("hide").show();x.removeClass("hide").show();y.removeClass("hide").show();z.removeClass("hide").show();A.removeClass("hide").show();B.removeClass("hide").show()}else $(a+"__row1").hide(),c.hide().after(g).after(f);
e.val()?(E(b),$(a+"_edit_bar .icon-remove").hide()):(F(b),$(a+"_edit_bar .icon-edit").hide());e.data("lookup_contact",J);$(a+"_edit_bar").removeClass("hide").show();$(a+"_edit_bar .icon-edit").click(function(){var a="#"+b;F(b);$(a+"_full_name").prop("disabled",!1).val("");$(a+"_first_name").prop("disabled",!1).val("");$(a+"_middle_name").prop("disabled",!1).val("");$(a+"_last_name").prop("disabled",!1).val("");r(b);$(a+"_edit_bar .icon-edit").hide();$(a+"_edit_bar .icon-remove").removeClass("hide").show()});
...
$(a+"_edit_bar .icon-remove").hide()):($(a+"_full_name").prop("disabled",!1).val(""),$(a+"_first_name").prop("disabled",!1).val(""),$(a+"_middle_name").prop("disabled",!1).val(""),$(a+"_last_name").prop("disabled",!1).val(""),r(b))});if(d){c='<div id="'+b+'_duplicates" class="req"></div>';l?$(a+"_box_bottom").before(c):$(a+"_box_bottom1").before(c);var C=$(a+"_duplicates");C.data("results",[]).click(function(){G(b)});$(a+"_full_name,"+a+"_first_name,"+a+"_middle_name,"+a+"_last_name,"+a+"_date_of_birth,"+
...
var l=$(d+"_throbber"),d=a.attr("data-f"),e=S3.Ap.concat("/"+e+"/"+d+"/search_ac");c.data("url",e);a.autocomplete({delay:$(this).data("delay")||800,minLength:$(this).data("chars")||2,source:function(a,b){$.ajax({url:c.data("url"),data:{term:a.term}}).done(function(a){0==a.length?c.val(""):a.push({id:0,value:"",label:i18n.none_of_the_above});b(a)})},search:function(a,b){l.removeClass("hide").show();return!0},response:function(a,b,c){l.hide();return c},focus:function(a,b){return!1},select:function(d,
...
!1);r(b);var f=$(a),a=e.attr("data-f"),c=S3.Ap.concat("/"+c+"/"+a+"/"+d+"/lookup");$.getJSONS3(c,function(a){try{f.val(d);var c={value:d};if(fullname){var h=e.val();c.full_name=h}f.data("existing",c);I(a,b)}catch(k){f.val(""),r(b)}})},J=function(b,d){var a="#"+b,e=$(a+"_full_name");0==e.length?(fullname=!1,e=$(a+"_first_name"),e.prop("disabled",!1).val(""),void 0==e.attr("data-c")?(e=$(a+"_last_name"),e.prop("disabled",!1).val("")):$(a+"_last_name").prop("disabled",!1).val(""),$(a+"_middle_name").prop("disabled",
!1).val("")):(fullname=!0,e.prop("disabled",!1).val(""));r(b);var c=$(a),f=S3.Ap.concat("/org/site/"+d+"/site_contact_person");$.getJSONS3(f,function(d){try{var f=d.id;c.val(f);var h={value:f};if(fullname){var k=d.name;e.val(k);h.full_name=k}else{var m=d.first_name;$(a+"_first_name").val(m);h.first_name=m;var n=d.last_name;$(a+"_last_name").val(n);h.last_name=n;var p=$(a+"_middle_name");if(p.length){var q=d.middle_name;p.val(q);h.middle_name=q}}c.data("existing",h);I(d,b,f)}catch(t){c.val(""),fullname?
e.prop("disabled",!1).val(""):($(a+"_first_name").prop("disabled",!1).val(""),$(a+"_middle_name").prop("disabled",!1).val(""),$(a+"_last_name").prop("disabled",!1).val("")),r(b)}})},I=function(b,d){var a="#"+d,e=$(a).data("existing");if(b.hasOwnProperty("email")){var c=b.email;$(a+"_email").val(c);e.email=c}b.hasOwnProperty("mphone")&&(c=b.mphone,$(a+"_mobile_phone").val(c),e.mobile_phone=c);b.hasOwnProperty("hphone")&&(c=b.hphone,$(a+"_home_phone").val(c),e.home_phone=c);b.hasOwnProperty("sex")&&
...
i18n.grandfather_name_label+"</label>"+f.grandfather_name+"</div>"),f.year_of_birth&&(c+='<div class="card_1_line">'+f.year_of_birth+"</div>"),f.dob&&(c+='<div class="card_1_line">'+f.dob+"</div>"),f.email&&(c+='<div class="card_1_line">'+f.email+"</div>"),f.phone&&(c+='<div class="card_1_line">'+f.phone+"</div>"),c+='<button type="button" class="fright"><i class="icon icon-ok"> </i> '+i18n.Yes+"</button></div></div>",l+=c;l+='<div class="dl-item"><button type="button" class="fright"><i class="icon icon-remove"> </i> '+
