(function(){S3.addPersonWidget=function(b,d){var a="#"+b,e=$(a),c=$(a+"__row"),f=$(a+"_title__row"),g=e.next(".error_wrapper"),h=c.hasClass("control-group")||c.hasClass("form-row");if(h){var l=$(a+"_organisation_id__row"),k=$(a+"_full_name__row"),m=$(a+"_first_name__row"),n=$(a+"_middle_name__row"),p=$(a+"_last_name__row"),r=$(a+"_father_name__row"),t=$(a+"_grandfather_name__row"),u=$(a+"_date_of_birth__row"),v=$(a+"_year_of_birth__row"),w=$(a+"_gender__row"),x=$(a+"_occupation__row"),y=$(a+"_mobile_phone__row"),
z=$(a+"_home_phone__row"),A=$(a+"_email__row"),B=$(a+"_box_bottom");m.length?void 0==$(a+"_first_name").attr("data-c")?c.hide().after(B).after(A).after(z).after(y).after(x).after(w).after(u).after(v).after(t).after(r).after(m).after(n).after(p).after(l).after(g).after(f):c.hide().after(B).after(A).after(z).after(y).after(x).after(w).after(u).after(v).after(t).after(r).after(p).after(n).after(m).after(l).after(g).after(f):c.hide().after(B).after(A).after(z).after(y).after(x).after(w).after(u).after(v).after(t).after(r).after(p).after(n).after(m).after(k).after(l).after(g).after(f);
f.removeClass("hide").show();l.removeClass("hide").show();k.removeClass("hide").show();m.removeClass("hide").show();n.removeClass("hide").show();p.removeClass("hide").show();r.removeClass("hide").show();t.removeClass("hide").show();u.removeClass("hide").show();v.removeClass("hide").show();w.removeClass("hide").show();x.removeClass("hide").show();y.removeClass("hide").show();z.removeClass("hide").show();A.removeClass("hide").show();B.removeClass("hide").show()}else $(a+"__row1").hide(),c.hide().after(g).after(f);
e.val()?(E(b),$(a+"_edit_bar .icon-remove").hide()):(F(b),$(a+"_edit_bar .icon-edit").hide());e.data("lookup_contact",J);$(a+"_edit_bar").removeClass("hide").show();$(a+"_edit_bar .icon-edit").click(function(){var a="#"+b;F(b);$(a+"_full_name").prop("disabled",!1).val("");$(a+"_first_name").prop("disabled",!1).val("");$(a+"_middle_name").prop("disabled",!1).val("");$(a+"_last_name").prop("disabled",!1).val("");q(b);$(a+"_edit_bar .icon-edit").hide();$(a+"_edit_bar .icon-remove").removeClass("hide").show()});
...
$(a+"_edit_bar .icon-remove").hide()):($(a+"_full_name").prop("disabled",!1).val(""),$(a+"_first_name").prop("disabled",!1).val(""),$(a+"_middle_name").prop("disabled",!1).val(""),$(a+"_last_name").prop("disabled",!1).val(""),q(b))});if(d){c='<div id="'+b+'_duplicates" class="req"></div>';h?$(a+"_box_bottom").before(c):$(a+"_box_bottom1").before(c);var C=$(a+"_duplicates");C.data("results",[]).click(function(){G(b)});$(a+"_full_name,"+a+"_first_name,"+a+"_middle_name,"+a+"_last_name,"+a+"_date_of_birth,"+
...
var h=$(d+"_throbber");d=a.attr("data-f");e=S3.Ap.concat("/"+e+"/"+d+"/search_ac");c.data("url",e);a.autocomplete({delay:$(this).data("delay")||800,minLength:$(this).data("chars")||2,source:function(a,b){$.ajax({url:c.data("url"),data:{term:a.term}}).done(function(a){0==a.length?c.val(""):a.push({id:0,value:"",label:i18n.none_of_the_above});b(a)})},search:function(a,b){h.removeClass("hide").show();return!0},response:function(a,b,c){h.hide();return c},focus:function(a,b){return!1},select:function(d,
e){d=e.item;if(d.id){if(void 0!=d.label)var f=d.label;else fullname&&(f=d.name);a.val(f);c.val(d.id).change();H(b,d.id)}else c.val("").change();return!1}}).data("ui-autocomplete")._renderItem=function(a,b){if(void 0!=b.label)var c=b.label;else{c=b.name;var d=b.org,e=b.job;if(d||e)e?(c+=" ("+e,d&&(c+=", "+d),c+=")"):c+=" ("+d+")"}return $("<li>").data("item.autocomplete",b).append("<a>"+c+"</a>").appendTo(a)};a.blur(function(){fullname?g.hasOwnProperty("full_name")&&g.full_name!=a.val()&&c.val("").change():
...
!1);q(b);var f=$(a);a=e.attr("data-f");c=S3.Ap.concat("/"+c+"/"+a+"/"+d+"/lookup.json");$.getJSONS3(c,function(a){try{f.val(d);var c={value:d};if(fullname){var g=e.val();c.full_name=g}f.data("existing",c);I(a,b)}catch(k){f.val(""),q(b)}})},J=function(b,d){var a="#"+b,e=$(a+"_full_name");0==e.length?(fullname=!1,e=$(a+"_first_name"),e.prop("disabled",!1).val(""),void 0==e.attr("data-c")?(e=$(a+"_last_name"),e.prop("disabled",!1).val("")):$(a+"_last_name").prop("disabled",!1).val(""),$(a+"_middle_name").prop("disabled",
!1).val("")):(fullname=!0,e.prop("disabled",!1).val(""));q(b);var c=$(a);d=S3.Ap.concat("/org/site/"+d+"/site_contact_person");$.getJSONS3(d,function(d){try{var f=d.id;c.val(f);var h={value:f};if(fullname){var l=d.name;e.val(l);h.full_name=l}else{var k=d.first_name;$(a+"_first_name").val(k);h.first_name=k;var m=d.last_name;$(a+"_last_name").val(m);h.last_name=m;var n=$(a+"_middle_name");if(n.length){var p=d.middle_name;n.val(p);h.middle_name=p}}c.data("existing",h);I(d,b,f)}catch(r){c.val(""),fullname?
e.prop("disabled",!1).val(""):($(a+"_first_name").prop("disabled",!1).val(""),$(a+"_middle_name").prop("disabled",!1).val(""),$(a+"_last_name").prop("disabled",!1).val("")),q(b)}})},I=function(b,d){var a="#"+d,e=$(a).data("existing");if(b.hasOwnProperty("email")){var c=b.email;$(a+"_email").val(c);e.email=c}b.hasOwnProperty("mphone")&&(c=b.mphone,$(a+"_mobile_phone").val(c),e.mobile_phone=c);b.hasOwnProperty("hphone")&&(c=b.hphone,$(a+"_home_phone").val(c),e.home_phone=c);b.hasOwnProperty("sex")&&
...
e.occupation=c);b.hasOwnProperty("org_id")&&(c=b.org_id,$(a+"_organisation_id").val(c),e.organisation_id=c);b.hasOwnProperty("first_name")&&(c=b.first_name,$(a+"_first_name").val(c),e.first_name=c,c=b.last_name,$(a+"_last_name").val(c),e.last_name=c,b.hasOwnProperty("middle_name")&&(b=b.middle_name,$(a+"_middle_name").val(b),e.middle_name=b));E(d)},K=function(b){var d="#"+b;if(!$(d).val()){b=$(d+"_full_name");if(0==b.length){b=$(d+"_first_name").val();if(!b)return;b={first_name:b};var a=$(d+"_last_name").val();
...
$.ajaxS3({url:a,data:b,type:"POST",dataType:"json",success:function(a){$(d+"_results").remove();if(a.length){var b=i18n.dupes_found.replace("_NUM_",a.length);$(d+"_duplicates").html(b+' <i class="icon-caret-down"> </i>').show().data("results",a)}else $(d+"_duplicates").data("results",[]).hide()}})}},G=function(b){var d="#"+b;if(!$(d+"_results").length){for(var a=$(d+"_duplicates"),e=a.data("results"),c,f,g,h='<div id="'+b+'_results">',l=e.length,k=0;k<l;k++)f=e[k],g=f.name,c='<div class="dl-item" data-id='+
...
i18n.grandfather_name_label+"</label>"+f.grandfather_name+"</div>"),f.year_of_birth&&(c+='<div class="card_1_line">'+f.year_of_birth+"</div>"),f.dob&&(c+='<div class="card_1_line">'+f.dob+"</div>"),f.email&&(c+='<div class="card_1_line">'+f.email+"</div>"),f.phone&&(c+='<div class="card_1_line">'+f.phone+"</div>"),c+='<button type="button" class="fright"><i class="icon icon-ok"> </i> '+i18n.Yes+"</button></div></div>",h+=c;h+='<div class="dl-item"><button type="button" class="fright"><i class="icon icon-remove"> </i> '+
i18n.No+"</button></div></div>";a.after(h);$("html,body").animate({scrollTop:a.offset().top},250);$(d+"_results .dl-item").click(function(){$this=$(this);if(g=$this.attr("data-name")){$(d+"_full_name").val(g);var c=$this.attr("data-id");D(b);a.data("submit")?($(d).val(c),a.closest("form").submit()):H(b,c)}else D(b),a.data("submit")&&a.closest("form").submit()})}},D=function(b){b="#"+b;$(b+"_duplicates").data("results",[]).empty();$(b+"_results").remove()}})();
