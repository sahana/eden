...
var $jscomp$lookupPolyfilledValue=function(f,z){var F=$jscomp.propertyToPolyfillSymbol[z];if(null==F)return f[z];F=f[F];return void 0!==F?F:f[z]};$jscomp.polyfill=function(f,z,F,p){z&&($jscomp.ISOLATE_POLYFILLS?$jscomp.polyfillIsolated(f,z,F,p):$jscomp.polyfillUnisolated(f,z,F,p))};
$jscomp.polyfillUnisolated=function(f,z,F,p){F=$jscomp.global;f=f.split(".");for(p=0;p<f.length-1;p++){var r=f[p];if(!(r in F))return;F=F[r]}f=f[f.length-1];p=F[f];z=z(p);z!=p&&null!=z&&$jscomp.defineProperty(F,f,{configurable:!0,writable:!0,value:z})};
$jscomp.polyfillIsolated=function(f,z,F,p){var r=f.split(".");f=1===r.length;p=r[0];p=!f&&p in $jscomp.polyfills?$jscomp.polyfills:$jscomp.global;for(var A=0;A<r.length-1;A++){var w=r[A];if(!(w in p))return;p=p[w]}r=r[r.length-1];F=$jscomp.IS_SYMBOL_NATIVE&&"es6"===F?p[r]:null;z=z(F);null!=z&&(f?$jscomp.defineProperty($jscomp.polyfills,r,{configurable:!0,writable:!0,value:z}):z!==F&&(void 0===$jscomp.propertyToPolyfillSymbol[r]&&(F=1E9*Math.random()>>>0,$jscomp.propertyToPolyfillSymbol[r]=$jscomp.IS_SYMBOL_NATIVE?
$jscomp.global.Symbol(r):$jscomp.POLYFILL_PREFIX+F+"$"+r),$jscomp.defineProperty(p,$jscomp.propertyToPolyfillSymbol[r],{configurable:!0,writable:!0,value:z})))};
...
R.parents.length;J<O;J++)L[R.parents[J]].children_d=L[R.parents[J]].children_d.concat(y);t={cnt:T,mod:L,sel:m,par:u,dpc:y,add:q};if("undefined"===typeof window||"undefined"===typeof window.document)postMessage(t);else return t},l=function(m,n){if(null!==this.element){this._cnt=m.cnt;var t=this._model.data;for(D in t)t.hasOwnProperty(D)&&t[D].state&&t[D].state.loading&&m.mod[D]&&(m.mod[D].state.loading=!0);this._model.data=m.mod;if(n){var u,v=m.add,y=m.sel,q=this._data.core.selected.slice();t=this._model.data;
if(y.length!==q.length||f.vakata.array_unique(y.concat(q)).length!==y.length){var D=0;for(u=y.length;D<u;D++)-1===f.inArray(y[D],v)&&-1===f.inArray(y[D],q)&&(t[y[D]].state.selected=!1);D=0;for(u=q.length;D<u;D++)-1===f.inArray(q[D],y)&&(t[q[D]].state.selected=!0)}}m.add.length&&(this._data.core.selected=this._data.core.selected.concat(m.add));this.trigger("model",{nodes:m.dpc,parent:m.par});m.par!==f.jstree.root?(this._node_changed(m.par),this.redraw()):this.redraw(!0);m.add.length&&this.trigger("changed",
...
(a.childNodes[1].setAttribute("aria-setsize",n[e.parent].children.length),a.childNodes[1].setAttribute("aria-posinset",n[e.parent].children.indexOf(e.id)+1));e.state.disabled&&a.childNodes[1].setAttribute("aria-disabled",!0);l=0;for(m=e.children.length;l<m;l++)if(!n[e.children[l]].state.hidden){y=!0;break}if(null!==e.parent&&n[e.parent]&&!e.state.hidden&&(l=f.inArray(e.id,n[e.parent].children),q=e.id,-1!==l))for(l++,m=n[e.parent].children.length;l<m&&(n[n[e.parent].children[l]].state.hidden||(q=n[e.parent].children[l]),
q===e.id);l++);e.state.hidden&&(t+=" jstree-hidden");e.state.loading&&(t+=" jstree-loading");e.state.loaded&&!y?t+=" jstree-leaf":(t+=e.state.opened&&e.state.loaded?" jstree-open":" jstree-closed",a.childNodes[1].setAttribute("aria-expanded",e.state.opened&&e.state.loaded));q===e.id&&(t+=" jstree-last");a.id=e.id;a.className=t;t=(e.state.selected?" jstree-clicked":"")+(e.state.disabled?" jstree-disabled":"");for(m in e.a_attr)!e.a_attr.hasOwnProperty(m)||"href"===m&&"#"===e.a_attr[m]||("class"!==
...
this.redraw(t.id===f.jstree.root)}else{l=a.children_d.concat();l.push(a.id);y=0;for(m=a.parents.length;y<m;y++){e=[];var q=u._model.data[a.parents[y]].children_d;var D=0;for(n=q.length;D<n;D++)-1===f.inArray(q[D],l)&&e.push(q[D]);u._model.data[a.parents[y]].children_d=e}u._model.data[k].children=f.vakata.array_remove_item(u._model.data[k].children,a.id);y=0;for(m=t.parents.length;y<m;y++)this._model.data[t.parents[y]].children_d=this._model.data[t.parents[y]].children_d.concat(l);e=[];y=0;for(m=t.children.length;y<
m;y++)e[y>=b?y+1:y]=t.children[y];e[b]=a.id;t.children=e;t.children_d.push(a.id);t.children_d=t.children_d.concat(a.children_d);a.parent=t.id;l=t.parents.concat();l.unshift(t.id);q=a.parents.length;a.parents=l;l=l.concat();y=0;for(m=a.children_d.length;y<m;y++)this._model.data[a.children_d[y]].parents=this._model.data[a.children_d[y]].parents.slice(0,-1*q),Array.prototype.push.apply(this._model.data[a.children_d[y]].parents,l);if(k===f.jstree.root||t.id===f.jstree.root)this._model.force_full_redraw=
...
new_instance:this});return l.id},cut:function(a){a||(a=this._data.core.selected.concat());f.vakata.is_array(a)||(a=[a]);if(!a.length)return!1;var d=[],b,c;var e=0;for(c=a.length;e<c;e++)(b=this.get_node(a[e]))&&b.id&&b.id!==f.jstree.root&&d.push(b);if(!d.length)return!1;p=d;A=this;r="move_node";this.trigger("cut",{node:a})},copy:function(a){a||(a=this._data.core.selected.concat());f.vakata.is_array(a)||(a=[a]);if(!a.length)return!1;var d=[],b,c;var e=0;for(c=a.length;e<c;e++)(b=this.get_node(a[e]))&&
b.id&&b.id!==f.jstree.root&&d.push(b);if(!d.length)return!1;p=d;A=this;r="copy_node";this.trigger("copy",{node:a})},get_buffer:function(){return{mode:r,node:p,inst:A}},can_paste:function(){return!1!==r&&!1!==p},paste:function(a,d){a=this.get_node(a);if(!(a&&r&&r.match(/^(copy_node|move_node)$/)&&p))return!1;this[r](p,a,d,!1,!1,!1,A)&&this.trigger("paste",{parent:a.id,node:p,mode:r});A=r=p=!1},clear_buffer:function(){A=r=p=!1;this.trigger("clear_buffer")},edit:function(a,d,b){var c=!1;a=this.get_node(a);
...
n.get_node(this);if(y)if(y.state.loaded)for(u=0,c=y.children_d.length;u<c;u++){var q=g[y.children_d[u]];if(!q.state.loaded&&q.original&&q.original.state&&q.original.state.undetermined&&!0===q.original.state.undetermined)for(h[q.id]===z&&q.id!==f.jstree.root&&(h[q.id]=!0,m.push(q.id)),v=0,e=q.parents.length;v<e;v++)h[q.parents[v]]===z&&q.parents[v]!==f.jstree.root&&(h[q.parents[v]]=!0,m.push(q.parents[v]))}else if(y.original&&y.original.state&&y.original.state.undetermined&&!0===y.original.state.undetermined)for(h[y.id]===
...
function(y,q){if(!q)return!0;g.push(q);!u&&q.separator_before&&(t+="<li class='vakata-context-separator'><a href='#' "+(a.vakata.context.settings.icons?"":'class="vakata-context-no-icons"')+">&#160;</a></li>");u=!1;t+="<li class='"+(q._class||"")+(!0===q._disabled||a.vakata.is_function(q._disabled)&&q._disabled({item:q,reference:c,element:b})?" vakata-contextmenu-disabled ":"")+"' "+(q.shortcut?" data-shortcut='"+q.shortcut+"' ":"")+">";t+="<a href='#' rel='"+(g.length-1)+"' "+(q.title?"title='"+
q.title+"'":"")+">";a.vakata.context.settings.icons&&(t+="<i ",q.icon&&(t=-1!==q.icon.indexOf("/")||-1!==q.icon.indexOf(".")?t+(" style='background:url(\""+q.icon+"\") center center no-repeat' "):t+(" class='"+q.icon+"' ")),t+="></i><span class='vakata-contextmenu-sep'>&#160;</span>");t+=(a.vakata.is_function(q.label)?q.label({item:y,reference:c,element:b}):q.label)+(q.shortcut?' <span class="vakata-contextmenu-shortcut vakata-contextmenu-shortcut-'+q.shortcut+'">'+(q.shortcut_label||"")+"</span>":
"")+"</a>";q.submenu&&(v=a.vakata.context._parse(q.submenu,!0))&&(t+=v);t+="</li>";q.separator_after&&(t+="<li class='vakata-context-separator'><a href='#' "+(a.vakata.context.settings.icons?"":'class="vakata-context-no-icons"')+">&#160;</a></li>",u=!0)});t=t.replace(/<li class='vakata-context-separator'><\/li>$/,"");n&&(t+="</ul>");n||(k=t,a.vakata.context._trigger("parse"));return 10<t.length?t:!1},_show_submenu:function(m){m=a(m);if(m.length&&m.children("ul").length){var n=m.children("ul"),t=m.offset().left,
u=t+m.outerWidth(),v=m.offset().top,y=n.width(),q=n.height(),D=a(window).width()+a(window).scrollLeft(),K=a(window).height()+a(window).scrollTop();if(d)m[0>u-(y+10+m.outerWidth())?"addClass":"removeClass"]("vakata-context-left");else m[u+y>D&&t>D-u?"addClass":"removeClass"]("vakata-context-right");v+q+10>K&&n.css("bottom","-1px");m.hasClass("vakata-context-right")?t<y&&n.css("margin-right",t-y):D-u<y&&n.css("margin-left",D-u-y);n.show()}},show:function(m,n,t){b&&b.length&&b.width("");switch(!0){case !n&&
!m:return!1;case !!n&&!!m:c=m;e=n.x;h=n.y;break;case !n&&!!m:c=m;n=m.offset();e=n.left+m.outerHeight();h=n.top;break;case !!n&&!m:e=n.x,h=n.y}m&&!t&&a(m).data("vakata_contextmenu")&&(t=a(m).data("vakata_contextmenu"));a.vakata.context._parse(t)&&b.html(k);if(g.length){b.appendTo(x.body);t=b;n=e;var u=h;var v=t.width();var y=t.height();var q=a(window).width()+a(window).scrollLeft();var D=a(window).height()+a(window).scrollTop();d&&(n-=t.outerWidth()-a(m).outerWidth(),n<a(window).scrollLeft()+20&&(n=
a(window).scrollLeft()+20));n+v+20>q&&(n=q-(v+20));u+y+20>D&&(u=D-(y+20));b.css({left:n,top:u}).show().find("a").first().trigger("focus").parent().addClass("vakata-context-hover");l=!0;a.vakata.context._trigger("show")}},hide:function(){l&&(b.hide().find("ul").hide().end().find(":focus").trigger("blur").end().detach(),l=!1,a.vakata.context._trigger("hide"))}};a(function(){d="rtl"===a(x.body).css("direction");var m=!1;b=a("<ul class='vakata-context'></ul>");b.on("mouseenter","li",function(n){n.stopImmediatePropagation();
...
g.event;var l=f.jstree.reference(g.event.target),m=!1,n=!1;h=!1;var t,u,v,y,q,D,K,T,L,R,O,P;if(l&&l._data&&l._data.dnd){e.attr("class","jstree-"+l.get_theme()+(l.settings.core.themes.responsive?" jstree-dnd-responsive":""));var X=g.data.origin&&(g.data.origin.settings.dnd.always_copy||g.data.origin.settings.dnd.copy&&(g.event.metaKey||g.event.ctrlKey));g.helper.children().attr("class","jstree-"+l.get_theme()+" jstree-"+l.get_theme()+"-"+l.get_theme_variant()+" "+(l.settings.core.themes.responsive?
...
escape:function(q){return a.vakata.html.div.text(q).html()},strip:function(q){return a.vakata.html.div.empty().append(a.parseHTML(q)).text()}};var d=!1,b=!1,c=!1,e=!1,h=!1,g=0,k=!1,l=0,m=0,n=0,t=0,u=!1,v=!1,y=!1;a.vakata.dnd={settings:{scroll_speed:10,scroll_proximity:20,helper_left:5,helper_top:10,threshold:5,threshold_touch:10},_trigger:function(q,D,K){K===z&&(K=a.vakata.dnd._get());K.event=D;a(x).triggerHandler("dnd_"+q+".vakata",K)},_get:function(){return{data:k,element:d,helper:h}},_clean:function(){h&&
h.remove();v&&(clearInterval(v),v=!1);h=e=c=b=d=!1;g=0;k=!1;t=n=m=l=0;y=v=u=!1;Q=null;a(x).off("mousemove.vakata.jstree touchmove.vakata.jstree",a.vakata.dnd.drag);a(x).off("mouseup.vakata.jstree touchend.vakata.jstree",a.vakata.dnd.stop)},_scroll:function(q){if(!u||!n&&!t)return v&&(clearInterval(v),v=!1),!1;if(!v)return v=setInterval(a.vakata.dnd._scroll,100),!1;if(!0===q)return!1;q=u.scrollTop();var D=u.scrollLeft();u.scrollTop(q+t*a.vakata.dnd.settings.scroll_speed);u.scrollLeft(D+n*a.vakata.dnd.settings.scroll_speed);
q===u.scrollTop()&&D===u.scrollLeft()||a.vakata.dnd._trigger("scroll",u)},start:function(q,D,K){"touchstart"===q.type&&q.originalEvent&&q.originalEvent.changedTouches&&q.originalEvent.changedTouches[0]&&(q.pageX=q.originalEvent.changedTouches[0].pageX,q.pageY=q.originalEvent.changedTouches[0].pageY,q.target=x.elementFromPoint(q.originalEvent.changedTouches[0].pageX-window.pageXOffset,q.originalEvent.changedTouches[0].pageY-window.pageYOffset));e&&a.vakata.dnd.stop({});try{q.currentTarget.unselectable=
"on",q.currentTarget.onselectstart=function(){return!1},q.currentTarget.style&&(q.currentTarget.style.touchAction="none",q.currentTarget.style.msTouchAction="none",q.currentTarget.style.MozUserSelect="none")}catch(T){}l=q.pageX;m=q.pageY;k=D;c=!0;d=q.currentTarget;b=q.target;y="touchstart"===q.type;!1!==K&&(h=a("<div id='vakata-dnd'></div>").html(K).css({display:"block",margin:"0",padding:"0",position:"absolute",top:"-2000px",lineHeight:"16px",zIndex:"10000"}));a(x).on("mousemove.vakata.jstree touchmove.vakata.jstree",
a.vakata.dnd.drag);a(x).on("mouseup.vakata.jstree touchend.vakata.jstree",a.vakata.dnd.stop);return!1},drag:function(q){"touchmove"===q.type&&q.originalEvent&&q.originalEvent.changedTouches&&q.originalEvent.changedTouches[0]&&(q.pageX=q.originalEvent.changedTouches[0].pageX,q.pageY=q.originalEvent.changedTouches[0].pageY,q.target=x.elementFromPoint(q.originalEvent.changedTouches[0].pageX-window.pageXOffset,q.originalEvent.changedTouches[0].pageY-window.pageYOffset));if(c){if(!e)if(Math.abs(q.pageX-
l)>(y?a.vakata.dnd.settings.threshold_touch:a.vakata.dnd.settings.threshold)||Math.abs(q.pageY-m)>(y?a.vakata.dnd.settings.threshold_touch:a.vakata.dnd.settings.threshold))h&&(h.appendTo(x.body),g=h.outerWidth()),e=!0,a(b).one("click.vakata",!1),a.vakata.dnd._trigger("start",q);else return;var D=!1,K=!1,T=!1,L=!1,R=!1,O=K=!1,P=!1;L=D=!1;n=t=0;u=!1;a(a(q.target).parentsUntil("body").addBack().get().reverse()).filter(function(){return/^auto|scroll$/.test(a(this).css("overflow"))&&(this.scrollHeight>
this.offsetHeight||this.scrollWidth>this.offsetWidth)}).each(function(){var X=a(this),J=X.offset();this.scrollHeight>this.offsetHeight&&(J.top+X.height()-q.pageY<a.vakata.dnd.settings.scroll_proximity&&(t=1),q.pageY-J.top<a.vakata.dnd.settings.scroll_proximity&&(t=-1));this.scrollWidth>this.offsetWidth&&(J.left+X.width()-q.pageX<a.vakata.dnd.settings.scroll_proximity&&(n=1),q.pageX-J.left<a.vakata.dnd.settings.scroll_proximity&&(n=-1));if(t||n)return u=a(this),!1});!u&&(D=a(x),K=a(window),T=D.height(),
L=K.height(),R=D.width(),K=K.width(),O=D.scrollTop(),P=D.scrollLeft(),T>L&&q.pageY-O<a.vakata.dnd.settings.scroll_proximity&&(t=-1),T>L&&L-(q.pageY-O)<a.vakata.dnd.settings.scroll_proximity&&(t=1),R>K&&q.pageX-P<a.vakata.dnd.settings.scroll_proximity&&(n=-1),R>K&&K-(q.pageX-P)<a.vakata.dnd.settings.scroll_proximity&&(n=1),t||n)&&(u=D);u&&a.vakata.dnd._scroll(!0);h&&(D=parseInt(q.pageY+a.vakata.dnd.settings.helper_top,10),L=parseInt(q.pageX+a.vakata.dnd.settings.helper_left,10),T&&D+25>T&&(D=T-50),
R&&L+g>R&&(L=R-(g+2)),h.css({left:L+"px",top:D+"px"}));a.vakata.dnd._trigger("move",q);return!1}},stop:function(q){"touchend"===q.type&&q.originalEvent&&q.originalEvent.changedTouches&&q.originalEvent.changedTouches[0]&&(q.pageX=q.originalEvent.changedTouches[0].pageX,q.pageY=q.originalEvent.changedTouches[0].pageY,q.target=x.elementFromPoint(q.originalEvent.changedTouches[0].pageX-window.pageXOffset,q.originalEvent.changedTouches[0].pageY-window.pageYOffset));if(e)q.target!==b&&a(b).off("click.vakata"),
a.vakata.dnd._trigger("stop",q);else if("touchend"===q.type&&q.target===b){var D=setTimeout(function(){a(q.target).trigger("click")},100);a(q.target).one("click",function(){D&&clearTimeout(D)})}a.vakata.dnd._clean();return!1}}})(f);f.jstree.defaults.massload=null;f.jstree.plugins.massload=function(a,d){this.init=function(b,c){this._data.massload={};d.init.call(this,b,c)};this._load_nodes=function(b,c,e,h){var g=this.settings.massload,k=[],l=this._model.data,m,n;if(!e){var t=0;for(m=b.length;t<m;t++)if(!l[b[t]]||
...
g&&g.url)return g=f.extend(!0,{},g),f.vakata.is_function(g.url)&&(g.url=g.url.call(this,k)),f.vakata.is_function(g.data)&&(g.data=g.data.call(this,k)),f.ajax(g).done(function(u,v,y){if(u)for(q in u)u.hasOwnProperty(q)&&(this._data.massload[q]=u[q]);var q=0;for(u=b.length;q<u;q++)(n=this.get_node(b[q],!0))&&n.length&&n.removeClass("jstree-loading").attr("aria-busy",!1);d._load_nodes.call(this,b,c,e,h)}.bind(this)).fail(function(u){d._load_nodes.call(this,b,c,e,h)}.bind(this))}}return d._load_nodes.call(this,
...
!0),this.redraw(!0))}.bind(this))};this.search=function(b,c,e,h,g,k){if(!1===b||""===f.vakata.trim(b.toString()))return this.clear_search();h=(h=this.get_node(h))&&h.id?h.id:null;b=b.toString();var l=this.settings.search,m=l.ajax?l.ajax:!1,n=this._model.data,t=null,u=[],v=[],y;this._data.search.res.length&&!g&&this.clear_search();e===z&&(e=l.show_only_matches);k===z&&(k=l.show_only_matches_children);if(!c&&!1!==m){if(f.vakata.is_function(m))return m.call(this,b,function(q){q&&q.d&&(q=q.d);this._load_nodes(f.vakata.is_array(q)?
f.vakata.array_unique(q):[],function(){this.search(b,!0,e,h,g,k)})}.bind(this),h);m=f.extend({},m);m.data||(m.data={});m.data.str=b;h&&(m.data.inside=h);this._data.search.lastRequest&&this._data.search.lastRequest.abort();this._data.search.lastRequest=f.ajax(m).fail(function(){this._data.core.last_error={error:"ajax",plugin:"search",id:"search_01",reason:"Could not load search parents",data:JSON.stringify(m)};this.settings.core.error.call(this,this._data.core.last_error)}.bind(this)).done(function(q){q&&
q.d&&(q=q.d);this._load_nodes(f.vakata.is_array(q)?f.vakata.array_unique(q):[],function(){this.search(b,!0,e,h,g,k)})}.bind(this));return this._data.search.lastRequest}g||(this._data.search.str=b,this._data.search.dom=f(),this._data.search.res=[],this._data.search.opn=[],this._data.search.som=e,this._data.search.smc=k);t=new f.vakata.search(b,!0,{caseSensitive:l.case_sensitive,fuzzy:l.fuzzy});f.each(n[h?h:f.jstree.root].children_d,function(q,D){q=n[D];q.text&&!q.state.hidden&&(!l.search_leaves_only||
q.state.loaded&&0===q.children.length)&&(l.search_callback&&l.search_callback.call(this,b,q)||!l.search_callback&&t.search(q.text).isMatch)&&(u.push(D),v=v.concat(q.parents))});if(u.length){v=f.vakata.array_unique(v);c=0;for(y=v.length;c<y;c++)v[c]!==f.jstree.root&&n[v[c]]&&!0===this.open_node(v[c],null,0)&&this._data.search.opn.push(v[c]);g?(this._data.search.dom=this._data.search.dom.add(f(this.element[0].querySelectorAll("#"+f.map(u,function(q){return-1!=="0123456789".indexOf(q[0])?"\\3"+q[0]+
" "+q.substr(1).replace(f.jstree.idregex,"\\$&"):q.replace(f.jstree.idregex,"\\$&")}).join(", #")))),this._data.search.res=f.vakata.array_unique(this._data.search.res.concat(u))):(this._data.search.dom=f(this.element[0].querySelectorAll("#"+f.map(u,function(q){return-1!=="0123456789".indexOf(q[0])?"\\3"+q[0]+" "+q.substr(1).replace(f.jstree.idregex,"\\$&"):q.replace(f.jstree.idregex,"\\$&")}).join(", #"))),this._data.search.res=u);this._data.search.dom.children(".jstree-anchor").addClass("jstree-search")}this.trigger("search",
...
function(u){u=c.caseSensitive?u:u.toLowerCase();if(d===u||-1!==u.indexOf(d))return{isMatch:!0,score:0};if(!c.fuzzy)return{isMatch:!1,score:1};var v,y=u.length,q=g,D=u.indexOf(d,e),K,T=k+y,L=1,R=[];-1!==D&&(q=Math.min(n(0,D),q),D=u.lastIndexOf(d,e+k),-1!==D&&(q=Math.min(n(0,D),q)));D=-1;for(v=0;v<k;v++){var O=0;for(K=T;O<K;)n(v,e+K)<=q?O=K:T=K,K=Math.floor((T-O)/2+O);T=K;O=Math.max(1,e-K+1);var P=Math.min(e+K,y)+k;K=Array(P+2);for(K[P+1]=(1<<v)-1;P>=O;P--){var X=m[u.charAt(P-1)];K[P]=0===v?(K[P+1]<<
1|1)&X:(K[P+1]<<1|1)&X|(J[P+1]|J[P])<<1|1|J[P+1];if(K[P]&l&&(L=n(v,P-1),L<=q))if(q=L,D=P-1,R.push(D),D>e)O=Math.max(1,2*e-D);else break}if(n(v+1,e)>q)break;var J=K}return{isMatch:0<=D,score:L}};return!0===b?{search:t}:t(b)};a.vakata.search.defaults={location:0,distance:100,threshold:.6,fuzzy:!1,caseSensitive:!1}})(f);f.jstree.defaults.sort=function(a,d){return this.get_text(a)>this.get_text(d)?1:-1};f.jstree.plugins.sort=function(a,d){this.bind=function(){d.bind.call(this);this.element.on("model.jstree",
...
var v=k=this.get_string("New node");var y=[];var q=0;for(l=b.children.length;q<l;q++){var D=m[b.children[q]].text;n||(D=D.toLowerCase());t&&(D=D.replace(/^[\s\uFEFF\xA0]+|[\s\uFEFF\xA0]+$/g,""));y.push(D)}q=1;D=v;n||(D=D.toLowerCase());for(t&&(D=D.replace(/^[\s\uFEFF\xA0]+|[\s\uFEFF\xA0]+$/g,""));-1!==f.inArray(D,y);)D=v=u.call(this,k,++q).toString(),n||(D=D.toLowerCase()),t&&(D=D.replace(/^[\s\uFEFF\xA0]+|[\s\uFEFF\xA0]+$/g,""));c.text=v}return d.create_node.call(this,b,c,e,h,g)}};var V=x.createElement("DIV");
...
(function(f,z){var F=0;f.widget("s3.hierarchicalopts",{options:{selected:null,multiple:!0,leafonly:!0,cascade:!1,bulkSelect:!1,cascadeOptionInTree:!0,noneSelectedText:"Select",selectedText:"selected",noOptionsText:"No options available",selectAllText:"Select All",deselectAllText:"Deselect All",icons:!1,sep:null,stripes:!0,htmlTitles:!0},_create:function(){var p=f(this.element),r=this.options;this.id=F;F+=1;this._namespace=".hierarchicalopts"+F;this.treeID=p.attr("id")+"-tree";this.input=p.find(".s3-hierarchy-input").first();
this.input.data("input",!0);var A=r.selected;A&&this.input.val(this._stringifySelection(A));this.button=f('<button type="button" class="s3-hierarchy-button ui-multiselect ui-widget ui-state-default ui-corner-all"><span class="ui-icon ui-icon-triangle-1-s"></span></button>');this.buttonText=f("<span>"+r.noneSelectedText+"</span>").appendTo(this.button);this.noopts=f('<span class="no-options-available">'+r.noOptionsText+"</span>").hide();this.tree=p.find(".s3-hierarchy-tree").first();this.wrapper=this.tree.closest(".s3-hierarchy-wrapper").hide().before(this.noopts).before(this.button).detach().appendTo("body");
this.manualCascadeOption=this._isBulk=this._isOpen=!1},_init:function(){this.refresh()},_destroy:function(){f.Widget.prototype.destroy.call(this);this._unbindEvents();f(this.button).remove();f(this.noopts).remove();f(this.tree).detach().appendTo(this.element).show()},refresh:function(){this._unbindEvents();var p=this.options,r=this.tree,A=this;if(r.find("li").length){this.noopts.hide();this.button.attr("style","");this.input.next(".error_wrapper").insertAfter(this.button).one("click",function(){f(this).fadeOut()});
var w=this.input.val();if(w){w=this._parseSelection(w);var B=this.treeID;f.each(w,function(){f("#"+B+"-"+this).data("jstree",{selected:!0})})}w=r.find("> ul > li");if(1==w.length){w=w.first();var x=w.data("jstree");w.data("jstree",f.extend({},x,{opened:!0}))}w=p.multiple;x=p.leafonly;var G=p.cascade,M=!1,I=null,N=["sort","checkbox"];this.input.data("multiple",w);(G||x)&&w?M=!0:w&&(p.cascadeOptionInTree?this.manualCascadeOption=!0:(I=function(S){return r.jstree("is_parent",S)?(f.vakata.context.settings.icons=
!1,{select_all:{label:A.options.selectAllText,icon:!1,action:function(V){A._selectBranch(S)}},deselect_all:{label:A.options.deselectAllText,icon:!1,action:function(V){A._deselectBranch(S)}}}):null},N.push("contextmenu")));r.jstree({core:{themes:{icons:p.icons,stripes:p.stripes},animation:100,multiple:w,check_callback:!0},checkbox:{three_state:M},sort:function(S,V){var a=this.get_text(S)>this.get_text(V)?1:-1;try{var d=this.get_node(S).li_attr.rel}catch(c){d=null}try{var b=this.get_node(V).li_attr.rel}catch(c){b=
null}return"bulk"==d?-1:"bulk"==b?1:"none"==d?-1:"none"==b?1:a},contextmenu:{items:I,select_node:!1,icons:!1},plugins:N});var Q=jQuery.jstree.reference(r);w&&((p.bulkSelect||this.manualCascadeOption)&&p.cascadeOptionInTree?(this.wrapper.find(".s3-hierarchy-header").hide(),Q.create_node("#",{id:this.treeID+"-select-all",text:p.selectAllText,li_attr:{rel:"bulk","class":"s3-hierarchy-action-node"}},"first")):this.wrapper.find(".s3-hierarchy-header").removeClass("hide").show());r.on("ready.jstree",function(){var S=
Q.get_checked();A._updateButtonText(S)});this._bindEvents()}else this.button.attr("style","display:none!important"),this.noopts.show()},reload:function(p){var r=this;p=p.includes("?")?p+("&widget_id="+f(this.element).attr("id")):p+("?widget_id="+f(this.element).attr("id"));this.tree.jstree("destroy");this.input.val("");f.getS3(p,function(A){r.tree.html(A);r.refresh()},"html")},_setOption:function(p,r){if("selected"===p){var A=jQuery.jstree.reference(f(this.tree));if(A&&(A.uncheck_all(),r)){this.input.val(this._stringifySelection(r));
var w=this.treeID;f.each(r,function(){A.check_node("#"+w+"-"+this)});this._updateButtonText(r)}}this._super(p,r)},_updateSelectedNodes:function(){var p=jQuery.jstree.reference(f(this.tree)),r=this._parseSelection(this.input.val()),A=[],w=[],B=this.options.leafonly,x=p.get_checked(!0);f(x).each(function(){var M=f(this).attr("id");if("bulk"!=f(this).attr("rel")&&M&&(!B||p.is_leaf(this))){var I=M.split("-").pop();"None"!=I&&(I=parseInt(I));I&&(A.push(I),w.push(M))}});x=!1;var G=f(A).not(r).get();G.length?
x=!0:(G=f(r).not(A).get(),G.length&&(x=!0));r=this.input.val(this._stringifySelection(A));this._updateButtonText(w);x&&(r.change(),f(this.element).trigger("select.s3hierarchy"));return!0},_updateButtonText:function(p){var r=this.options,A=0;p&&(A=p.length);if(A)if(1<A)p=r.selectedText.replace("#",A);else{for(var w=jQuery.jstree.reference(f(this.tree)),B=[],x,G,M=r.sep,I=0;I<A;I++){if(M)for(G=p[I],x=f("#"+G+" > a").text().replace(/^\s+|\s+$/g,""),G=w.get_parent(G);"#"!=G;)x=f("#"+G+" > a").text().replace(/^\s+|\s+$/g,
"")+M+x,G=w.get_parent(G);else x=f("#"+p[I]+" > a").text().replace(/^\s+|\s+$/g,"");B.push(x)}p=B.length?B.join(" "):r.noneSelectedText}else p=r.noneSelectedText;this.buttonText.text(p)},_updateAndClose:function(){if(!this._isBulk){this._updateSelectedNodes();var p=this.wrapper,r=this;if(r.options.multiple)p.unbind("mouseleave.hierarchicalopts").one("mouseleave.hierarchicalopts",function(){window.setTimeout(function(){r.closeMenu()},100)});else window.setTimeout(function(){r.closeMenu()},100)}},_selectNode:function(p){var r=
this.options,A=jQuery.jstree.reference(f(this.tree)),w=r.multiple,B=r.leafonly;if(w)this._isBulk||"bulk"!=p.li_attr.rel||((r=A.get_parent(p))&&"#"!=r?(r=A.get_node(r),this._selectBranch(r,!0)):A.select_all());else{var x=p.id;f(A.get_checked(!0)).each(function(){x==this.id||B&&!A.is_leaf(this)||A.uncheck_node(this)})}if(B&&!w&&!A.is_leaf(p))return A.deselect_node(p),!1;this._updateAndClose()},_deselectNode:function(p){var r=jQuery.jstree.reference(f(this.tree));if(!this._isBulk){if("bulk"==p.li_attr.rel)(p=
r.get_parent(p))&&"#"!=p?(p=r.get_node(p),this._deselectBranch(p,!0)):r.deselect_all();else if(p=r.get_parent(p))if(p=r.get_node(p+"-select-all"))this._isBulk=!0,r.uncheck_node(p),this._isBulk=!1;if(p=r.get_node(this.treeID+"-select-all"))this._isBulk=!0,r.uncheck_node(p),this._isBulk=!1}this._updateSelectedNodes()},_openNode:function(p){var r=jQuery.jstree.reference(f(this.tree)),A=r.get_parent(p);A.length&&r.open_node(A,!1,!0);if(this.manualCascadeOption){var w=p.id;A=r.get_node(w+"-select-all");
A||(p=r.create_node(p,{id:w+"-select-all",text:this.options.selectAllText,li_attr:{rel:"bulk","class":"s3-hierarchy-action-node"}},"first"),A=r.get_node(p));(p=r.get_node(this.treeID+"-select-all"))&&r.is_checked(p)&&(this._isBulk=!0,r.check_node(A),this._isBulk=!1)}},_selectBranch:function(p,r){var A=jQuery.jstree.reference(f(this.tree)),w=this;this._isBulk=!0;A.open_node(p,function(){var B=A.get_children_dom(p);f(B).each(function(){w._selectBranch(this)});r||A.check_node(p)});this._isBulk=!1;this._updateAndClose()},
_deselectBranch:function(p,r){var A=jQuery.jstree.reference(f(this.tree)),w=this;this._isBulk=!0;A.open_node(p,function(){var B=A.get_children_dom(p);f(B).each(function(){w._deselectBranch(this)});r||A.uncheck_node(p)});this._isBulk=!1;this._updateSelectedNodes()},set:function(p){var r=jQuery.jstree.reference(f(this.tree)),A=this.treeID;this._isBulk=!0;r.uncheck_all();r.close_all();if(p){var w=function(B,x){if(B=r.get_parent(B))"#"!=B?(w(B),r.open_node(B,x)):x&&x()};p.forEach(function(B){var x=r.get_node(A+
"-"+B);x&&w(x,function(){r.check_node(x)})})}this._isBulk=!1;this._updateSelectedNodes()},get:function(){return this._parseSelection(this.input.val())},reset:function(){var p=jQuery.jstree.reference(f(this.tree));this._isBulk=!0;p.uncheck_all();p.close_all();this._isBulk=!1;this._updateSelectedNodes();return!0},_setMenuPosition:function(p,r){var A=r.offset(),w={position:"absolute",top:A.top+r.outerHeight(),minWidth:r.outerWidth()-8};"rtl"===f("body").css("direction")?w.right=f(window).width()-(A.left+
r.outerWidth()):w.left=A.left;p.css(w)},openMenu:function(){this._isOpen&&this.closeMenu();var p=f(this.button),r=f(this.wrapper),A=this;this._setMenuPosition(r,p);f(window).on("resize"+this._namespace+"-mpos",function(){A._setMenuPosition(r,p)});r.show();f(this.tree).jstree("set_focus");this._isOpen=!0;p.addClass("ui-state-active");f(this).trigger("open")},closeMenu:function(){f(window).off(this._namespace+"-mpos");f(this.tree).jstree("unset_focus").unbind("click.hierarchicalopts").unbind("mouseleave.hierarchicalopts");
f(this.wrapper).fadeOut(50);this._isOpen=!1;f(this.button).removeClass("ui-state-active");f(this).trigger("close")},addNode:function(p,r,A,w){var B=this.tree,x=this.treeID;r=x+"-"+r;if(jQuery.jstree.reference(B)){var G="#";p&&(G=f("#"+x+"-"+p+"_anchor"),G.length||(B.jstree("open_all"),G=f("#"+x+"-"+p+"_anchor")));B.jstree("create_node",G,{id:r,text:A,li_attr:{rel:"leaf"}},"last");p&&(G.attr({rel:"parent"}),B.jstree("open_node",G))}else p='<li rel="leaf" class="s3-hierarchy-node" id="'+r+'">'+A+"</li>",
f("#"+x).append(p),this.refresh();w&&B.jstree("check_node",f("#"+r))},_parseSelection:function(p){if(p)p=JSON.parse(p);else return[];p?p.constructor!==Array&&(p=[p]):p=[];return p},_stringifySelection:function(p){if(!this.options.multiple)if(p.length)p=p[0];else return"";return JSON.stringify(p)},_bindEvents:function(){var p=this,r=f(this.wrapper),A=f(this.tree),w=f(this.button),B=this._namespace,x=this.options,G=jQuery.jstree.reference(A),M=f.vakata.context.settings.icons;f(document).bind("context_show.vakata",
function(){r.unbind("mouseleave.hierarchicalopts");f.vakata.context.settings.icons=M});A.bind("select_node.jstree",function(I,N){p._selectNode(N.node)}).bind("deselect_node.jstree",function(I,N){p._deselectNode(N.node)}).bind("open_node.jstree",function(I,N){p._openNode(N.node)});w.bind("click"+B,function(){p._isOpen?p.closeMenu():p.openMenu()}).bind("keyup"+B,function(I){I.preventDefault();switch(I.keyCode){case 27:case 38:case 37:p.closeMenu();break;case 39:case 40:p.openMenu()}}).bind("mouseenter"+
B,function(){w.hasClass("ui-state-disabled")||f(this).addClass("ui-state-hover")}).bind("mouseleave"+B,function(){f(this).removeClass("ui-state-hover")}).bind("focus"+B,function(){w.hasClass("ui-state-disabled")||f(this).addClass("ui-state-focus")}).bind("blur"+B,function(){f(this).removeClass("ui-state-focus")});x.multiple&&x.bulkSelect&&!x.cascadeOptionInTree&&(r.find(".s3-hierarchy-select-all").bind("click"+B,function(I){I.preventDefault();G.select_all();p._updateSelectedNodes()}),r.find(".s3-hierarchy-deselect-all").bind("click"+
B,function(I){I.preventDefault();G.deselect_all();p._updateSelectedNodes()}),r.find(".s3-hierarchy-header").bind("click"+B,function(I){I.preventDefault();I.stopPropagation()}).bind("mousedown"+B,function(I){I.preventDefault();I.stopPropagation()}));f(document).bind("mousedown"+B,function(I){var N=I.target;if(f(".jstree-contextmenu").has(N).length||3==I.which)return r.unbind("mouseleave.hierarchicalopts"),!0;A.is(N)||w.is(N)||0!==A.has(N).length||0!==w.has(N).length||p.closeMenu()});return!0},_unbindEvents:function(){var p=
f(this.tree);f(this.button);var r=this._namespace;p.unbind("select_node.jstree").unbind("deselect_node.jstree").unbind("loaded.jstree").unbind("open_node.jstree");f(this.button).unbind(r);f(document).unbind(r);return!0}})})(jQuery);
...
refresh:function(){var p=this.tree,r=this.options;this._unbindEvents();var A=p.find("> ul > li");if(1==A.length){A=A.first();var w=A.data("jstree");A.data("jstree",f.extend({},w,{opened:!0}))}var B=this;p.jstree({core:{themes:{icons:r.icons,stripes:r.stripes},animation:100,multiple:!1,check_callback:!0},contextmenu:{items:function(x){var G=!0;"#"==p.jstree(!0).get_parent(x)&&(G=r.deleteRoot);var M={};r.openURL&&(M.open={label:r.openLabel,action:function(I){B._openNode(x)},separator_after:!0});r.editURL&&
(M.edit={label:r.editLabel,action:function(I){B._editNode(x)}});G&&r.deleteURL&&(M.delete={label:r.deleteLabel,separator_after:!0,action:function(I){B._deleteNode(x)}});r.addURL&&(M.add={label:r.addLabel,action:function(I){B._addNode(x)}});return M},select_node:!0},plugins:["sort","contextmenu"]});this._bindEvents()},reload:function(){var p=this.options.ajaxURL.replace("json","tree"),r=this;f.getS3(p,function(A){r.tree.html(A);r.refresh()},"html")},_openNode:function(p){var r=this.options.openURL;
p=p.id;r&&p&&(p=parseInt(p.split("-").pop()))&&(r=r.replace("[id]",p).replace("%5Bid%5D",p),window.location.href=r)},refreshNode:function(p){var r=this.options.ajaxURL,A=p.attr("id");if(r&&A&&(A=parseInt(A.split("-").pop()))){var w=this.tree,B=this.treeID;f.getJSONS3(r+"?node="+A,function(x){x.label&&w.jstree("rename_node",p,x.label);if(x=x.children){w.jstree("open_node",p,!1,!1);for(var G,M,I,N=0,Q=0,S=x.length;Q<S;Q++)G=x[Q],M=B+"-"+G.node,(I=w.jstree("get_node","#"+M,!0))?w.jstree("rename_node",
I,G.label):(w.jstree("create_node",p,{id:M,text:G.label,li_attr:{rel:"leaf"}}),N++);N&&(w.jstree("open_node",p),p.attr({rel:"parent"}))}})}},_editNode:function(p){var r=this.options.editURL;p=p.id;if(r&&p){var A=parseInt(p.split("-").pop());A&&(r=r.replace("[id]",A).replace("%5Bid%5D",A).split("?")[0],r+="?node="+p+"&hierarchy="+this.options.widgetID,p=S3.uid(),r=f('<iframe id="'+p+'" src='+r+" onload=\"S3.popup_loaded('"+p+'\')" class="loading" marginWidth="0" marginHeight="0" frameBorder="0"></iframe>').appendTo("body"),
this._openPopup(r,this.options.editTitle))}},_addNode:function(p){var r=this.options.addURL;p=p.id;if(r&&p){var A=parseInt(p.split("-").pop());A&&(r=r.split("?")[0],r+="?node="+p+"&link_to_parent="+A+"&hierarchy="+this.options.widgetID,p=S3.uid(),r=f('<iframe id="'+p+'" src='+r+" onload=\"S3.popup_loaded('"+p+'\')" class="loading" marginWidth="0" marginHeight="0" frameBorder="0"></iframe>').appendTo("body"),this._openPopup(r,this.options.addTitle))}},_deleteNode:function(p){if(!confirm(i18n.delete_confirmation))return!1;
var r=this.options.deleteURL,A=p.id,w=this.tree;r&&A&&(A=parseInt(A.split("-").pop()))&&(r=r.replace("[id]",A).replace("%5Bid%5D",A).split("?")[0],f.ajaxS3({type:"POST",url:r+"?recursive=1",data:{},dataType:"JSON",success:function(B){w.jstree("delete_node",p)},error:function(B,x,G){x=B.responseText;if("{"==x[0])try{x=JSON.parse(B.responseText),S3.showAlert(x.message,"error")}catch(M){}}}))},_openPopup:function(p,r){p.dialog({close:function(A,w){if(self.parent){var B=self.parent.$("iframe.ui-dialog-content"),
x=B.width();B.width(0);window.setTimeout(function(){B.width(x)},300)}p.remove()},minHeight:480,modal:!0,open:function(A,w){f(".ui-widget-overlay").bind("click",function(){p.dialog("close")})},title:r,minWidth:320,closeText:""})},_bindEvents:function(){return!0},_unbindEvents:function(){this.tree.jstree("destroy","true");return!0}})})(jQuery);
