...
" ").replace(/^ /,"").replace(/ $/,""))});k.hasClass("jstree-open")&&(h.state="open");k.hasClass("jstree-closed")&&(h.state="closed");k.data()&&(h.metadata=k.data());r=k.children("a");r.each(function(){p=b(this);d.length||-1!==b.inArray("languages",g.plugins)||p.children("ins").get(0).style.backgroundImage.length||p.children("ins").get(0).className&&p.children("ins").get(0).className.replace(/jstree[^ ]*|$/ig,"").length?(n=!1,-1!==b.inArray("languages",g.plugins)&&(b.isArray(g.languages)&&g.languages.length)&&
b.each(g.languages,function(a,b){if(p.hasClass(b))return n=b,!1}),m={attr:{},title:l.get_text(p,n)},b.each(d,function(a,b){m.attr[b]=(" "+(p.attr(b)||"")).replace(/ jstree[^ ]*/ig,"").replace(/\s+$/ig," ").replace(/^ /,"").replace(/ $/,"")}),-1!==b.inArray("languages",g.plugins)&&(b.isArray(g.languages)&&g.languages.length)&&b.each(g.languages,function(a,b){if(p.hasClass(b))return m.language=b,!0}),p.children("ins").get(0).className.replace(/jstree[^ ]*|$/ig,"").replace(/^\s+$/ig,"").length&&(m.icon=
...
(function(n,t){var s=0;n.widget("s3.hierarchicalopts",{options:{selected:null,noneSelectedText:"Select",selectedText:"selected",multiple:!0,leafonly:!0},_create:function(){var b=n(this.element),a=this.options.noneSelectedText;this.id=s;s+=1;this._namespace=".hierarchicalopts"+s;this.treeID=b.attr("id")+"-tree";this.input=b.find(".s3-hierarchy-input").first();this.button=n('<button type="button" class="s3-hierarchy-button ui-multiselect ui-widget ui-state-default ui-corner-all"><span class="ui-icon ui-icon-triangle-1-s"></span></button>');this.buttonText=
n("<span>"+a+"</span>").appendTo(this.button);this.tree=b.find(".s3-hierarchy-tree").first().hide().before(this.button);this._isOpen=!1},_init:function(){this.refresh()},_destroy:function(){n.Widget.prototype.destroy.call(this);this._unbindEvents();n(this.button).remove();n(this.tree).show()},refresh:function(){this._unbindEvents();var b=this.options,a=[],c=b.selected;if(c){for(var d=this.treeID,e=0,f=c.length;e<f;e++)a.push(d+"-"+c[e]);this._updateButtonText(a)}n.jstree._themes=S3.Ap.concat("/static/styles/jstree/");
theme=(rtl="ltr"==n("body").css("direction")?!1:!0)?"default-rtl":"default";var c=b.multiple,g=b.leafonly;this.tree.jstree({core:{animation:100,rtl:rtl},themes:{theme:theme,icons:!1},ui:{select_limit:c?-1:1,select_multiple_modifier:"on",initially_select:a},checkbox:{override_ui:!0,two_state:!g},plugins:["themes","html_data","ui","sort","checkbox"]});if(!c){var b=this.tree,l=jQuery.jstree._reference(b);b.bind("check_node.jstree",function(a,b){var c=b.rslt.obj.attr("id");l.get_checked(null,!0).each(function(){c!=
this.id&&(g&&!l.is_leaf("#"+this.id)||l.uncheck_node("#"+this.id))})})}this._bindEvents()},_updateSelectedNodes:function(b){b=this.input.val();var a=[];b=b?JSON.parse(b):[];var c=this.tree,d=this.options,e=c.jstree("get_checked",null,!0);n(e).each(function(){var b=n(this).attr("id"),e=d.leafonly;!b||e&&!c.jstree("is_leaf",this)||(b=parseInt(b.split("-").pop()))&&a.push(b)});var e=!1,f=n(a).not(b).get();f.length?e=!0:(f=n(b).not(a).get(),f.length&&(e=!0));this.input.val(JSON.stringify(a));this._updateButtonText(a);
e&&n(this.element).trigger("select.s3hierarchy");return!0},_updateButtonText:function(b){var a=this.buttonText,c=this.options;text=(b=b?b.length:0)?c.selectedText.replace("#",b):c.noneSelectedText;a.text(text)},set:function(b){this.tree.jstree("uncheck_all");if(b)for(var a=0,c=b.length;a<c;a++)node=n("#"+this.treeID+"-"+b[a]),this.tree.jstree("check_node",node)},get:function(){var b=this.input.val();return b?JSON.parse(b):[]},reset:function(){this.tree.jstree("uncheck_all");this._updateSelectedNodes()},
open:function(){this._isOpen&&this._close();var b=n(this.button),a=b.position();n(this.tree).css({position:"absolute",top:a.top+b.outerHeight()+3,left:a.left,minWidth:b.outerWidth(),"z-index":999999}).show();this._isOpen=!0;b.addClass("ui-state-active");n(this).trigger("open")},close:function(){n(this.tree).hide();this._isOpen=!1;n(this.button).removeClass("ui-state-active");n(this).trigger("close")},_bindEvents:function(){var b=this,a=n(this.tree),c=n(this.button),d=this._namespace;a.bind("check_node.jstree",
function(a,c){b._updateSelectedNodes()}).bind("uncheck_node.jstree",function(a,c){b._updateSelectedNodes()});b.options.leafonly&&!b.options.multiple&&a.bind("loaded.jstree",function(b,c){a.find(' li[rel="parent"] > a > ins.jstree-checkbox').css({display:"none"})});c.bind("click"+d,function(){b._isOpen?b.close():b.open()}).bind("keyup"+d,function(a){a.preventDefault();switch(a.keyCode){case 27:case 38:case 37:b.close();break;case 39:case 40:b.open()}}).bind("mouseenter"+d,function(){c.hasClass("ui-state-disabled")||
...
b.unbind("check_node.jstree").unbind("uncheck_node.jstree").unbind("loaded.jstree");n(this.button).unbind(a);n(document).unbind(a);return!0}})})(jQuery);
