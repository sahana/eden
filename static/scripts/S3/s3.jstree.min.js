...
p.length?p.before(k):a.children("ul").append(k),p=a}if(-1===p||p.get(0)===this.get_container().get(0))p=-1;this.clean_node(p);this.__callback({obj:k,parent:p});d&&d.call(this,k);return k},get_text:function(a){a=this._get_node(a);if(!a.length)return!1;var c=this._get_settings().core.html_titles;a=a.children("a:eq(0)");if(c)return a=a.clone(),a.children("INS").remove(),a.html();a=a.contents().filter(function(){return 3==this.nodeType})[0];return a.nodeValue},set_text:function(a,c){a=this._get_node(a);
...
refresh:function(){this._unbindEvents();var b=this.options;if(this.tree.find("li").length){this.noopts.hide();this.button.show();var a=[],c=b.selected;if(c){for(var d=this.treeID,e=0,f=c.length;e<f;e++)a.push(d+"-"+c[e]);this._updateButtonText(a)}n.jstree._themes=S3.Ap.concat("/",b.themesFolder,"/");"ltr"==n("body").css("direction")?(c=!1,d=b.theme):(c=!0,d=b.theme+"-rtl");var e=b.multiple,g=b.leafonly;this.tree.jstree({core:{animation:100,rtl:c},themes:{theme:d,icons:!1},ui:{select_limit:e?-1:1,
select_multiple_modifier:"on",initially_select:a},checkbox:{override_ui:!0,two_state:!g},plugins:["themes","html_data","ui","sort","checkbox"]});if(!e){var b=this.tree,l=jQuery.jstree._reference(b);b.bind("check_node.jstree",function(a,b){var c=b.rslt.obj.attr("id");l.get_checked(null,!0).each(function(){c!=this.id&&(g&&!l.is_leaf("#"+this.id)||l.uncheck_node("#"+this.id))})})}this._bindEvents()}else this.button.hide(),this.noopts.show()},_updateSelectedNodes:function(b){b=(b=this.input.val())?JSON.parse(b):
[];var a=this.tree,c=[],d=[],e=this.options.leafonly,f=a.jstree("get_checked",null,!0);n(f).each(function(){var b=n(this).attr("id");if(b&&(!e||a.jstree("is_leaf",this))){var f=parseInt(b.split("-").pop());f&&(c.push(f),d.push(b))}});var f=!1,g=n(c).not(b).get();g.length?f=!0:(g=n(b).not(c).get(),g.length&&(f=!0));this.input.val(JSON.stringify(c));this._updateButtonText(d);f&&n(this.element).trigger("select.s3hierarchy");return!0},_updateButtonText:function(b){var a=null,a=this.options;if(numSelected=
b?b.length:0)if(1<numSelected)a=a.selectedText.replace("#",numSelected);else{for(var c=[],d=0;d<numSelected;d++)c.push(n("#"+b[d]+" > a").text().replace(/^\s+|\s+$/g,""));a=c.length?c.join(" "):a.noneSelectedText}else a=a.noneSelectedText;this.buttonText.text(a)},set:function(b){this.tree.jstree("uncheck_all");if(b)for(var a=0,c=b.length;a<c;a++)node=n("#"+this.treeID+"-"+b[a]),this.tree.jstree("check_node",node)},get:function(){var b=this.input.val();return b?JSON.parse(b):[]},reset:function(){this.tree.jstree("uncheck_all");
this._updateSelectedNodes()},open:function(){this._isOpen&&this._close();var b=n(this.button),a=b.offset();n(this.tree).css({position:"absolute",top:a.top+b.outerHeight(),left:a.left,minWidth:b.outerWidth()-8,"z-index":999999}).show();this._isOpen=!0;b.addClass("ui-state-active");n(this).trigger("open")},close:function(){n(this.tree).hide();this._isOpen=!1;n(this.button).removeClass("ui-state-active");n(this).trigger("close")},_bindEvents:function(){var b=this,a=n(this.tree),c=n(this.button),d=this._namespace;
a.bind("check_node.jstree",function(a,c){b._updateSelectedNodes()}).bind("uncheck_node.jstree",function(a,c){b._updateSelectedNodes()});c.bind("click"+d,function(){b._isOpen?b.close():b.open()}).bind("keyup"+d,function(a){a.preventDefault();switch(a.keyCode){case 27:case 38:case 37:b.close();break;case 39:case 40:b.open()}}).bind("mouseenter"+d,function(){c.hasClass("ui-state-disabled")||n(this).addClass("ui-state-hover")}).bind("mouseleave"+d,function(){n(this).removeClass("ui-state-hover")}).bind("focus"+
d,function(){c.hasClass("ui-state-disabled")||n(this).addClass("ui-state-focus")}).bind("blur"+d,function(){n(this).removeClass("ui-state-focus")});n(document).bind("mousedown"+d,function(d){var f=d.target;a.is(f)||c.is(f)||0!==a.has(d.target).length||0!==c.has(d.target).length||b.close()});return!0},_unbindEvents:function(){var b=n(this.tree);n(this.button);var a=this._namespace;b.unbind("check_node.jstree").unbind("uncheck_node.jstree").unbind("loaded.jstree");n(this.button).unbind(a);n(document).unbind(a);
return!0}})})(jQuery);
