/*
 ...
 requires jQuery jstree 3.0.3
*/
(function(d){"function"===typeof define&&define.amd?define(["jquery"],d):"object"===typeof exports?d(require("jquery")):d(jQuery)})(function(d,v){if(!d.jstree){var D=0,h=!1,m=!1,n=!1,u=[],t=d("script:last").attr("src"),r=document,C=r.createElement("LI"),z;C.setAttribute("role","treeitem");z=r.createElement("I");z.className="jstree-icon jstree-ocl";z.setAttribute("role","presentation");C.appendChild(z);z=r.createElement("A");z.className="jstree-anchor";z.setAttribute("href","#");z.setAttribute("tabindex",
"-1");r=r.createElement("I");r.className="jstree-icon jstree-themeicon";r.setAttribute("role","presentation");z.appendChild(r);C.appendChild(z);z=r=null;d.jstree={version:"3.0.9",defaults:{plugins:[]},plugins:{},path:t&&-1!==t.indexOf("/")?t.replace(/\/[^\/]+$/,""):"",idregex:/[\\:&!^|()\[\]<>@*'+~#";.,=\- \/${}%?`]/g};d.jstree.create=function(a,b){var e=new d.jstree.core(++D),k=b;b=d.extend(!0,{},d.jstree.defaults,b);k&&k.plugins&&(b.plugins=k.plugins);d.each(b.plugins,function(a,k){"core"!==a&&
...
(c.state.loaded=!1,c.children=[],c.children_d=[]);delete a.data;delete a.children;c.original=a;h[c.id]=c;c.state.selected&&f.push(c.id);return c.id};if(c.length&&c[0].id!==e&&c[0].parent!==e){r=0;for(u=c.length;r<u;r++)c[r].children||(c[r].children=[]),h[c[r].id.toString()]=c[r];r=0;for(u=c.length;r<u;r++)h[c[r].parent.toString()].children.push(c[r].id.toString()),m.children_d.push(c[r].id.toString());r=0;for(u=m.children.length;r<u;r++)t=y(h[m.children[r]],b,m.parents.concat()),d.push(t),h[t].children_d.length&&
(d=d.concat(h[t].children_d))}else{r=0;for(u=c.length;r<u;r++)if(t=v(c[r],b,m.parents.concat()))k.push(t),d.push(t),h[t].children_d.length&&(d=d.concat(h[t].children_d));m.children=k;m.children_d=d}r=0;for(u=m.parents.length;r<u;r++)h[m.parents[r]].children_d=h[m.parents[r]].children_d.concat(d);c={cnt:q,mod:h,sel:n,par:b,dpc:d,add:f};if("undefined"===typeof window||"undefined"===typeof window.document)postMessage(c);else return c},q=function(a,c){this._cnt=a.cnt;this._model.data=a.mod;if(c){var b,
...
C.cloneNode(!0);w="jstree-node ";for(l in c.li_attr)c.li_attr.hasOwnProperty(l)&&"id"!==l&&("class"!==l?a.setAttribute(l,c.li_attr[l]):w+=c.li_attr[l]);c.a_attr.id||(c.a_attr.id=c.id+"_anchor");a.setAttribute("aria-selected",!!c.state.selected);a.setAttribute("aria-level",c.parents.length);a.setAttribute("aria-labelledby",c.a_attr.id);c.state.disabled&&a.setAttribute("aria-disabled",!0);c.state.loaded&&!c.children.length?w+=" jstree-leaf":(w+=c.state.opened&&c.state.loaded?" jstree-open":" jstree-closed",
...
d(g.event.target).closest(".jstree-anchor"))&&h.length&&h.parent().is(".jstree-closed, .jstree-open, .jstree-leaf")&&(l=h.offset(),m=g.event.pageY-l.top,r=h.height(),B=m<r/3?["b","i","a"]:m>r-r/3?["a","i","b"]:m>r/2?["i","a","b"]:["i","b","a"],d.each(B,function(c,m){switch(m){case "b":p=l.left-6;n=l.top;t=f.get_parent(h);u=h.parent().index();break;case "i":F=f.settings.dnd.inside_pos;G=f.get_node(h.parent());p=l.left-2;n=l.top+r/2+1;t=G.id;u="first"===F?0:"last"===F?G.children.length:Math.min(F,G.children.length);
break;case "a":p=l.left-6,n=l.top+r,t=f.get_parent(h),u=h.parent().index()+1}E=!0;y=0;for(v=g.data.nodes.length;y<v;y++)if(C=g.data.origin&&(g.data.origin.settings.dnd.always_copy||g.data.origin.settings.dnd.copy&&(g.event.metaKey||g.event.ctrlKey))?"copy_node":"move_node",A=u,"move_node"===C&&"a"===m&&g.data.origin&&g.data.origin===f&&t===f.get_parent(g.data.nodes[y])&&(z=f.get_node(t),A>d.inArray(g.data.nodes[y],z.children)&&--A),E=E&&(f&&f.settings&&f.settings.dnd&&!1===f.settings.dnd.check_while_dragging||
f.check(C,g.data.origin&&g.data.origin!==f?g.data.origin.get_node(g.data.nodes[y]):g.data.nodes[y],t,A,{dnd:!0,ref:f.get_node(h.parent()),pos:m,is_multi:g.data.origin&&g.data.origin!==f,is_foreign:!g.data.origin})),!E){f&&f.last_error&&(b=f.last_error());break}"i"===m&&h.parent().is(".jstree-closed")&&f.settings.dnd.open_timeout&&(e=setTimeout(function(a,e){return function(){a.open_node(e)}}(f,h),f.settings.dnd.open_timeout));if(E)return a={ins:f,par:t,pos:"i"!==m||"last"!==F||0!==u||f.is_loaded(G)?
u:"last"},k.css({left:p+"px",top:n+"px"}).show(),g.helper.find(".jstree-icon").first().removeClass("jstree-er").addClass("jstree-ok"),b={},B=!0,!1}),!0===B))return}else{E=!0;y=0;for(v=g.data.nodes.length;y<v&&(E=E&&f.check(g.data.origin&&(g.data.origin.settings.dnd.always_copy||g.data.origin.settings.dnd.copy&&(g.event.metaKey||g.event.ctrlKey))?"copy_node":"move_node",g.data.origin&&g.data.origin!==f?g.data.origin.get_node(g.data.nodes[y]):g.data.nodes[y],"#","last",{dnd:!0,ref:f.get_node("#"),pos:"i",
is_multi:g.data.origin&&g.data.origin!==f,is_foreign:!g.data.origin}),E);y++);if(E){a={ins:f,par:"#",pos:"last"};k.hide();g.helper.find(".jstree-icon").first().removeClass("jstree-er").addClass("jstree-ok");return}}a=!1;g.helper.find(".jstree-icon").removeClass("jstree-ok").addClass("jstree-er");k.hide()}}).on("dnd_scroll.vakata.jstree",function(e,b){b&&b.data&&b.data.jstree&&(k.hide(),a=!1,b.helper.find(".jstree-icon").first().removeClass("jstree-ok").addClass("jstree-er"))}).on("dnd_stop.vakata.jstree",
...
window.pageXOffset,e.originalEvent.changedTouches[0].pageY-window.pageYOffset));if(d){if(!c)if(Math.abs(e.pageX-l)>(u?a.vakata.dnd.settings.threshold_touch:a.vakata.dnd.settings.threshold)||Math.abs(e.pageY-m)>(u?a.vakata.dnd.settings.threshold_touch:a.vakata.dnd.settings.threshold))g&&(g.appendTo("body"),f=g.outerWidth()),c=!0,a.vakata.dnd._trigger("start",e);else return;var b=!1,h=!1,q=!1,t=!1,y=!1,v=h=!1,C=!1,t=b=!1;p=n=0;r=!1;a(a(e.target).parentsUntil("body").addBack().get().reverse()).filter(function(){return/^auto|scroll$/.test(a(this).css("overflow"))&&
(this.scrollHeight>this.offsetHeight||this.scrollWidth>this.offsetWidth)}).each(function(){var b=a(this),c=b.offset();this.scrollHeight>this.offsetHeight&&(c.top+b.height()-e.pageY<a.vakata.dnd.settings.scroll_proximity&&(n=1),e.pageY-c.top<a.vakata.dnd.settings.scroll_proximity&&(n=-1));this.scrollWidth>this.offsetWidth&&(c.left+b.width()-e.pageX<a.vakata.dnd.settings.scroll_proximity&&(p=1),e.pageX-c.left<a.vakata.dnd.settings.scroll_proximity&&(p=-1));if(n||p)return r=a(this),!1});!r&&(b=a(document),
h=a(window),q=b.height(),t=h.height(),y=b.width(),h=h.width(),v=b.scrollTop(),C=b.scrollLeft(),q>t&&e.pageY-v<a.vakata.dnd.settings.scroll_proximity&&(n=-1),q>t&&t-(e.pageY-v)<a.vakata.dnd.settings.scroll_proximity&&(n=1),y>h&&e.pageX-C<a.vakata.dnd.settings.scroll_proximity&&(p=-1),y>h&&h-(e.pageX-C)<a.vakata.dnd.settings.scroll_proximity&&(p=1),n||p)&&(r=b);r&&a.vakata.dnd._scroll(!0);g&&(b=parseInt(e.pageY+a.vakata.dnd.settings.helper_top,10),t=parseInt(e.pageX+a.vakata.dnd.settings.helper_left,
10),q&&b+25>q&&(b=q-50),y&&t+f>y&&(t=y-(f+2)),g.css({left:t+"px",top:b+"px"}));a.vakata.dnd._trigger("move",e);return!1}},stop:function(b){"touchend"===b.type&&b.originalEvent&&b.originalEvent.changedTouches&&b.originalEvent.changedTouches[0]&&(b.pageX=b.originalEvent.changedTouches[0].pageX,b.pageY=b.originalEvent.changedTouches[0].pageY,b.target=document.elementFromPoint(b.originalEvent.changedTouches[0].pageX-window.pageXOffset,b.originalEvent.changedTouches[0].pageY-window.pageYOffset));if(c)a.vakata.dnd._trigger("stop",
...
0;for(c=0;c<h;c++)e[a.charAt(c)]|=1<<h-c-1;return e}(),p=function(a,e){var b=a/h,d=Math.abs(c-e);return g?b+d/g:d?1:b});n=function(e){e=d.caseSensitive?e:e.toLowerCase();if(a===e||-1!==e.indexOf(a))return{isMatch:!0,score:0};if(!d.fuzzy)return{isMatch:!1,score:1};var g,n,t=e.length,r=f,u=e.indexOf(a,c),w,y,v=h+t,C,A,z=1,I=[];-1!==u&&(r=Math.min(p(0,u),r),u=e.lastIndexOf(a,c+h),-1!==u&&(r=Math.min(p(0,u),r)));u=-1;for(g=0;g<h;g++){w=0;for(y=v;w<y;)p(g,c+y)<=r?w=y:v=y,y=Math.floor((v-w)/2+w);v=y;w=
Math.max(1,c-y+1);n=Math.min(c+y,t)+h;y=Array(n+2);for(y[n+1]=(1<<g)-1;n>=w;n--)if(A=m[e.charAt(n-1)],y[n]=0===g?(y[n+1]<<1|1)&A:(y[n+1]<<1|1)&A|(C[n+1]|C[n])<<1|1|C[n+1],y[n]&l&&(z=p(g,n-1),z<=r))if(r=z,u=n-1,I.push(u),u>c)w=Math.max(1,2*c-u);else break;if(p(g+1,c)>r)break;C=y}return{isMatch:0<=u,score:z}};return!0===e?{search:n}:n(e)}})(d);d.jstree.defaults.sort=function(a,b){return this.get_text(a)>this.get_text(b)?1:-1};d.jstree.plugins.sort=function(a,b){this.bind=function(){b.bind.call(this);
...
c:this.get_node(c);if(!c||!c.children)return!0;f="rename_node"===a?g:h.text;var m=[],l=this.settings.unique.case_sensitive,n=this._model.data,p,r;p=0;for(r=c.children.length;p<r;p++)m.push(l?n[c.children[p]].text:n[c.children[p]].text.toLowerCase());l||(f=f.toLowerCase());switch(a){case "rename_node":return p=-1===d.inArray(f,m)||h.text&&h.text[l?"toString":"toLowerCase"]()===f,p||(this._data.core.last_error={error:"check",plugin:"unique",id:"unique_01",reason:"Child with name "+f+" already exists. Preventing: "+
...
a,h,c,g,f);c=c===v?"last":c;if(!c.toString().match(/^(before|after)$/)&&!f&&!this.is_loaded(a))return b.create_node.call(this,a,h,c,g,f);h||(h={});var m,l,n,p,r,t=this._model.data,u=this.settings.unique.case_sensitive,y=this.settings.unique.duplicate;l=m=this.get_string("New node");n=[];p=0;for(r=a.children.length;p<r;p++)n.push(u?t[a.children[p]].text:t[a.children[p]].text.toLowerCase());for(p=1;-1!==d.inArray(u?l:l.toLowerCase(),n);)l=y.call(this,m,++p).toString();h.text=l}return b.create_node.call(this,
...
{selected:!0})})}n=m.find("> ul > li");if(1==n.length){var n=n.first(),t=n.data("jstree");n.data("jstree",d.extend({},t,{opened:!0}))}var n=h.multiple,t=h.leafonly,r=!1,v=null,z=["sort","checkbox"];if((h.cascade||t)&&n)r=!0;else if(n)if(h.cascadeOptionInTree)this.manualCascadeOption=!0;else{var y=this,v=function(h){return m.jstree("is_parent",h)?(d.vakata.context.settings.icons=!1,{select_all:{label:y.options.selectAllText,icon:!1,action:function(d){y._selectBranch(h)}},deselect_all:{label:y.options.deselectAllText,
icon:!1,action:function(d){y._deselectBranch(h)}}}):null};z.push("contextmenu")}m.jstree({core:{themes:{name:"s3",icons:h.icons,stripes:h.stripes},animation:100,multiple:n,check_callback:!0},checkbox:{three_state:r},sort:function(d,h){var m=this.get_text(d)>this.get_text(h)?1:-1,n,r;try{n=this.get_node(d).li_attr.rel}catch(a){n=null}try{r=this.get_node(h).li_attr.rel}catch(b){r=null}return"bulk"==n?-1:"bulk"==r?1:"none"==n?-1:"none"==r?1:m},contextmenu:{items:v,select_node:!1,icons:!1},plugins:z});
t=jQuery.jstree.reference(m);n&&((h.bulkSelect||this.manualCascadeOption)&&h.cascadeOptionInTree?(this.wrapper.find(".s3-hierarchy-header").hide(),t.create_node("#",{id:this.treeID+"-select-all",text:h.selectAllText,li_attr:{rel:"bulk","class":"s3-hierarchy-action-node"}},"first")):this.wrapper.find(".s3-hierarchy-header").removeClass("hide").show());h=t.get_checked();this._updateButtonText(h);this._bindEvents()}else this.button.hide(),this.noopts.show()},_updateSelectedNodes:function(){var h=jQuery.jstree.reference(d(this.tree)),
m=this.input.val(),m=m?JSON.parse(m):[],n=[],u=[],t=this.options.leafonly,r=h.get_checked(!0);d(r).each(function(){var m=d(this).attr("id");if("bulk"!=d(this).attr("rel")&&m&&(!t||h.is_leaf(this))){var r=m.split("-").pop();"None"!=r&&(r=parseInt(r));r&&(n.push(r),u.push(m))}});var r=!1,v=d(n).not(m).get();v.length?r=!0:(v=d(m).not(n).get(),v.length&&(r=!0));this.input.val(JSON.stringify(n));this._updateButtonText(u);r&&(this.input.change(),d(this.element).trigger("select.s3hierarchy"));return!0},
_updateButtonText:function(h){var m=null,m=this.options,n=0;h&&(n=h.length);if(n)if(1<n)m=m.selectedText.replace("#",n);else{for(var u=[],t=0;t<n;t++)u.push(d("#"+h[t]+" > a").text().replace(/^\s+|\s+$/g,""));m=u.length?u.join(" "):m.noneSelectedText}else m=m.noneSelectedText;this.buttonText.text(m)},_updateAndClose:function(){if(!this._isBulk){this._updateSelectedNodes();var d=this.wrapper,m=this;if(m.options.multiple)d.unbind("mouseleave.hierarchicalopts").one("mouseleave.hierarchicalopts",function(){window.setTimeout(function(){m.closeMenu()},
100)});else window.setTimeout(function(){m.closeMenu()},100)}},_selectNode:function(h){var m=this.options,n=jQuery.jstree.reference(d(this.tree)),u=m.multiple,t=m.leafonly;if(u)this._isBulk||"bulk"!=h.li_attr.rel||((m=n.get_parent(h))&&"#"!=m?(m=n.get_node(m),this._selectBranch(m,!0)):n.select_all());else{var r=h.id;d(n.get_checked(!0)).each(function(){r!=this.id&&(t&&!n.is_leaf(this)||n.uncheck_node(this))})}if(t&&!u&&!n.is_leaf(h))return n.deselect_node(h),!1;this._updateAndClose()},_deselectNode:function(h){var m=
...
!1,!0);if(this.manualCascadeOption){var u=h.id,n=m.get_node(u+"-select-all");n||(h=m.create_node(h,{id:u+"-select-all",text:this.options.selectAllText,li_attr:{rel:"bulk","class":"s3-hierarchy-action-node"}},"first"),n=m.get_node(h));(h=m.get_node(this.treeID+"-select-all"))&&m.is_checked(h)&&(this._isBulk=!0,m.check_node(n),this._isBulk=!1)}},_selectBranch:function(h,m){var n=jQuery.jstree.reference(d(this.tree)),u=this;this._isBulk=!0;n.open_node(h,function(){var t=n.get_children_dom(h);d(t).each(function(){u._selectBranch(this)});
m||n.check_node(h)});this._isBulk=!1;this._updateAndClose()},_deselectBranch:function(h,m){var n=jQuery.jstree.reference(d(this.tree)),u=this;this._isBulk=!0;n.open_node(h,function(){var t=n.get_children_dom(h);d(t).each(function(){u._deselectBranch(this)});m||n.uncheck_node(h)});this._isBulk=!1;this._updateSelectedNodes()},set:function(h){var m=jQuery.jstree.reference(d(this.tree));m.uncheck_all();if(h)for(var n=0,u=h.length,t;n<u;n++)t=d("#"+this.treeID+"-"+h[n]),m.check_node(t)},get:function(){var d=
...
d(this.wrapper).fadeOut(50);this._isOpen=!1;d(this.button).removeClass("ui-state-active");d(this).trigger("close")},addNode:function(h,m,n,u){var t=this.tree,r=this.treeID,v="#";h&&(v=d("#"+r+"-"+h),v.length||(v="#"));m=r+"-"+m;t.jstree("create_node",v,{id:m,text:n,li_attr:{rel:"leaf"}},"last");h&&(v.attr({rel:"parent"}),t.jstree("open_node",v));u&&t.jstree("check_node",d("#"+m))},_bindEvents:function(){var h=this,m=d(this.wrapper),n=d(this.tree),u=d(this.button),t=this._namespace,r=this.options,
v=jQuery.jstree.reference(n),z=d.vakata.context.settings.icons;d(document).bind("context_show.vakata",function(){m.unbind("mouseleave.hierarchicalopts");d.vakata.context.settings.icons=z});n.bind("select_node.jstree",function(d,m){h._selectNode(m.node)}).bind("deselect_node.jstree",function(d,m){h._deselectNode(m.node)}).bind("open_node.jstree",function(d,m){h._openNode(m.node)});u.bind("click"+t,function(){h._isOpen?h.closeMenu():h.openMenu()}).bind("keyup"+t,function(d){d.preventDefault();switch(d.keyCode){case 27:case 38:case 37:h.closeMenu();
break;case 39:case 40:h.openMenu()}}).bind("mouseenter"+t,function(){u.hasClass("ui-state-disabled")||d(this).addClass("ui-state-hover")}).bind("mouseleave"+t,function(){d(this).removeClass("ui-state-hover")}).bind("focus"+t,function(){u.hasClass("ui-state-disabled")||d(this).addClass("ui-state-focus")}).bind("blur"+t,function(){d(this).removeClass("ui-state-focus")});r.multiple&&r.bulkSelect&&!r.cascadeOptionInTree&&(m.find(".s3-hierarchy-select-all").bind("click"+t,function(d){d.preventDefault();
v.select_all();h._updateSelectedNodes()}),m.find(".s3-hierarchy-deselect-all").bind("click"+t,function(d){d.preventDefault();v.deselect_all();h._updateSelectedNodes()}),m.find(".s3-hierarchy-header").bind("click"+t,function(d){d.preventDefault();d.stopPropagation()}).bind("mousedown"+t,function(d){d.preventDefault();d.stopPropagation()}));d(document).bind("mousedown"+t,function(r){var t=r.target;if(d(".jstree-contextmenu").has(t).length||3==r.which)return m.unbind("mouseleave.hierarchicalopts"),!0;
n.is(t)||u.is(t)||0!==n.has(t).length||0!==u.has(t).length||h.closeMenu()});return!0},_unbindEvents:function(){var h=d(this.tree);d(this.button);var m=this._namespace;h.unbind("select_node.jstree").unbind("deselect_node.jstree").unbind("loaded.jstree").unbind("open_node.jstree");d(this.button).unbind(m);d(document).unbind(m);return!0}})})(jQuery);
...
refresh:function(){var h=this.tree,m=this.options;this._unbindEvents();var n=h.find("> ul > li");if(1==n.length){var n=n.first(),u=n.data("jstree");n.data("jstree",d.extend({},u,{opened:!0}))}var t=this;h.jstree({core:{themes:{name:"s3",icons:m.icons,stripes:m.stripes},animation:100,multiple:!1,check_callback:!0},contextmenu:{items:function(d){return{open:{label:t.options.openLabel,action:function(h){t._openNode(d)},separator_after:!0},edit:{label:t.options.editLabel,action:function(h){t._editNode(d)}},
"delete":{label:t.options.deleteLabel,separator_after:!0,action:function(h){t._deleteNode(d)}},add:{label:t.options.addLabel,action:function(h){t._addNode(d)}}}},select_node:!0},plugins:["sort","contextmenu"]});this._bindEvents()},_openNode:function(d){var m=this.options.openURL;d=d.id;m&&d&&(d=parseInt(d.split("-").pop()))&&(m=m.replace("[id]",d).replace("%5Bid%5D",d),window.location.href=m)},refreshNode:function(h){var m=this.options.ajaxURL,n=h.attr("id");if(m&&n&&(n=parseInt(n.split("-").pop()))){var u=
this.tree,t=this.treeID;d.getJSONS3(m+"?node="+n,function(d){d.label&&u.jstree("rename_node",h,d.label);if(d=d.children){u.jstree("open_node",h,!1,!1);for(var m,n,v,A=0,D=0,J=d.length;D<J;D++)m=d[D],n=t+"-"+m.node,(v=u.jstree("get_node","#"+n,!0))?u.jstree("rename_node",v,m.label):(u.jstree("create_node",h,{id:n,text:m.label,li_attr:{rel:"leaf"}}),A++);A&&(u.jstree("open_node",h),h.attr({rel:"parent"}))}})}},_editNode:function(h){var m=this.options.editURL;h=h.id;if(m&&h){var n=parseInt(h.split("-").pop());
...
this.options.widgetID,h=S3.uid(),m=d('<iframe id="'+h+'" src='+m+" onload=\"S3.popup_loaded('"+h+'\')" class="loading" marginWidth="0" marginHeight="0" frameBorder="0"></iframe>').appendTo("body"),this._openPopup(m,this.options.addTitle))}},_deleteNode:function(h){if(!confirm(i18n.delete_confirmation))return!1;var m=this.options.deleteURL,n=h.id,u=this.tree;m&&n&&(n=parseInt(n.split("-").pop()))&&(m=m.replace("[id]",n).replace("%5Bid%5D",n).split("?")[0],d.ajaxS3({type:"POST",url:m+"?recursive=1",
data:{},dataType:"JSON",success:function(d){u.jstree("delete_node",h)},error:function(d,h,m){h=d.responseText;if("{"==h[0])try{h=JSON.parse(d.responseText),S3.showAlert(h.message,"error")}catch(n){}}}))},_openPopup:function(h,m){h.dialog({close:function(d,m){if(self.parent){var t=self.parent.$("iframe.ui-dialog-content"),r=t.width();t.width(0);window.setTimeout(function(){t.width(r)},300)}h.remove()},minHeight:480,modal:!0,open:function(m,u){d(".ui-widget-overlay").bind("click",function(){h.dialog("close")})},
