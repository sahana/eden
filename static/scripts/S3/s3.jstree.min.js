...
F.get_checked();p._updateButtonText(d)});this._bindEvents()}else this.button.hide(),this.noopts.show()},_setOption:function(g,l){if("selected"===g){var p=jQuery.jstree.reference(d(this.tree));if(p&&(p.uncheck_all(),l)){this.input.val(this._stringifySelection(l));var q=this.treeID;d.each(l,function(){p.check_node("#"+q+"-"+this)});this._updateButtonText(l)}}this._super(g,l)},_updateSelectedNodes:function(){var g=jQuery.jstree.reference(d(this.tree)),l=this._parseSelection(this.input.val()),p=[],q=
[],u=this.options.leafonly,t=g.get_checked(!0);d(t).each(function(){var l=d(this).attr("id");if("bulk"!=d(this).attr("rel")&&l&&(!u||g.is_leaf(this))){var t=l.split("-").pop();"None"!=t&&(t=parseInt(t));t&&(p.push(t),q.push(l))}});var t=!1,y=d(p).not(l).get();y.length?t=!0:(y=d(l).not(p).get(),y.length&&(t=!0));l=this.input.val(this._stringifySelection(p));this._updateButtonText(q);t&&(l.change(),d(this.element).trigger("select.s3hierarchy"));return!0},_updateButtonText:function(g){var l=this.options,
