/*
 ...
 requires jQuery jstree 3.0.3
*/
(function(d){"function"===typeof define&&define.amd?define(["jquery"],d):"object"===typeof exports?d(require("jquery")):d(jQuery)})(function(d,u){if(!d.jstree){var F=0,h=!1,l=!1,n=!1,q=[],t=d("script:last").attr("src"),v=document,A=v.createElement("LI"),z;A.setAttribute("role","treeitem");z=v.createElement("I");z.className="jstree-icon jstree-ocl";z.setAttribute("role","presentation");A.appendChild(z);z=v.createElement("A");z.className="jstree-anchor";z.setAttribute("href","#");z.setAttribute("tabindex",
"-1");v=v.createElement("I");v.className="jstree-icon jstree-themeicon";v.setAttribute("role","presentation");z.appendChild(v);A.appendChild(z);z=v=null;d.jstree={version:"3.0.9",defaults:{plugins:[]},plugins:{},path:t&&-1!==t.indexOf("/")?t.replace(/\/[^\/]+$/,""):"",idregex:/[\\:&!^|()\[\]<>@*'+~#";.,=\- \/${}%?`]/g};d.jstree.create=function(a,b){var e=new d.jstree.core(++F),k=b;b=d.extend(!0,{},d.jstree.defaults,b);k&&k.plugins&&(b.plugins=k.plugins);d.each(b.plugins,function(a,k){"core"!==a&&
...
b.children():b;var k=a.id,c=[],g=[],f=this._model.data,r=f[k];a=this._data.core.selected.length;var m,x;b.each(d.proxy(function(a,e){if(m=this._parse_model_from_html(d(e),k,r.parents.concat()))c.push(m),g.push(m),f[m].children_d.length&&(g=g.concat(f[m].children_d))},this));r.children=c;r.children_d=g;b=0;for(x=r.parents.length;b<x;b++)f[r.parents[b]].children_d=f[r.parents[b]].children_d.concat(g);this.trigger("model",{nodes:g,parent:k});"#"!==k?(this._node_changed(k),this.redraw()):(this.get_container_ul().children(".jstree-initial-node").remove(),
...
u(l[c.children[d]],c.id,b),p=l[m],c.children_d.push(m),p.children_d.length&&(c.children_d=c.children_d.concat(p.children_d));delete a.data;delete a.children;l[c.id].original=a;c.state.selected&&f.push(c.id);return c.id},A=function(a,c,b){b=b?b.concat():[];c&&b.unshift(c);var k,d,m,p;do k="j"+r+"_"+ ++h;while(l[k]);c={id:!1,text:"string"===typeof a?a:"",icon:"object"===typeof a&&a.icon!==e?a.icon:!0,parent:c,parents:b,children:[],children_d:[],data:null,state:{},li_attr:{id:!1},a_attr:{href:"#"},original:!1};
...
a.li_attr[d]);c.li_attr.id&&!c.id&&(c.id=c.li_attr.id.toString());c.id||(c.id=k);c.li_attr.id||(c.li_attr.id=c.id);if(a&&"object"===typeof a.a_attr)for(d in a.a_attr)a.a_attr.hasOwnProperty(d)&&(c.a_attr[d]=a.a_attr[d]);if(a&&a.children&&a.children.length){d=0;for(k=a.children.length;d<k;d++)m=A(a.children[d],c.id,b),p=l[m],c.children.push(m),p.children_d.length&&(c.children_d=c.children_d.concat(p.children_d));c.children_d=c.children_d.concat(c.children)}a&&a.children&&!0===a.children&&(c.state.loaded=
!1,c.children=[],c.children_d=[]);delete a.data;delete a.children;c.original=a;l[c.id]=c;c.state.selected&&f.push(c.id);return c.id};if(c.length&&c[0].id!==e&&c[0].parent!==e){q=0;for(w=c.length;q<w;q++)c[q].children||(c[q].children=[]),l[c[q].id.toString()]=c[q];q=0;for(w=c.length;q<w;q++)l[c[q].parent.toString()].children.push(c[q].id.toString()),n.children_d.push(c[q].id.toString());q=0;for(w=n.children.length;q<w;q++)v=u(l[n.children[q]],b,n.parents.concat()),d.push(v),l[v].children_d.length&&
(d=d.concat(l[v].children_d))}else{q=0;for(w=c.length;q<w;q++)if(v=A(c[q],b,n.parents.concat()))k.push(v),d.push(v),l[v].children_d.length&&(d=d.concat(l[v].children_d));n.children=k;n.children_d=d}q=0;for(w=n.parents.length;q<w;q++)l[n.parents[q]].children_d=l[n.parents[q]].children_d.concat(d);c={cnt:h,mod:l,sel:t,par:b,dpc:d,add:f};if("undefined"===typeof window||"undefined"===typeof window.document)postMessage(c);else return c},r=function(a,c){this._cnt=a.cnt;this._model.data=a.mod;if(c){var b,
k,f=a.add,g=a.sel,r=this._data.core.selected.slice(),h=this._model.data;if(g.length!==r.length||d.vakata.array_unique(g.concat(r)).length!==g.length){b=0;for(k=g.length;b<k;b++)-1===d.inArray(g[b],f)&&-1===d.inArray(g[b],r)&&(h[g[b]].state.selected=!1);b=0;for(k=r.length;b<k;b++)-1===d.inArray(r[b],g)&&(h[r[b]].state.selected=!0)}}a.add.length&&(this._data.core.selected=this._data.core.selected.concat(a.add));this.trigger("model",{nodes:a.dpc,parent:a.par});"#"!==a.par?(this._node_changed(a.par),
this.redraw()):this.redraw(!0);a.add.length&&this.trigger("changed",{action:"model",selected:this._data.core.selected});e.call(this,!0)};if(this.settings.core.worker&&window.Blob&&window.URL&&window.Worker)try{null===this._wrk&&(this._wrk=window.URL.createObjectURL(new window.Blob(["self.onmessage = "+f.toString()],{type:"text/javascript"}))),!this._data.core.working||k?(this._data.core.working=!0,c=new window.Worker(this._wrk),c.onmessage=d.proxy(function(a){r.call(this,a.data,!0);try{c.terminate(),
c=null}catch(e){}this._data.core.worker_queue.length?this._append_json_data.apply(this,this._data.core.worker_queue.shift()):this._data.core.working=!1},this),g.par?c.postMessage(g):this._data.core.worker_queue.length?this._append_json_data.apply(this,this._data.core.worker_queue.shift()):this._data.core.working=!1):this._data.core.worker_queue.push([a,b,e,!0])}catch(m){r.call(this,f(g),!1),this._data.core.worker_queue.length?this._append_json_data.apply(this,this._data.core.worker_queue.shift()):
this._data.core.working=!1}else r.call(this,f(g),!1)},_parse_model_from_html:function(a,b,e){e=e?[].concat(e):[];b&&e.unshift(b);var k,c,g=this._model.data,f={id:!1,text:!1,icon:!0,parent:b,parents:e,children:[],children_d:[],data:null,state:{},li_attr:{id:!1},a_attr:{href:"#"},original:!1},r;for(r in this._model.default_state)this._model.default_state.hasOwnProperty(r)&&(f.state[r]=this._model.default_state[r]);b=d.vakata.attributes(a,!0);d.each(b,function(a,e){e=d.trim(e);if(!e.length)return!0;
...
f.state.disabled=a.children("a").hasClass("jstree-disabled");if(f.data&&f.data.jstree)for(r in f.data.jstree)f.data.jstree.hasOwnProperty(r)&&(f.state[r]=f.data.jstree[r]);b=a.children("a").children(".jstree-themeicon");b.length&&(f.icon=b.hasClass("jstree-themeicon-hidden")?!1:b.attr("rel"));f.state.icon&&(f.icon=f.state.icon);b=a.children("ul").children("li");do r="j"+this._id+"_"+ ++this._cnt;while(g[r]);f.id=f.li_attr.id?f.li_attr.id.toString():r;b.length?(b.each(d.proxy(function(a,b){k=this._parse_model_from_html(d(b),
...
k;c++)d=this._parse_model_from_json(a.children[c],b.id,e),f=r[d],b.children.push(d),f.children_d.length&&(b.children_d=b.children_d.concat(f.children_d));b.children_d=b.children_d.concat(b.children)}a&&a.children&&!0===a.children&&(b.state.loaded=!1,b.children=[],b.children_d=[]);delete a.data;delete a.children;b.original=a;r[b.id]=b;b.state.selected&&this._data.core.selected.push(b.id);return b.id},_redraw:function(){var a=this._model.force_full_redraw?this._model.data["#"].children.concat([]):this._model.changed.concat([]),
...
e=0;for(k=b.children.length;e<k;e++)c.appendChild(this.redraw_node(b.children[e],!0,!0));a.appendChild(c)}},redraw_node:function(a,b,e,k){var c=this.get_node(a),g=!1,f=!1,r=!1,m=!1,x=!1,p=!1,h="",l=document,p=this._model.data,n=!1,q=null,B=h=0;if(!c)return!1;if("#"===c.id)return this.redraw(!0);b=b||0===c.children.length;if(a=document.querySelector?this.element[0].querySelector("#"+(-1!=="0123456789".indexOf(c.id[0])?"\\3"+c.id[0]+" "+c.id.substr(1).replace(d.jstree.idregex,"\\$&"):c.id.replace(d.jstree.idregex,
"\\$&"))):document.getElementById(c.id))a=d(a),e||(g=a.parent().parent()[0],g===this.element[0]&&(g=null),f=a.index()),b||!c.children.length||a.children(".jstree-children").length||(b=!0),b||(r=a.children(".jstree-children")[0]),n=a.children(".jstree-anchor")[0]===document.activeElement,a.remove();else if(b=!0,!e){g="#"!==c.parent?d("#"+c.parent.replace(d.jstree.idregex,"\\$&"),this.element)[0]:null;if(!(null===g||g&&p[c.parent].state.opened))return!1;f=d.inArray(c.id,null===g?p["#"].children:p[c.parent].children)}a=
A.cloneNode(!0);h="jstree-node ";for(m in c.li_attr)c.li_attr.hasOwnProperty(m)&&"id"!==m&&("class"!==m?a.setAttribute(m,c.li_attr[m]):h+=c.li_attr[m]);c.a_attr.id||(c.a_attr.id=c.id+"_anchor");a.setAttribute("aria-selected",!!c.state.selected);a.setAttribute("aria-level",c.parents.length);a.setAttribute("aria-labelledby",c.a_attr.id);c.state.disabled&&a.setAttribute("aria-disabled",!0);c.state.loaded&&!c.children.length?h+=" jstree-leaf":(h+=c.state.opened&&c.state.loaded?" jstree-open":" jstree-closed",
a.setAttribute("aria-expanded",c.state.opened&&c.state.loaded));null!==c.parent&&p[c.parent].children[p[c.parent].children.length-1]===c.id&&(h+=" jstree-last");a.id=c.id;a.className=h;h=(c.state.selected?" jstree-clicked":"")+(c.state.disabled?" jstree-disabled":"");for(x in c.a_attr)!c.a_attr.hasOwnProperty(x)||"href"===x&&"#"===c.a_attr[x]||("class"!==x?a.childNodes[1].setAttribute(x,c.a_attr[x]):h+=" "+c.a_attr[x]);h.length&&(a.childNodes[1].className="jstree-anchor "+h);if(c.icon&&!0!==c.icon||
...
a.childNodes[1].appendChild(l.createTextNode(c.text)):a.childNodes[1].innerHTML+=c.text;if(b&&c.children.length&&(c.state.opened||k)&&c.state.loaded){p=l.createElement("UL");p.setAttribute("role","group");p.className="jstree-children";m=0;for(x=c.children.length;m<x;m++)p.appendChild(this.redraw_node(c.children[m],b,!0));a.appendChild(p)}r&&a.appendChild(r);if(!e){g||(g=this.element[0]);m=0;for(x=g.childNodes.length;m<x;m++)if(g.childNodes[m]&&g.childNodes[m].className&&-1!==g.childNodes[m].className.indexOf("jstree-children")){q=
...
a,data:JSON.stringify({chk:a,pos:k,obj:b&&b.id?b.id:!1,par:e&&e.id?e.id:!1})},!1):!0},last_error:function(){return this._data.core.last_error},move_node:function(a,b,e,k,c,g){var f,r,m,x,p,h,l,n,q,B;b=this.get_node(b);e=e===u?0:e;if(!b)return!1;if(!e.toString().match(/^(before|after)$/)&&!c&&!this.is_loaded(b))return this.load_node(b,function(){this.move_node(a,b,e,k,!0)});if(d.isArray(a)){a=a.slice();g=0;for(f=a.length;g<f;g++)this.move_node(a[g],b,e,k,c,!0)&&(b=a[g],e="after");this.redraw();return!0}a=
a&&a.id?a:this.get_node(a);if(!a||"#"===a.id)return!1;f=(a.parent||"#").toString();m=e.toString().match(/^(before|after)$/)&&"#"!==b.id?this.get_node(b.parent):b;x=a.instance?a.instance:this._model.data[a.id]?this:d.jstree.reference(a.id);p=!x||!x._id||this._id!==x._id;r=x&&x._id&&f&&x._model.data[f]&&x._model.data[f].children?d.inArray(a.id,x._model.data[f].children):-1;if(p)return this.copy_node(a,b,e,k,c)?(x&&x.delete_node(a),!0):!1;"#"===b.id&&("before"===e&&(e="first"),"after"===e&&(e="last"));
switch(e){case "before":e=d.inArray(b.id,m.children);break;case "after":e=d.inArray(b.id,m.children)+1;break;case "inside":case "first":e=0;break;case "last":e=m.children.length;break;default:e||(e=0)}e>m.children.length&&(e=m.children.length);if(!this.check("move_node",a,m,e,{core:!0,is_multi:x&&x._id&&x._id!==this._id,is_foreign:!x||!x._id}))return this.settings.core.error.call(this,this._data.core.last_error),!1;if(a.parent===m.id){c=m.children.concat();p=d.inArray(a.id,c);-1!==p&&(c=d.vakata.array_remove(c,
p),e>p&&e--);p=[];h=0;for(l=c.length;h<l;h++)p[h>=e?h+1:h]=c[h];p[e]=a.id;m.children=p;this._node_changed(m.id);this.redraw("#"===m.id)}else{p=a.children_d.concat();p.push(a.id);h=0;for(l=a.parents.length;h<l;h++){c=[];B=x._model.data[a.parents[h]].children_d;n=0;for(q=B.length;n<q;n++)-1===d.inArray(B[n],p)&&c.push(B[n]);x._model.data[a.parents[h]].children_d=c}x._model.data[f].children=d.vakata.array_remove_item(x._model.data[f].children,a.id);h=0;for(l=m.parents.length;h<l;h++)this._model.data[m.parents[h]].children_d=
...
p);if("#"===f||"#"===m.id)this._model.force_full_redraw=!0;this._model.force_full_redraw||(this._node_changed(f),this._node_changed(m.id));g||this.redraw()}k&&k.call(this,a,m,e);this.trigger("move_node",{node:a,parent:m.id,position:e,old_parent:f,old_position:r,is_multi:x&&x._id&&x._id!==this._id,is_foreign:!x||!x._id,old_instance:x,new_instance:this});return!0},copy_node:function(a,b,e,k,c,g){var f,r,m,h,p,l;b=this.get_node(b);e=e===u?0:e;if(!b)return!1;if(!e.toString().match(/^(before|after)$/)&&
!c&&!this.is_loaded(b))return this.load_node(b,function(){this.copy_node(a,b,e,k,!0)});if(d.isArray(a)){a=a.slice();g=0;for(f=a.length;g<f;g++)if(r=this.copy_node(a[g],b,e,k,c,!0))b=r,e="after";this.redraw();return!0}a=a&&a.id?a:this.get_node(a);if(!a||"#"===a.id)return!1;f=(a.parent||"#").toString();p=e.toString().match(/^(before|after)$/)&&"#"!==b.id?this.get_node(b.parent):b;l=a.instance?a.instance:this._model.data[a.id]?this:d.jstree.reference(a.id);"#"===b.id&&("before"===e&&(e="first"),"after"===
...
!0===m.id&&delete m.id;m=this._parse_model_from_json(m,p.id,p.parents.concat());if(!m)return!1;r=this.get_node(m);a&&a.state&&!1===a.state.loaded&&(r.state.loaded=!1);c=[];c.push(m);c=c.concat(r.children_d);this.trigger("model",{nodes:c,parent:p.id});m=0;for(h=p.parents.length;m<h;m++)this._model.data[p.parents[m]].children_d=this._model.data[p.parents[m]].children_d.concat(c);c=[];m=0;for(h=p.children.length;m<h;m++)c[m>=e?m+1:m]=p.children[m];c[e]=r.id;p.children=c;p.children_d.push(r.id);p.children_d=
p.children_d.concat(r.children_d);"#"===p.id&&(this._model.force_full_redraw=!0);this._model.force_full_redraw||this._node_changed(p.id);g||this.redraw("#"===p.id);k&&k.call(this,r,p,e);this.trigger("copy_node",{node:r,original:a,parent:p.id,position:e,old_parent:f,old_position:l&&l._id&&f&&l._model.data[f]&&l._model.data[f].children?d.inArray(a.id,l._model.data[f].children):-1,is_multi:l&&l._id&&l._id!==this._id,is_foreign:!l||!l._id,old_instance:l,new_instance:this});return r.id},cut:function(a){a||
...
a.length);a.length=0>b?a.length+b:b;a.push.apply(a,e);return a};d.vakata.array_remove_item=function(a,b){var e=d.inArray(b,a);return-1!==e?d.vakata.array_remove(a,e):a};var w=document.createElement("I");w.className="jstree-icon jstree-checkbox";w.setAttribute("role","presentation");d.jstree.defaults.checkbox={visible:!0,three_state:!0,whole_node:!0,keep_selected_style:!0,cascade:"",tie_selection:!0};d.jstree.plugins.checkbox=function(a,b){this.bind=function(){b.bind.call(this);this._data.checkbox.uto=
...
b){var c=this._model.data,d=b.nodes,f,r;f=0;for(r=d.length;f<r;f++)c[d[f]].state.checked=c[d[f]].original&&c[d[f]].original.state&&c[d[f]].original.state.checked,c[d[f]].state.checked&&this._data.checkbox.selected.push(d[f])},this));if(-1!==this.settings.checkbox.cascade.indexOf("up")||-1!==this.settings.checkbox.cascade.indexOf("down"))this.element.on("model.jstree",d.proxy(function(a,b){var c=this._model.data,g=c[b.parent],f=b.nodes,r=[],m,h,p,l,n=this.settings.checkbox.cascade,q=this.settings.checkbox.tie_selection;
...
n.indexOf("up")){m=0;for(h=g.children_d.length;m<h;m++)c[g.children_d[m]].children.length||r.push(c[g.children_d[m]].parent);r=d.vakata.array_unique(r);p=0;for(l=r.length;p<l;p++)for(g=c[r[p]];g&&"#"!==g.id;){m=f=0;for(h=g.children.length;m<h;m++)f+=c[g.children[m]].state[q?"selected":"checked"];if(f===h)g.state[q?"selected":"checked"]=!0,this._data[q?"core":"checkbox"].selected.push(g.id),(m=this.get_node(g,!0))&&m.length&&m.attr("aria-selected",!0).children(".jstree-anchor").addClass(q?"jstree-clicked":
"jstree-checked");else break;g=this.get_node(g.parent)}}this._data[q?"core":"checkbox"].selected=d.vakata.array_unique(this._data[q?"core":"checkbox"].selected)},this)).on(this.settings.checkbox.tie_selection?"select_node.jstree":"check_node.jstree",d.proxy(function(a,b){var c=b.node,g=this._model.data,f=this.get_node(c.parent),r=this.get_node(c,!0),m,h,p,l=this.settings.checkbox.cascade,n=this.settings.checkbox.tie_selection;if(-1!==l.indexOf("down"))for(this._data[n?"core":"checkbox"].selected=
...
(p=this.get_node(f,!0))&&p.length&&p.attr("aria-selected",!0).children(".jstree-anchor").addClass(n?"jstree-clicked":"jstree-checked");else break;f=this.get_node(f.parent)}-1!==l.indexOf("down")&&r.length&&r.find(".jstree-anchor").addClass(n?"jstree-clicked":"jstree-checked").parent().attr("aria-selected",!0)},this)).on(this.settings.checkbox.tie_selection?"deselect_all.jstree":"uncheck_all.jstree",d.proxy(function(a,b){var c=this.get_node("#"),d=this._model.data,f,r,m;f=0;for(r=c.children_d.length;f<
r;f++)(m=d[c.children_d[f]])&&m.original&&m.original.state&&m.original.state.undetermined&&(m.original.state.undetermined=!1)},this)).on(this.settings.checkbox.tie_selection?"deselect_node.jstree":"uncheck_node.jstree",d.proxy(function(a,b){var c=b.node,g=this.get_node(c,!0),f,r,m,h=this.settings.checkbox.cascade,p=this.settings.checkbox.tie_selection;c&&c.original&&c.original.state&&c.original.state.undetermined&&(c.original.state.undetermined=!1);if(-1!==h.indexOf("down"))for(f=0,r=c.children_d.length;f<
r;f++)m=this._model.data[c.children_d[f]],m.state[p?"selected":"checked"]=!1,m&&m.original&&m.original.state&&m.original.state.undetermined&&(m.original.state.undetermined=!1);if(-1!==h.indexOf("up"))for(f=0,r=c.parents.length;f<r;f++)m=this._model.data[c.parents[f]],m.state[p?"selected":"checked"]=!1,m&&m.original&&m.original.state&&m.original.state.undetermined&&(m.original.state.undetermined=!1),(m=this.get_node(c.parents[f],!0))&&m.length&&m.attr("aria-selected",!1).children(".jstree-anchor").removeClass(p?
"jstree-clicked":"jstree-checked");m=[];f=0;for(r=this._data[p?"core":"checkbox"].selected.length;f<r;f++)-1!==h.indexOf("down")&&-1!==d.inArray(this._data[p?"core":"checkbox"].selected[f],c.children_d)||-1!==h.indexOf("up")&&-1!==d.inArray(this._data[p?"core":"checkbox"].selected[f],c.parents)||m.push(this._data[p?"core":"checkbox"].selected[f]);this._data[p?"core":"checkbox"].selected=d.vakata.array_unique(m);-1!==h.indexOf("down")&&g.length&&g.find(".jstree-anchor").removeClass(p?"jstree-clicked":
"jstree-checked").parent().attr("aria-selected",!1)},this));if(-1!==this.settings.checkbox.cascade.indexOf("up"))this.element.on("delete_node.jstree",d.proxy(function(a,b){for(var c=this.get_node(b.parent),d=this._model.data,f,r,m,h=this.settings.checkbox.tie_selection;c&&"#"!==c.id;){f=m=0;for(r=c.children.length;f<r;f++)m+=d[c.children[f]].state[h?"selected":"checked"];if(m===r)c.state[h?"selected":"checked"]=!0,this._data[h?"core":"checkbox"].selected.push(c.id),(f=this.get_node(c,!0))&&f.length&&
f.attr("aria-selected",!0).children(".jstree-anchor").addClass(h?"jstree-clicked":"jstree-checked");else break;c=this.get_node(c.parent)}},this)).on("move_node.jstree",d.proxy(function(a,b){var c=b.is_multi,g=b.old_parent,f=this.get_node(b.parent),h=this._model.data,m,l,p=this.settings.checkbox.tie_selection;if(!c)for(c=this.get_node(g);c&&"#"!==c.id;){m=g=0;for(l=c.children.length;m<l;m++)g+=h[c.children[m]].state[p?"selected":"checked"];if(g===l)c.state[p?"selected":"checked"]=!0,this._data[p?"core":
...
this.show_contextmenu=function(a,b,c,g){a=this.get_node(a);if(!a||"#"===a.id)return!1;var f=this.settings.contextmenu,h=this.get_node(a,!0).children(".jstree-anchor"),m=!1,m=!1;if(f.show_at_node||b===u||c===u)m=h.offset(),b=m.left,c=m.top+this._data.core.li_height;this.settings.contextmenu.select_node&&!this.is_selected(a)&&this.activate_node(a,g);m=f.items;d.isFunction(m)&&(m=m.call(this,a,d.proxy(function(d){this._show_contextmenu(a,b,c,d)},this)));d.isPlainObject(m)&&this._show_contextmenu(a,b,
...
g.length?g:!1},_show_submenu:function(e){e=a(e);if(e.length&&e.children("ul").length){var c=e.children("ul"),d=e.offset().left+e.outerWidth(),f=e.offset().top,k=c.width(),g=c.height(),m=a(window).width()+a(window).scrollLeft(),h=a(window).height()+a(window).scrollTop();if(b)e[0>d-(k+10+e.outerWidth())?"addClass":"removeClass"]("vakata-context-left");else e[d+k+10>m?"addClass":"removeClass"]("vakata-context-right");f+g+10>h&&c.css("bottom","-1px");c.show()}},show:function(l,p,n){var q,t,v,B,w;e&&e.length&&
e.width("");switch(!0){case !p&&!l:return!1;case !!p&&!!l:d=l;c=p.x;g=p.y;break;case !p&&!!l:d=l;p=l.offset();c=p.left+l.outerHeight();g=p.top;break;case !!p&&!l:c=p.x,g=p.y}l&&!n&&a(l).data("vakata_contextmenu")&&(n=a(l).data("vakata_contextmenu"));a.vakata.context._parse(n)&&e.html(h);f.length&&(e.appendTo("body"),n=e,p=c,q=g,t=n.width(),v=n.height(),B=a(window).width()+a(window).scrollLeft(),w=a(window).height()+a(window).scrollTop(),b&&(p-=n.outerWidth()-a(l).outerWidth(),p<a(window).scrollLeft()+
20&&(p=a(window).scrollLeft()+20)),p+t+20>B&&(p=B-(t+20)),q+v+20>w&&(q=w-(v+20)),e.css({left:p,top:q}).show().find("a").first().focus().parent().addClass("vakata-context-hover"),m=!0,a.vakata.context._trigger("show"))},hide:function(){m&&(e.hide().find("ul").hide().end().find(":focus").blur().end().detach(),m=!1,a.vakata.context._trigger("hide"))}};a(function(){b="rtl"===a("body").css("direction");var c=!1;e=a("<ul class='vakata-context'></ul>");e.on("mouseenter","li",function(b){b.stopImmediatePropagation();
...
d(g.event.target).closest(".jstree-anchor"))&&h.length&&h.parent().is(".jstree-closed, .jstree-open, .jstree-leaf")&&(m=h.offset(),l=g.event.pageY-m.top,q=h.height(),B=l<q/3?["b","i","a"]:l>q-q/3?["a","i","b"]:l>q/2?["i","a","b"]:["i","b","a"],d.each(B,function(c,l){switch(l){case "b":p=m.left-6;n=m.top;t=f.get_parent(h);v=h.parent().index();break;case "i":C=f.settings.dnd.inside_pos;G=f.get_node(h.parent());p=m.left-2;n=m.top+q/2+1;t=G.id;v="first"===C?0:"last"===C?G.children.length:Math.min(C,G.children.length);
break;case "a":p=m.left-6,n=m.top+q,t=f.get_parent(h),v=h.parent().index()+1}w=!0;D=0;for(u=g.data.nodes.length;D<u;D++)if(A=g.data.origin&&(g.data.origin.settings.dnd.always_copy||g.data.origin.settings.dnd.copy&&(g.event.metaKey||g.event.ctrlKey))?"copy_node":"move_node",y=v,"move_node"===A&&"a"===l&&g.data.origin&&g.data.origin===f&&t===f.get_parent(g.data.nodes[D])&&(z=f.get_node(t),y>d.inArray(g.data.nodes[D],z.children)&&--y),w=w&&(f&&f.settings&&f.settings.dnd&&!1===f.settings.dnd.check_while_dragging||
f.check(A,g.data.origin&&g.data.origin!==f?g.data.origin.get_node(g.data.nodes[D]):g.data.nodes[D],t,y,{dnd:!0,ref:f.get_node(h.parent()),pos:l,is_multi:g.data.origin&&g.data.origin!==f,is_foreign:!g.data.origin})),!w){f&&f.last_error&&(b=f.last_error());break}"i"===l&&h.parent().is(".jstree-closed")&&f.settings.dnd.open_timeout&&(e=setTimeout(function(a,e){return function(){a.open_node(e)}}(f,h),f.settings.dnd.open_timeout));if(w)return a={ins:f,par:t,pos:"i"!==l||"last"!==C||0!==v||f.is_loaded(G)?
v:"last"},k.css({left:p+"px",top:n+"px"}).show(),g.helper.find(".jstree-icon").first().removeClass("jstree-er").addClass("jstree-ok"),b={},B=!0,!1}),!0===B))return}else{w=!0;D=0;for(u=g.data.nodes.length;D<u&&(w=w&&f.check(g.data.origin&&(g.data.origin.settings.dnd.always_copy||g.data.origin.settings.dnd.copy&&(g.event.metaKey||g.event.ctrlKey))?"copy_node":"move_node",g.data.origin&&g.data.origin!==f?g.data.origin.get_node(g.data.nodes[D]):g.data.nodes[D],"#","last",{dnd:!0,ref:f.get_node("#"),pos:"i",
is_multi:g.data.origin&&g.data.origin!==f,is_foreign:!g.data.origin}),w);D++);if(w){a={ins:f,par:"#",pos:"last"};k.hide();g.helper.find(".jstree-icon").first().removeClass("jstree-er").addClass("jstree-ok");return}}a=!1;g.helper.find(".jstree-icon").removeClass("jstree-ok").addClass("jstree-er");k.hide()}}).on("dnd_scroll.vakata.jstree",function(e,b){b&&b.data&&b.data.jstree&&(k.hide(),a=!1,b.helper.find(".jstree-icon").first().removeClass("jstree-ok").addClass("jstree-er"))}).on("dnd_stop.vakata.jstree",
...
window.pageXOffset,e.originalEvent.changedTouches[0].pageY-window.pageYOffset));if(d){if(!c)if(Math.abs(e.pageX-m)>(v?a.vakata.dnd.settings.threshold_touch:a.vakata.dnd.settings.threshold)||Math.abs(e.pageY-l)>(v?a.vakata.dnd.settings.threshold_touch:a.vakata.dnd.settings.threshold))g&&(g.appendTo("body"),f=g.outerWidth()),c=!0,a.vakata.dnd._trigger("start",e);else return;var b=!1,h=!1,r=!1,t=!1,w=!1,u=h=!1,A=!1,t=b=!1;p=n=0;q=!1;a(a(e.target).parentsUntil("body").addBack().get().reverse()).filter(function(){return/^auto|scroll$/.test(a(this).css("overflow"))&&
...
h=a(window),r=b.height(),t=h.height(),w=b.width(),h=h.width(),u=b.scrollTop(),A=b.scrollLeft(),r>t&&e.pageY-u<a.vakata.dnd.settings.scroll_proximity&&(n=-1),r>t&&t-(e.pageY-u)<a.vakata.dnd.settings.scroll_proximity&&(n=1),w>h&&e.pageX-A<a.vakata.dnd.settings.scroll_proximity&&(p=-1),w>h&&h-(e.pageX-A)<a.vakata.dnd.settings.scroll_proximity&&(p=1),n||p)&&(q=b);q&&a.vakata.dnd._scroll(!0);g&&(b=parseInt(e.pageY+a.vakata.dnd.settings.helper_top,10),t=parseInt(e.pageX+a.vakata.dnd.settings.helper_left,
10),r&&b+25>r&&(b=r-50),w&&t+f>w&&(t=w-(f+2)),g.css({left:t+"px",top:b+"px"}));a.vakata.dnd._trigger("move",e);return!1}},stop:function(b){"touchend"===b.type&&b.originalEvent&&b.originalEvent.changedTouches&&b.originalEvent.changedTouches[0]&&(b.pageX=b.originalEvent.changedTouches[0].pageX,b.pageY=b.originalEvent.changedTouches[0].pageY,b.target=document.elementFromPoint(b.originalEvent.changedTouches[0].pageX-window.pageXOffset,b.originalEvent.changedTouches[0].pageY-window.pageYOffset));if(c)a.vakata.dnd._trigger("stop",
...
this)).on("clear_search.jstree",d.proxy(function(a,b){this._data.search.som&&b.nodes.length&&this.element.find(".jstree-node").css("display","").filter(".jstree-last").filter(function(){return this.nextSibling}).removeClass("jstree-last")},this))};this.search=function(a,b,c){if(!1===a||""===d.trim(a.toString()))return this.clear_search();a=a.toString();var g=this.settings.search,f=g.ajax?g.ajax:!1,h=null,m=[],l=[];this._data.search.res.length&&this.clear_search();c===u&&(c=g.show_only_matches);if(!b&&
...
0;for(c=0;c<h;c++)e[a.charAt(c)]|=1<<h-c-1;return e}(),p=function(a,e){var b=a/h,d=Math.abs(c-e);return g?b+d/g:d?1:b});n=function(e){e=d.caseSensitive?e:e.toLowerCase();if(a===e||-1!==e.indexOf(a))return{isMatch:!0,score:0};if(!d.fuzzy)return{isMatch:!1,score:1};var g,n,q=e.length,t=f,v=e.indexOf(a,c),w,u,A=h+q,y,z,C=1,E=[];-1!==v&&(t=Math.min(p(0,v),t),v=e.lastIndexOf(a,c+h),-1!==v&&(t=Math.min(p(0,v),t)));v=-1;for(g=0;g<h;g++){w=0;for(u=A;w<u;)p(g,c+u)<=t?w=u:A=u,u=Math.floor((A-w)/2+w);A=u;w=
Math.max(1,c-u+1);n=Math.min(c+u,q)+h;u=Array(n+2);for(u[n+1]=(1<<g)-1;n>=w;n--)if(z=l[e.charAt(n-1)],u[n]=0===g?(u[n+1]<<1|1)&z:(u[n+1]<<1|1)&z|(y[n+1]|y[n])<<1|1|y[n+1],u[n]&m&&(C=p(g,n-1),C<=t))if(t=C,v=n-1,E.push(v),v>c)w=Math.max(1,2*c-v);else break;if(p(g+1,c)>t)break;y=u}return{isMatch:0<=v,score:C}};return!0===e?{search:n}:n(e)}})(d);d.jstree.defaults.sort=function(a,b){return this.get_text(a)>this.get_text(b)?1:-1};d.jstree.plugins.sort=function(a,b){this.bind=function(){b.bind.call(this);
...
reason:"valid_children prevents function: "+a,data:JSON.stringify({chk:a,pos:g,obj:k&&k.id?k.id:!1,par:c&&c.id?c.id:!1})},!1;if(f&&k.children_d&&k.parents){l=m=0;for(p=k.children_d.length;l<p;l++)m=Math.max(m,f[k.children_d[l]].parents.length);m=m-k.parents.length+1}if(0>=m||m===u)m=1;do{if(h.max_depth!==u&&-1!==h.max_depth&&h.max_depth<m)return this._data.core.last_error={error:"check",plugin:"types",id:"types_03",reason:"max_depth prevents function: "+a,data:JSON.stringify({chk:a,pos:g,obj:k&&k.id?
...
a,data:JSON.stringify({chk:a,pos:g,obj:k&&k.id?k.id:!1,par:c&&c.id?c.id:!1})}),p;case "move_node":return p=k.parent===c.id||-1===d.inArray(f,h),p||(this._data.core.last_error={error:"check",plugin:"unique",id:"unique_03",reason:"Child with name "+f+" already exists. Preventing: "+a,data:JSON.stringify({chk:a,pos:g,obj:k&&k.id?k.id:!1,par:c&&c.id?c.id:!1})}),p}return!0};this.create_node=function(a,k,c,g,f){if(!k||k.text===u){null===a&&(a="#");a=this.get_node(a);if(!a)return b.create_node.call(this,
a,k,c,g,f);c=c===u?"last":c;if(!c.toString().match(/^(before|after)$/)&&!f&&!this.is_loaded(a))return b.create_node.call(this,a,k,c,g,f);k||(k={});var h,m,l,p,n,q=this._model.data,t=this.settings.unique.case_sensitive,v=this.settings.unique.duplicate;m=h=this.get_string("New node");l=[];p=0;for(n=a.children.length;p<n;p++)l.push(t?q[a.children[p]].text:q[a.children[p]].text.toLowerCase());for(p=1;-1!==d.inArray(t?m:m.toLowerCase(),l);)m=v.call(this,h,++p).toString();k.text=m}return b.create_node.call(this,
...
this.manualCascadeOption=this._isBulk=this._isOpen=!1},_init:function(){this.refresh()},_destroy:function(){d.Widget.prototype.destroy.call(this);this._unbindEvents();d(this.button).remove();d(this.noopts).remove();d(this.tree).detach().appendTo(this.element).show()},refresh:function(){this._unbindEvents();var h=this.options,l=this.tree;if(l.find("li").length){this.noopts.hide();this.button.show();var n=this.input.val();if(n){var n=JSON.parse(n),q=this.treeID;d.each(n,function(){d("#"+q+"-"+this).data("jstree",
{selected:!0})})}n=l.find("> ul > li");if(1==n.length){var n=n.first(),t=n.data("jstree");n.data("jstree",d.extend({},t,{opened:!0}))}var n=h.multiple,t=h.leafonly,v=h.cascade,u=!1,z=null,w=["sort","checkbox"];this.input.data("multiple",n);if((v||t)&&n)u=!0;else if(n)if(h.cascadeOptionInTree)this.manualCascadeOption=!0;else{var y=this,z=function(h){return l.jstree("is_parent",h)?(d.vakata.context.settings.icons=!1,{select_all:{label:y.options.selectAllText,icon:!1,action:function(d){y._selectBranch(h)}},
deselect_all:{label:y.options.deselectAllText,icon:!1,action:function(d){y._deselectBranch(h)}}}):null};w.push("contextmenu")}l.jstree({core:{themes:{name:"s3",icons:h.icons,stripes:h.stripes},animation:100,multiple:n,check_callback:!0},checkbox:{three_state:u},sort:function(d,h){var l=this.get_text(d)>this.get_text(h)?1:-1,a,b;try{a=this.get_node(d).li_attr.rel}catch(e){a=null}try{b=this.get_node(h).li_attr.rel}catch(e){b=null}return"bulk"==a?-1:"bulk"==b?1:"none"==a?-1:"none"==b?1:l},contextmenu:{items:z,
select_node:!1,icons:!1},plugins:w});var E=jQuery.jstree.reference(l);n&&((h.bulkSelect||this.manualCascadeOption)&&h.cascadeOptionInTree?(this.wrapper.find(".s3-hierarchy-header").hide(),E.create_node("#",{id:this.treeID+"-select-all",text:h.selectAllText,li_attr:{rel:"bulk","class":"s3-hierarchy-action-node"}},"first")):this.wrapper.find(".s3-hierarchy-header").removeClass("hide").show());y=this;l.on("ready.jstree",function(){var d=E.get_checked();y._updateButtonText(d)});this._bindEvents()}else this.button.hide(),
this.noopts.show()},_updateSelectedNodes:function(){var h=jQuery.jstree.reference(d(this.tree)),l=this.input.val(),l=l?JSON.parse(l):[],n=[],q=[],t=this.options.leafonly,v=h.get_checked(!0);d(v).each(function(){var l=d(this).attr("id");if("bulk"!=d(this).attr("rel")&&l&&(!t||h.is_leaf(this))){var v=l.split("-").pop();"None"!=v&&(v=parseInt(v));v&&(n.push(v),q.push(l))}});var v=!1,u=d(n).not(l).get();u.length?v=!0:(u=d(l).not(n).get(),u.length&&(v=!0));this.input.val(JSON.stringify(n));this._updateButtonText(q);
v&&(this.input.change(),d(this.element).trigger("select.s3hierarchy"));return!0},_updateButtonText:function(h){var l=this.options,n=0;h&&(n=h.length);if(n)if(1<n)h=l.selectedText.replace("#",n);else{for(var q=[],t=0;t<n;t++)q.push(d("#"+h[t]+" > a").text().replace(/^\s+|\s+$/g,""));h=q.length?q.join(" "):l.noneSelectedText}else h=l.noneSelectedText;this.buttonText.text(h)},_updateAndClose:function(){if(!this._isBulk){this._updateSelectedNodes();var d=this.wrapper,l=this;if(l.options.multiple)d.unbind("mouseleave.hierarchicalopts").one("mouseleave.hierarchicalopts",
function(){window.setTimeout(function(){l.closeMenu()},100)});else window.setTimeout(function(){l.closeMenu()},100)}},_selectNode:function(h){var l=this.options,n=jQuery.jstree.reference(d(this.tree)),q=l.multiple,t=l.leafonly;if(q)this._isBulk||"bulk"!=h.li_attr.rel||((l=n.get_parent(h))&&"#"!=l?(l=n.get_node(l),this._selectBranch(l,!0)):n.select_all());else{var v=h.id;d(n.get_checked(!0)).each(function(){v!=this.id&&(t&&!n.is_leaf(this)||n.uncheck_node(this))})}if(t&&!q&&!n.is_leaf(h))return n.deselect_node(h),
!1;this._updateAndClose()},_deselectNode:function(h){var l=jQuery.jstree.reference(d(this.tree));if(!this._isBulk){if("bulk"==h.li_attr.rel)(h=l.get_parent(h))&&"#"!=h?(h=l.get_node(h),this._deselectBranch(h,!0)):l.deselect_all();else if(h=l.get_parent(h))if(h=l.get_node(h+"-select-all"))this._isBulk=!0,l.uncheck_node(h),this._isBulk=!1;if(h=l.get_node(this.treeID+"-select-all"))this._isBulk=!0,l.uncheck_node(h),this._isBulk=!1}this._updateSelectedNodes()},_openNode:function(h){var l=jQuery.jstree.reference(d(this.tree)),
n=l.get_parent(h);n.length&&l.open_node(n,!1,!0);if(this.manualCascadeOption){var q=h.id,n=l.get_node(q+"-select-all");n||(h=l.create_node(h,{id:q+"-select-all",text:this.options.selectAllText,li_attr:{rel:"bulk","class":"s3-hierarchy-action-node"}},"first"),n=l.get_node(h));(h=l.get_node(this.treeID+"-select-all"))&&l.is_checked(h)&&(this._isBulk=!0,l.check_node(n),this._isBulk=!1)}},_selectBranch:function(h,l){var n=jQuery.jstree.reference(d(this.tree)),q=this;this._isBulk=!0;n.open_node(h,function(){var t=
n.get_children_dom(h);d(t).each(function(){q._selectBranch(this)});l||n.check_node(h)});this._isBulk=!1;this._updateAndClose()},_deselectBranch:function(h,l){var n=jQuery.jstree.reference(d(this.tree)),q=this;this._isBulk=!0;n.open_node(h,function(){var t=n.get_children_dom(h);d(t).each(function(){q._deselectBranch(this)});l||n.uncheck_node(h)});this._isBulk=!1;this._updateSelectedNodes()},set:function(h){var l=jQuery.jstree.reference(d(this.tree));l.uncheck_all();if(h)for(var n=0,q=h.length,t;n<
q;n++)t=d("#"+this.treeID+"-"+h[n]),l.check_node(t)},get:function(){var d=this.input.val();return d?JSON.parse(d):[]},reset:function(){this._isBulk=!0;this.tree.jstree("uncheck_all");this._isBulk=!1;this._updateSelectedNodes();return!0},openMenu:function(){this._isOpen&&this.closeMenu();var h=d(this.button),l=h.offset();d(this.wrapper).css({position:"absolute",top:l.top+h.outerHeight(),left:l.left,minWidth:h.outerWidth()-8}).show();d(this.tree).jstree("set_focus");this._isOpen=!0;h.addClass("ui-state-active");
d(this).trigger("open")},closeMenu:function(){d(this.tree).jstree("unset_focus").unbind("click.hierarchicalopts").unbind("mouseleave.hierarchicalopts");d(this.wrapper).fadeOut(50);this._isOpen=!1;d(this.button).removeClass("ui-state-active");d(this).trigger("close")},addNode:function(h,l,n,q){var t=this.tree,v=this.treeID,u="#";h&&(u=d("#"+v+"-"+h),u.length||(u="#"));l=v+"-"+l;t.jstree("create_node",u,{id:l,text:n,li_attr:{rel:"leaf"}},"last");h&&(u.attr({rel:"parent"}),t.jstree("open_node",u));q&&
t.jstree("check_node",d("#"+l))},_bindEvents:function(){var h=this,l=d(this.wrapper),n=d(this.tree),q=d(this.button),t=this._namespace,v=this.options,u=jQuery.jstree.reference(n),z=d.vakata.context.settings.icons;d(document).bind("context_show.vakata",function(){l.unbind("mouseleave.hierarchicalopts");d.vakata.context.settings.icons=z});n.bind("select_node.jstree",function(d,l){h._selectNode(l.node)}).bind("deselect_node.jstree",function(d,l){h._deselectNode(l.node)}).bind("open_node.jstree",function(d,
l){h._openNode(l.node)});q.bind("click"+t,function(){h._isOpen?h.closeMenu():h.openMenu()}).bind("keyup"+t,function(d){d.preventDefault();switch(d.keyCode){case 27:case 38:case 37:h.closeMenu();break;case 39:case 40:h.openMenu()}}).bind("mouseenter"+t,function(){q.hasClass("ui-state-disabled")||d(this).addClass("ui-state-hover")}).bind("mouseleave"+t,function(){d(this).removeClass("ui-state-hover")}).bind("focus"+t,function(){q.hasClass("ui-state-disabled")||d(this).addClass("ui-state-focus")}).bind("blur"+
t,function(){d(this).removeClass("ui-state-focus")});v.multiple&&v.bulkSelect&&!v.cascadeOptionInTree&&(l.find(".s3-hierarchy-select-all").bind("click"+t,function(d){d.preventDefault();u.select_all();h._updateSelectedNodes()}),l.find(".s3-hierarchy-deselect-all").bind("click"+t,function(d){d.preventDefault();u.deselect_all();h._updateSelectedNodes()}),l.find(".s3-hierarchy-header").bind("click"+t,function(d){d.preventDefault();d.stopPropagation()}).bind("mousedown"+t,function(d){d.preventDefault();
d.stopPropagation()}));d(document).bind("mousedown"+t,function(t){var v=t.target;if(d(".jstree-contextmenu").has(v).length||3==t.which)return l.unbind("mouseleave.hierarchicalopts"),!0;n.is(v)||q.is(v)||0!==n.has(v).length||0!==q.has(v).length||h.closeMenu()});return!0},_unbindEvents:function(){var h=d(this.tree);d(this.button);var l=this._namespace;h.unbind("select_node.jstree").unbind("deselect_node.jstree").unbind("loaded.jstree").unbind("open_node.jstree");d(this.button).unbind(l);d(document).unbind(l);
return!0}})})(jQuery);
(function(d,u){var F=0;d.widget("s3.hierarchicalcrud",{options:{widgetID:null,ajaxURL:null,openLabel:"Open",openURL:null,editTitle:"Edit Record",editLabel:"Edit",editURL:null,deleteLabel:"Delete",deleteURL:null,addTitle:"Add Record",addLabel:"Add",addURL:null,icons:!1,stripes:!0,htmlTitles:!0},_create:function(){this.treeID=d(this.element).attr("id")+"-tree";this.id=F;F+=1},_init:function(){this.tree=d(this.element).find(".s3-hierarchy-tree");this.refresh()},_destroy:function(){d.Widget.prototype.destroy.call(this)},refresh:function(){var h=
this.tree,l=this.options;this._unbindEvents();var n=h.find("> ul > li");if(1==n.length){var n=n.first(),q=n.data("jstree");n.data("jstree",d.extend({},q,{opened:!0}))}var t=this;h.jstree({core:{themes:{name:"s3",icons:l.icons,stripes:l.stripes},animation:100,multiple:!1,check_callback:!0},contextmenu:{items:function(d){return{open:{label:t.options.openLabel,action:function(h){t._openNode(d)},separator_after:!0},edit:{label:t.options.editLabel,action:function(h){t._editNode(d)}},"delete":{label:t.options.deleteLabel,
separator_after:!0,action:function(h){t._deleteNode(d)}},add:{label:t.options.addLabel,action:function(h){t._addNode(d)}}}},select_node:!0},plugins:["sort","contextmenu"]});this._bindEvents()},_openNode:function(d){var l=this.options.openURL;d=d.id;l&&d&&(d=parseInt(d.split("-").pop()))&&(l=l.replace("[id]",d).replace("%5Bid%5D",d),window.location.href=l)},refreshNode:function(h){var l=this.options.ajaxURL,n=h.attr("id");if(l&&n&&(n=parseInt(n.split("-").pop()))){var q=this.tree,t=this.treeID;d.getJSONS3(l+
"?node="+n,function(d){d.label&&q.jstree("rename_node",h,d.label);if(d=d.children){q.jstree("open_node",h,!1,!1);for(var l,n,u,y=0,E=0,C=d.length;E<C;E++)l=d[E],n=t+"-"+l.node,(u=q.jstree("get_node","#"+n,!0))?q.jstree("rename_node",u,l.label):(q.jstree("create_node",h,{id:n,text:l.label,li_attr:{rel:"leaf"}}),y++);y&&(q.jstree("open_node",h),h.attr({rel:"parent"}))}})}},_editNode:function(h){var l=this.options.editURL;h=h.id;if(l&&h){var n=parseInt(h.split("-").pop());n&&(l=l.replace("[id]",n).replace("%5Bid%5D",
n).split("?")[0],l+="?node="+h+"&hierarchy="+this.options.widgetID,h=S3.uid(),l=d('<iframe id="'+h+'" src='+l+" onload=\"S3.popup_loaded('"+h+'\')" class="loading" marginWidth="0" marginHeight="0" frameBorder="0"></iframe>').appendTo("body"),this._openPopup(l,this.options.editTitle))}},_addNode:function(h){var l=this.options.addURL;h=h.id;if(l&&h){var n=parseInt(h.split("-").pop());n&&(l=l.split("?")[0],l+="?node="+h+"&link_to_parent="+n+"&hierarchy="+this.options.widgetID,h=S3.uid(),l=d('<iframe id="'+
h+'" src='+l+" onload=\"S3.popup_loaded('"+h+'\')" class="loading" marginWidth="0" marginHeight="0" frameBorder="0"></iframe>').appendTo("body"),this._openPopup(l,this.options.addTitle))}},_deleteNode:function(h){if(!confirm(i18n.delete_confirmation))return!1;var l=this.options.deleteURL,n=h.id,q=this.tree;l&&n&&(n=parseInt(n.split("-").pop()))&&(l=l.replace("[id]",n).replace("%5Bid%5D",n).split("?")[0],d.ajaxS3({type:"POST",url:l+"?recursive=1",data:{},dataType:"JSON",success:function(d){q.jstree("delete_node",
h)},error:function(d,h,l){h=d.responseText;if("{"==h[0])try{h=JSON.parse(d.responseText),S3.showAlert(h.message,"error")}catch(n){}}}))},_openPopup:function(h,l){h.dialog({close:function(d,l){if(self.parent){var t=self.parent.$("iframe.ui-dialog-content"),u=t.width();t.width(0);window.setTimeout(function(){t.width(u)},300)}h.remove()},minHeight:480,modal:!0,open:function(l,q){d(".ui-widget-overlay").bind("click",function(){h.dialog("close")})},title:l,minWidth:320,closeText:""})},_bindEvents:function(){return!0},
