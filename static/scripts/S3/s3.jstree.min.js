/*
 ...
 requires jQuery jstree 3.0.3
*/
(function(d){"function"===typeof define&&define.amd?define(["jquery"],d):"object"===typeof exports?d(require("jquery")):d(jQuery)})(function(d,v){if(!d.jstree){var D=0,h=!1,m=!1,n=!1,u=[],r=d("script:last").attr("src"),t=document,B=t.createElement("LI"),z;B.setAttribute("role","treeitem");z=t.createElement("I");z.className="jstree-icon jstree-ocl";z.setAttribute("role","presentation");B.appendChild(z);z=t.createElement("A");z.className="jstree-anchor";z.setAttribute("href","#");z.setAttribute("tabindex",
"-1");t=t.createElement("I");t.className="jstree-icon jstree-themeicon";t.setAttribute("role","presentation");z.appendChild(t);B.appendChild(z);z=t=null;d.jstree={version:"3.0.9",defaults:{plugins:[]},plugins:{},path:r&&-1!==r.indexOf("/")?r.replace(/\/[^\/]+$/,""):"",idregex:/[\\:&!^|()\[\]<>@*'+~#";.,=\- \/${}%?`]/g};d.jstree.create=function(a,b){var e=new d.jstree.core(++D),k=b;b=d.extend(!0,{},d.jstree.defaults,b);k&&k.plugins&&(b.plugins=k.plugins);d.each(b.plugins,function(a,k){"core"!==a&&
...
(c.li_attr[d]=a.li_attr[d]);c.li_attr.id&&!c.id&&(c.id=c.li_attr.id.toString());c.id||(c.id=k);c.li_attr.id||(c.li_attr.id=c.id);if(a&&"object"===typeof a.a_attr)for(d in a.a_attr)a.a_attr.hasOwnProperty(d)&&(c.a_attr[d]=a.a_attr[d]);if(a&&a.children&&a.children.length){d=0;for(k=a.children.length;d<k;d++)x=v(a.children[d],c.id,b),w=h[x],c.children.push(x),w.children_d.length&&(c.children_d=c.children_d.concat(w.children_d));c.children_d=c.children_d.concat(c.children)}a&&a.children&&!0===a.children&&
(c.state.loaded=!1,c.children=[],c.children_d=[]);delete a.data;delete a.children;c.original=a;h[c.id]=c;c.state.selected&&f.push(c.id);return c.id};if(c.length&&c[0].id!==e&&c[0].parent!==e){t=0;for(u=c.length;t<u;t++)c[t].children||(c[t].children=[]),h[c[t].id.toString()]=c[t];t=0;for(u=c.length;t<u;t++)h[c[t].parent.toString()].children.push(c[t].id.toString()),m.children_d.push(c[t].id.toString());t=0;for(u=m.children.length;t<u;t++)r=y(h[m.children[t]],b,m.parents.concat()),d.push(r),h[r].children_d.length&&
(d=d.concat(h[r].children_d))}else{t=0;for(u=c.length;t<u;t++)if(r=v(c[t],b,m.parents.concat()))k.push(r),d.push(r),h[r].children_d.length&&(d=d.concat(h[r].children_d));m.children=k;m.children_d=d}t=0;for(u=m.parents.length;t<u;t++)h[m.parents[t]].children_d=h[m.parents[t]].children_d.concat(d);c={cnt:q,mod:h,sel:n,par:b,dpc:d,add:f};if("undefined"===typeof window||"undefined"===typeof window.document)postMessage(c);else return c},q=function(a,c){this._cnt=a.cnt;this._model.data=a.mod;if(c){var b,
...
e=0;for(k=b.children.length;e<k;e++)c.appendChild(this.redraw_node(b.children[e],!0,!0));a.appendChild(c)}},redraw_node:function(a,b,e,k){var c=this.get_node(a),g=!1,f=!1,q=!1,l=!1,x=!1,p=!1,w="",h=document,p=this._model.data,m=!1,n=null,C=w=0;if(!c)return!1;if("#"===c.id)return this.redraw(!0);b=b||0===c.children.length;if(a=document.querySelector?this.element[0].querySelector("#"+(-1!=="0123456789".indexOf(c.id[0])?"\\3"+c.id[0]+" "+c.id.substr(1).replace(d.jstree.idregex,"\\$&"):c.id.replace(d.jstree.idregex,
...
B.cloneNode(!0);w="jstree-node ";for(l in c.li_attr)c.li_attr.hasOwnProperty(l)&&"id"!==l&&("class"!==l?a.setAttribute(l,c.li_attr[l]):w+=c.li_attr[l]);c.a_attr.id||(c.a_attr.id=c.id+"_anchor");a.setAttribute("aria-selected",!!c.state.selected);a.setAttribute("aria-level",c.parents.length);a.setAttribute("aria-labelledby",c.a_attr.id);c.state.disabled&&a.setAttribute("aria-disabled",!0);c.state.loaded&&!c.children.length?w+=" jstree-leaf":(w+=c.state.opened&&c.state.loaded?" jstree-open":" jstree-closed",
...
g.childNodes[l];break}n||(n=h.createElement("UL"),n.setAttribute("role","group"),n.className="jstree-children",g.appendChild(n));g=n;f<g.childNodes.length?g.insertBefore(a,g.childNodes[f]):g.appendChild(a);m&&(w=this.element[0].scrollTop,C=this.element[0].scrollLeft,a.childNodes[1].focus(),this.element[0].scrollTop=w,this.element[0].scrollLeft=C)}c.state.opened&&!c.state.loaded&&(c.state.opened=!1,setTimeout(d.proxy(function(){this.open_node(c.id,!1,0)},this),0));return a},open_node:function(a,b,
...
return!1}if(a.core.scroll)return a.core.scroll&&a.core.scroll.left!==v&&this.element.scrollLeft(a.core.scroll.left),a.core.scroll&&a.core.scroll.top!==v&&this.element.scrollTop(a.core.scroll.top),delete a.core.scroll,this.set_state(a,b),!1;if(a.core.selected)return g=this,this.deselect_all(),d.each(a.core.selected,function(a,e){g.select_node(e)}),delete a.core.selected,this.set_state(a,b),!1;if(d.isEmptyObject(a.core))return delete a.core,this.set_state(a,b),!1}return d.isEmptyObject(a)?(a=null,b&&
...
g;c++)b&&b.flat?this.get_json(a.children[c],b,e):k.children.push(this.get_json(a.children[c],b));return b&&b.flat?e:"#"===a.id?k.children:k},create_node:function(a,b,e,k,c){null===a&&(a="#");a=this.get_node(a);if(!a)return!1;e=e===v?"last":e;if(!e.toString().match(/^(before|after)$/)&&!c&&!this.is_loaded(a))return this.load_node(a,function(){this.create_node(a,b,e,k,!0)});b||(b={text:this.get_string("New node")});"string"===typeof b&&(b={text:b});b.text===v&&(b.text=this.get_string("New node"));var g,
...
p),e>p&&e--);p=[];w=0;for(h=c.length;w<h;w++)p[w>=e?w+1:w]=c[w];p[e]=a.id;l.children=p;this._node_changed(l.id);this.redraw("#"===l.id)}else{p=a.children_d.concat();p.push(a.id);w=0;for(h=a.parents.length;w<h;w++){c=[];C=x._model.data[a.parents[w]].children_d;m=0;for(n=C.length;m<n;m++)-1===d.inArray(C[m],p)&&c.push(C[m]);x._model.data[a.parents[w]].children_d=c}x._model.data[f].children=d.vakata.array_remove_item(x._model.data[f].children,a.id);w=0;for(h=l.parents.length;w<h;w++)this._model.data[l.parents[w]].children_d=
this._model.data[l.parents[w]].children_d.concat(p);c=[];w=0;for(h=l.children.length;w<h;w++)c[w>=e?w+1:w]=l.children[w];c[e]=a.id;l.children=c;l.children_d.push(a.id);l.children_d=l.children_d.concat(a.children_d);a.parent=l.id;p=l.parents.concat();p.unshift(l.id);C=a.parents.length;a.parents=p;p=p.concat();w=0;for(h=a.children_d.length;w<h;w++)this._model.data[a.children_d[w]].parents=this._model.data[a.children_d[w]].parents.slice(0,-1*C),Array.prototype.push.apply(this._model.data[a.children_d[w]].parents,
...
a.length);a.length=0>b?a.length+b:b;a.push.apply(a,e);return a};d.vakata.array_remove_item=function(a,b){var e=d.inArray(b,a);return-1!==e?d.vakata.array_remove(a,e):a};var y=document.createElement("I");y.className="jstree-icon jstree-checkbox";y.setAttribute("role","presentation");d.jstree.defaults.checkbox={visible:!0,three_state:!0,whole_node:!0,keep_selected_style:!0,cascade:"",tie_selection:!0};d.jstree.plugins.checkbox=function(a,b){this.bind=function(){b.bind.call(this);this._data.checkbox.uto=
...
arguments);if(c&&a.checkbox){if(!this.settings.checkbox.tie_selection){this.uncheck_all();var g=this;d.each(a.checkbox,function(a,e){g.check_node(e)})}delete a.checkbox;return!1}return c}};var A=null,I,J;d.jstree.defaults.contextmenu={select_node:!0,show_at_node:!0,items:function(a,b){return{create:{separator_before:!1,separator_after:!0,_disabled:!1,label:"Create",action:function(a){var b=d.jstree.reference(a.reference);a=b.get_node(a.reference);b.create_node(a,{},"last",function(a){setTimeout(function(){b.edit(a)},
...
action:function(a){var b=d.jstree.reference(a.reference);a=b.get_node(a.reference);b.paste(a)}}}}}}};d.jstree.plugins.contextmenu=function(a,b){this.bind=function(){b.bind.call(this);var a=0;this.element.on("contextmenu.jstree",".jstree-anchor",d.proxy(function(b,c){b.preventDefault();a=b.ctrlKey?+new Date:0;if(c||A)a=+new Date+1E4;A&&clearTimeout(A);this.is_loading(b.currentTarget)||this.show_contextmenu(b.currentTarget,b.pageX,b.pageY,b)},this)).on("click.jstree",".jstree-anchor",d.proxy(function(b){this._data.contextmenu.visible&&
...
this.show_contextmenu=function(a,b,c,g){a=this.get_node(a);if(!a||"#"===a.id)return!1;var f=this.settings.contextmenu,h=this.get_node(a,!0).children(".jstree-anchor"),l=!1,l=!1;if(f.show_at_node||b===v||c===v)l=h.offset(),b=l.left,c=l.top+this._data.core.li_height;this.settings.contextmenu.select_node&&!this.is_selected(a)&&this.activate_node(a,g);l=f.items;d.isFunction(l)&&(l=l.call(this,a,d.proxy(function(d){this._show_contextmenu(a,b,c,d)},this)));d.isPlainObject(l)&&this._show_contextmenu(a,b,
c,l)};this._show_contextmenu=function(a,b,c,g){var f=this.get_node(a,!0).children(".jstree-anchor");d(document).one("context_show.vakata.jstree",d.proxy(function(a,e){var b="jstree-contextmenu jstree-"+this.get_theme()+"-contextmenu";d(e.element).addClass(b)},this));this._data.contextmenu.visible=!0;d.vakata.context.show(f,{x:b,y:c},g);this.trigger("show_contextmenu",{node:a,x:b,y:c})}};d(function(){d(document).on("touchmove.vakata.jstree",function(a){A&&a.originalEvent&&a.originalEvent.changedTouches&&
a.originalEvent.changedTouches[0]&&(50<Math.abs(I-a.pageX)||50<Math.abs(J-a.pageY))&&clearTimeout(A)}).on("touchend.vakata.jstree",function(a){A&&clearTimeout(A)})});(function(a){var b=!1,e=!1,d=!1,c=0,g=0,f=[],h="",l=!1;a.vakata.context={settings:{hide_onmouseleave:0,icons:!0},_trigger:function(b){a(document).triggerHandler("context_"+b+".vakata",{reference:d,element:e,position:{x:c,y:g}})},_execute:function(b){return(b=f[b])&&(!b._disabled||a.isFunction(b._disabled)&&!b._disabled({item:b,reference:d,
...
d(g.event.target).closest(".jstree-anchor"))&&h.length&&h.parent().is(".jstree-closed, .jstree-open, .jstree-leaf")&&(l=h.offset(),m=g.event.pageY-l.top,t=h.height(),C=m<t/3?["b","i","a"]:m>t-t/3?["a","i","b"]:m>t/2?["i","a","b"]:["i","b","a"],d.each(C,function(c,m){switch(m){case "b":p=l.left-6;n=l.top;r=f.get_parent(h);u=h.parent().index();break;case "i":z=f.settings.dnd.inside_pos;G=f.get_node(h.parent());p=l.left-2;n=l.top+t/2+1;r=G.id;u="first"===z?0:"last"===z?G.children.length:Math.min(z,G.children.length);
break;case "a":p=l.left-6,n=l.top+t,r=f.get_parent(h),u=h.parent().index()+1}E=!0;F=0;for(y=g.data.nodes.length;F<y;F++)if(v=g.data.origin&&(g.data.origin.settings.dnd.always_copy||g.data.origin.settings.dnd.copy&&(g.event.metaKey||g.event.ctrlKey))?"copy_node":"move_node",B=u,"move_node"===v&&"a"===m&&g.data.origin&&g.data.origin===f&&r===f.get_parent(g.data.nodes[F])&&(A=f.get_node(r),B>d.inArray(g.data.nodes[F],A.children)&&--B),E=E&&(f&&f.settings&&f.settings.dnd&&!1===f.settings.dnd.check_while_dragging||
f.check(v,g.data.origin&&g.data.origin!==f?g.data.origin.get_node(g.data.nodes[F]):g.data.nodes[F],r,B,{dnd:!0,ref:f.get_node(h.parent()),pos:m,is_multi:g.data.origin&&g.data.origin!==f,is_foreign:!g.data.origin})),!E){f&&f.last_error&&(b=f.last_error());break}"i"===m&&h.parent().is(".jstree-closed")&&f.settings.dnd.open_timeout&&(e=setTimeout(function(a,e){return function(){a.open_node(e)}}(f,h),f.settings.dnd.open_timeout));if(E)return a={ins:f,par:r,pos:"i"!==m||"last"!==z||0!==u||f.is_loaded(G)?
u:"last"},k.css({left:p+"px",top:n+"px"}).show(),g.helper.find(".jstree-icon").first().removeClass("jstree-er").addClass("jstree-ok"),b={},C=!0,!1}),!0===C))return}else{E=!0;F=0;for(y=g.data.nodes.length;F<y&&(E=E&&f.check(g.data.origin&&(g.data.origin.settings.dnd.always_copy||g.data.origin.settings.dnd.copy&&(g.event.metaKey||g.event.ctrlKey))?"copy_node":"move_node",g.data.origin&&g.data.origin!==f?g.data.origin.get_node(g.data.nodes[F]):g.data.nodes[F],"#","last",{dnd:!0,ref:f.get_node("#"),pos:"i",
...
window.pageXOffset,e.originalEvent.changedTouches[0].pageY-window.pageYOffset));if(d){if(!c)if(Math.abs(e.pageX-l)>(u?a.vakata.dnd.settings.threshold_touch:a.vakata.dnd.settings.threshold)||Math.abs(e.pageY-m)>(u?a.vakata.dnd.settings.threshold_touch:a.vakata.dnd.settings.threshold))g&&(g.appendTo("body"),f=g.outerWidth()),c=!0,a.vakata.dnd._trigger("start",e);else return;var b=!1,h=!1,q=!1,r=!1,y=!1,v=h=!1,B=!1,r=b=!1;p=n=0;t=!1;a(a(e.target).parentsUntil("body").addBack().get().reverse()).filter(function(){return/^auto|scroll$/.test(a(this).css("overflow"))&&
...
this)).on("clear_search.jstree",d.proxy(function(a,b){this._data.search.som&&b.nodes.length&&this.element.find(".jstree-node").css("display","").filter(".jstree-last").filter(function(){return this.nextSibling}).removeClass("jstree-last")},this))};this.search=function(a,b,c){if(!1===a||""===d.trim(a.toString()))return this.clear_search();a=a.toString();var g=this.settings.search,f=g.ajax?g.ajax:!1,h=null,l=[],m=[];this._data.search.res.length&&this.clear_search();c===v&&(c=g.show_only_matches);if(!b&&
...
0;for(c=0;c<h;c++)e[a.charAt(c)]|=1<<h-c-1;return e}(),p=function(a,e){var b=a/h,d=Math.abs(c-e);return g?b+d/g:d?1:b});n=function(e){e=d.caseSensitive?e:e.toLowerCase();if(a===e||-1!==e.indexOf(a))return{isMatch:!0,score:0};if(!d.fuzzy)return{isMatch:!1,score:1};var g,n,r=e.length,t=f,u=e.indexOf(a,c),w,y,v=h+r,B,z,A=1,I=[];-1!==u&&(t=Math.min(p(0,u),t),u=e.lastIndexOf(a,c+h),-1!==u&&(t=Math.min(p(0,u),t)));u=-1;for(g=0;g<h;g++){w=0;for(y=v;w<y;)p(g,c+y)<=t?w=y:v=y,y=Math.floor((v-w)/2+w);v=y;w=
Math.max(1,c-y+1);n=Math.min(c+y,r)+h;y=Array(n+2);for(y[n+1]=(1<<g)-1;n>=w;n--)if(z=m[e.charAt(n-1)],y[n]=0===g?(y[n+1]<<1|1)&z:(y[n+1]<<1|1)&z|(B[n+1]|B[n])<<1|1|B[n+1],y[n]&l&&(A=p(g,n-1),A<=t))if(t=A,u=n-1,I.push(u),u>c)w=Math.max(1,2*c-u);else break;if(p(g+1,c)>t)break;B=y}return{isMatch:0<=u,score:A}};return!0===e?{search:n}:n(e)}})(d);d.jstree.defaults.sort=function(a,b){return this.get_text(a)>this.get_text(b)?1:-1};d.jstree.plugins.sort=function(a,b){this.bind=function(){b.bind.call(this);
...
d.jstree.defaults.types={"#":{},"default":{}};d.jstree.plugins.types=function(a,b){this.init=function(a,d){var c,g;if(d&&d.types&&d.types["default"])for(c in d.types)if("default"!==c&&"#"!==c&&d.types.hasOwnProperty(c))for(g in d.types["default"])d.types["default"].hasOwnProperty(g)&&d.types[c][g]===v&&(d.types[c][g]=d.types["default"][g]);b.init.call(this,a,d);this._model.data["#"].type="#"};this.refresh=function(a,d){b.refresh.call(this,a,d);this._model.data["#"].type="#"};this.bind=function(){this.element.on("model.jstree",
...
a||-1===d.inArray(k.id,c.children)){h=this.get_rules(c);if(h.max_children!==v&&-1!==h.max_children&&h.max_children===c.children.length)return this._data.core.last_error={error:"check",plugin:"types",id:"types_01",reason:"max_children prevents function: "+a,data:JSON.stringify({chk:a,pos:g,obj:k&&k.id?k.id:!1,par:c&&c.id?c.id:!1})},!1;if(h.valid_children!==v&&-1!==h.valid_children&&-1===d.inArray(k.type||"default",h.valid_children))return this._data.core.last_error={error:"check",plugin:"types",id:"types_02",
reason:"valid_children prevents function: "+a,data:JSON.stringify({chk:a,pos:g,obj:k&&k.id?k.id:!1,par:c&&c.id?c.id:!1})},!1;if(f&&k.children_d&&k.parents){m=l=0;for(n=k.children_d.length;m<n;m++)l=Math.max(l,f[k.children_d[m]].parents.length);l=l-k.parents.length+1}if(0>=l||l===v)l=1;do{if(h.max_depth!==v&&-1!==h.max_depth&&h.max_depth<l)return this._data.core.last_error={error:"check",plugin:"types",id:"types_03",reason:"max_depth prevents function: "+a,data:JSON.stringify({chk:a,pos:g,obj:k&&k.id?
...
a,h,c,g,f);c=c===v?"last":c;if(!c.toString().match(/^(before|after)$/)&&!f&&!this.is_loaded(a))return b.create_node.call(this,a,h,c,g,f);h||(h={});var m,l,n,p,t,r=this._model.data,u=this.settings.unique.case_sensitive,y=this.settings.unique.duplicate;l=m=this.get_string("New node");n=[];p=0;for(t=a.children.length;p<t;p++)n.push(u?r[a.children[p]].text:r[a.children[p]].text.toLowerCase());for(p=1;-1!==d.inArray(u?l:l.toLowerCase(),n);)l=y.call(this,m,++p).toString();h.text=l}return b.create_node.call(this,
...
(function(d,v){var D=0;d.widget("s3.hierarchicalopts",{options:{selected:null,multiple:!0,leafonly:!0,cascade:!1,bulkSelect:!1,cascadeOptionInTree:!0,noneSelectedText:"Select",selectedText:"selected",noOptionsText:"No options available",selectAllText:"Select All",deselectAllText:"Deselect All",icons:!1,stripes:!0,htmlTitles:!0},_create:function(){var h=d(this.element),m=this.options;this.id=D;D+=1;this._namespace=".hierarchicalopts"+D;this.treeID=h.attr("id")+"-tree";this.input=h.find(".s3-hierarchy-input").first();
this.input.data("input",!0);var n=m.selected;n&&this.input.val(JSON.stringify(n));this.button=d('<button type="button" class="s3-hierarchy-button ui-multiselect ui-widget ui-state-default ui-corner-all"><span class="ui-icon ui-icon-triangle-1-s"></span></button>');this.buttonText=d("<span>"+m.noneSelectedText+"</span>").appendTo(this.button);this.noopts=d('<span class="no-options-available">'+m.noOptionsText+"</span>").hide();this.tree=h.find(".s3-hierarchy-tree").first();this.wrapper=this.tree.closest(".s3-hierarchy-wrapper").hide().before(this.noopts).before(this.button).detach().appendTo("body");
this.manualCascadeOption=this._isBulk=this._isOpen=!1},_init:function(){this.refresh()},_destroy:function(){d.Widget.prototype.destroy.call(this);this._unbindEvents();d(this.button).remove();d(this.noopts).remove();d(this.tree).detach().appendTo(this.element).show()},refresh:function(){this._unbindEvents();var h=this.options,m=this.tree;if(m.find("li").length){this.noopts.hide();this.button.show();var n=this.input.val();if(n){var n=JSON.parse(n),u=this.treeID;d.each(n,function(){d("#"+u+"-"+this).data("jstree",
{selected:!0})})}n=m.find("> ul > li");if(1==n.length){var n=n.first(),r=n.data("jstree");n.data("jstree",d.extend({},r,{opened:!0}))}var n=h.multiple,r=h.leafonly,t=h.cascade,v=!1,z=null,y=["sort","checkbox"];this.input.data("multiple",n);if((t||r)&&n)v=!0;else if(n)if(h.cascadeOptionInTree)this.manualCascadeOption=!0;else{var A=this,z=function(h){return m.jstree("is_parent",h)?(d.vakata.context.settings.icons=!1,{select_all:{label:A.options.selectAllText,icon:!1,action:function(d){A._selectBranch(h)}},
deselect_all:{label:A.options.deselectAllText,icon:!1,action:function(d){A._deselectBranch(h)}}}):null};y.push("contextmenu")}m.jstree({core:{themes:{name:"s3",icons:h.icons,stripes:h.stripes},animation:100,multiple:n,check_callback:!0},checkbox:{three_state:v},sort:function(d,h){var m=this.get_text(d)>this.get_text(h)?1:-1,n,a;try{n=this.get_node(d).li_attr.rel}catch(b){n=null}try{a=this.get_node(h).li_attr.rel}catch(e){a=null}return"bulk"==n?-1:"bulk"==a?1:"none"==n?-1:"none"==a?1:m},contextmenu:{items:z,
select_node:!1,icons:!1},plugins:y});r=jQuery.jstree.reference(m);n&&((h.bulkSelect||this.manualCascadeOption)&&h.cascadeOptionInTree?(this.wrapper.find(".s3-hierarchy-header").hide(),r.create_node("#",{id:this.treeID+"-select-all",text:h.selectAllText,li_attr:{rel:"bulk","class":"s3-hierarchy-action-node"}},"first")):this.wrapper.find(".s3-hierarchy-header").removeClass("hide").show());h=r.get_checked();this._updateButtonText(h);this._bindEvents()}else this.button.hide(),this.noopts.show()},_updateSelectedNodes:function(){var h=
jQuery.jstree.reference(d(this.tree)),m=this.input.val(),m=m?JSON.parse(m):[],n=[],u=[],r=this.options.leafonly,t=h.get_checked(!0);d(t).each(function(){var m=d(this).attr("id");if("bulk"!=d(this).attr("rel")&&m&&(!r||h.is_leaf(this))){var t=m.split("-").pop();"None"!=t&&(t=parseInt(t));t&&(n.push(t),u.push(m))}});var t=!1,v=d(n).not(m).get();v.length?t=!0:(v=d(m).not(n).get(),v.length&&(t=!0));this.input.val(JSON.stringify(n));this._updateButtonText(u);t&&(this.input.change(),d(this.element).trigger("select.s3hierarchy"));
return!0},_updateButtonText:function(h){var m=null,m=this.options,n=0;h&&(n=h.length);if(n)if(1<n)m=m.selectedText.replace("#",n);else{for(var u=[],r=0;r<n;r++)u.push(d("#"+h[r]+" > a").text().replace(/^\s+|\s+$/g,""));m=u.length?u.join(" "):m.noneSelectedText}else m=m.noneSelectedText;this.buttonText.text(m)},_updateAndClose:function(){if(!this._isBulk){this._updateSelectedNodes();var d=this.wrapper,m=this;if(m.options.multiple)d.unbind("mouseleave.hierarchicalopts").one("mouseleave.hierarchicalopts",
function(){window.setTimeout(function(){m.closeMenu()},100)});else window.setTimeout(function(){m.closeMenu()},100)}},_selectNode:function(h){var m=this.options,n=jQuery.jstree.reference(d(this.tree)),u=m.multiple,r=m.leafonly;if(u)this._isBulk||"bulk"!=h.li_attr.rel||((m=n.get_parent(h))&&"#"!=m?(m=n.get_node(m),this._selectBranch(m,!0)):n.select_all());else{var t=h.id;d(n.get_checked(!0)).each(function(){t!=this.id&&(r&&!n.is_leaf(this)||n.uncheck_node(this))})}if(r&&!u&&!n.is_leaf(h))return n.deselect_node(h),
!1;this._updateAndClose()},_deselectNode:function(h){var m=jQuery.jstree.reference(d(this.tree));if(!this._isBulk){if("bulk"==h.li_attr.rel)(h=m.get_parent(h))&&"#"!=h?(h=m.get_node(h),this._deselectBranch(h,!0)):m.deselect_all();else if(h=m.get_parent(h))if(h=m.get_node(h+"-select-all"))this._isBulk=!0,m.uncheck_node(h),this._isBulk=!1;if(h=m.get_node(this.treeID+"-select-all"))this._isBulk=!0,m.uncheck_node(h),this._isBulk=!1}this._updateSelectedNodes()},_openNode:function(h){var m=jQuery.jstree.reference(d(this.tree)),
n=m.get_parent(h);n.length&&m.open_node(n,!1,!0);if(this.manualCascadeOption){var u=h.id,n=m.get_node(u+"-select-all");n||(h=m.create_node(h,{id:u+"-select-all",text:this.options.selectAllText,li_attr:{rel:"bulk","class":"s3-hierarchy-action-node"}},"first"),n=m.get_node(h));(h=m.get_node(this.treeID+"-select-all"))&&m.is_checked(h)&&(this._isBulk=!0,m.check_node(n),this._isBulk=!1)}},_selectBranch:function(h,m){var n=jQuery.jstree.reference(d(this.tree)),u=this;this._isBulk=!0;n.open_node(h,function(){var r=
n.get_children_dom(h);d(r).each(function(){u._selectBranch(this)});m||n.check_node(h)});this._isBulk=!1;this._updateAndClose()},_deselectBranch:function(h,m){var n=jQuery.jstree.reference(d(this.tree)),u=this;this._isBulk=!0;n.open_node(h,function(){var r=n.get_children_dom(h);d(r).each(function(){u._deselectBranch(this)});m||n.uncheck_node(h)});this._isBulk=!1;this._updateSelectedNodes()},set:function(h){var m=jQuery.jstree.reference(d(this.tree));m.uncheck_all();if(h)for(var n=0,u=h.length,r;n<
u;n++)r=d("#"+this.treeID+"-"+h[n]),m.check_node(r)},get:function(){var d=this.input.val();return d?JSON.parse(d):[]},reset:function(){this._isBulk=!0;this.tree.jstree("uncheck_all");this._isBulk=!1;this._updateSelectedNodes();return!0},openMenu:function(){this._isOpen&&this.closeMenu();var h=d(this.button),m=h.offset();d(this.wrapper).css({position:"absolute",top:m.top+h.outerHeight(),left:m.left,minWidth:h.outerWidth()-8}).show();d(this.tree).jstree("set_focus");this._isOpen=!0;h.addClass("ui-state-active");
d(this).trigger("open")},closeMenu:function(){d(this.tree).jstree("unset_focus").unbind("click.hierarchicalopts").unbind("mouseleave.hierarchicalopts");d(this.wrapper).fadeOut(50);this._isOpen=!1;d(this.button).removeClass("ui-state-active");d(this).trigger("close")},addNode:function(h,m,n,u){var r=this.tree,t=this.treeID,v="#";h&&(v=d("#"+t+"-"+h),v.length||(v="#"));m=t+"-"+m;r.jstree("create_node",v,{id:m,text:n,li_attr:{rel:"leaf"}},"last");h&&(v.attr({rel:"parent"}),r.jstree("open_node",v));u&&
r.jstree("check_node",d("#"+m))},_bindEvents:function(){var h=this,m=d(this.wrapper),n=d(this.tree),u=d(this.button),r=this._namespace,t=this.options,v=jQuery.jstree.reference(n),z=d.vakata.context.settings.icons;d(document).bind("context_show.vakata",function(){m.unbind("mouseleave.hierarchicalopts");d.vakata.context.settings.icons=z});n.bind("select_node.jstree",function(d,m){h._selectNode(m.node)}).bind("deselect_node.jstree",function(d,m){h._deselectNode(m.node)}).bind("open_node.jstree",function(d,
m){h._openNode(m.node)});u.bind("click"+r,function(){h._isOpen?h.closeMenu():h.openMenu()}).bind("keyup"+r,function(d){d.preventDefault();switch(d.keyCode){case 27:case 38:case 37:h.closeMenu();break;case 39:case 40:h.openMenu()}}).bind("mouseenter"+r,function(){u.hasClass("ui-state-disabled")||d(this).addClass("ui-state-hover")}).bind("mouseleave"+r,function(){d(this).removeClass("ui-state-hover")}).bind("focus"+r,function(){u.hasClass("ui-state-disabled")||d(this).addClass("ui-state-focus")}).bind("blur"+
r,function(){d(this).removeClass("ui-state-focus")});t.multiple&&t.bulkSelect&&!t.cascadeOptionInTree&&(m.find(".s3-hierarchy-select-all").bind("click"+r,function(d){d.preventDefault();v.select_all();h._updateSelectedNodes()}),m.find(".s3-hierarchy-deselect-all").bind("click"+r,function(d){d.preventDefault();v.deselect_all();h._updateSelectedNodes()}),m.find(".s3-hierarchy-header").bind("click"+r,function(d){d.preventDefault();d.stopPropagation()}).bind("mousedown"+r,function(d){d.preventDefault();
d.stopPropagation()}));d(document).bind("mousedown"+r,function(t){var r=t.target;if(d(".jstree-contextmenu").has(r).length||3==t.which)return m.unbind("mouseleave.hierarchicalopts"),!0;n.is(r)||u.is(r)||0!==n.has(r).length||0!==u.has(r).length||h.closeMenu()});return!0},_unbindEvents:function(){var h=d(this.tree);d(this.button);var m=this._namespace;h.unbind("select_node.jstree").unbind("deselect_node.jstree").unbind("loaded.jstree").unbind("open_node.jstree");d(this.button).unbind(m);d(document).unbind(m);
return!0}})})(jQuery);
(function(d,v){var D=0;d.widget("s3.hierarchicalcrud",{options:{widgetID:null,ajaxURL:null,openLabel:"Open",openURL:null,editTitle:"Edit Record",editLabel:"Edit",editURL:null,deleteLabel:"Delete",deleteURL:null,addTitle:"Add Record",addLabel:"Add",addURL:null,icons:!1,stripes:!0,htmlTitles:!0},_create:function(){this.treeID=d(this.element).attr("id")+"-tree";this.id=D;D+=1},_init:function(){this.tree=d(this.element).find(".s3-hierarchy-tree");this.refresh()},_destroy:function(){d.Widget.prototype.destroy.call(this)},refresh:function(){var h=
this.tree,m=this.options;this._unbindEvents();var n=h.find("> ul > li");if(1==n.length){var n=n.first(),u=n.data("jstree");n.data("jstree",d.extend({},u,{opened:!0}))}var r=this;h.jstree({core:{themes:{name:"s3",icons:m.icons,stripes:m.stripes},animation:100,multiple:!1,check_callback:!0},contextmenu:{items:function(d){return{open:{label:r.options.openLabel,action:function(h){r._openNode(d)},separator_after:!0},edit:{label:r.options.editLabel,action:function(h){r._editNode(d)}},"delete":{label:r.options.deleteLabel,
separator_after:!0,action:function(h){r._deleteNode(d)}},add:{label:r.options.addLabel,action:function(h){r._addNode(d)}}}},select_node:!0},plugins:["sort","contextmenu"]});this._bindEvents()},_openNode:function(d){var m=this.options.openURL;d=d.id;m&&d&&(d=parseInt(d.split("-").pop()))&&(m=m.replace("[id]",d).replace("%5Bid%5D",d),window.location.href=m)},refreshNode:function(h){var m=this.options.ajaxURL,n=h.attr("id");if(m&&n&&(n=parseInt(n.split("-").pop()))){var u=this.tree,r=this.treeID;d.getJSONS3(m+
"?node="+n,function(d){d.label&&u.jstree("rename_node",h,d.label);if(d=d.children){u.jstree("open_node",h,!1,!1);for(var m,n,v,A=0,D=0,J=d.length;D<J;D++)m=d[D],n=r+"-"+m.node,(v=u.jstree("get_node","#"+n,!0))?u.jstree("rename_node",v,m.label):(u.jstree("create_node",h,{id:n,text:m.label,li_attr:{rel:"leaf"}}),A++);A&&(u.jstree("open_node",h),h.attr({rel:"parent"}))}})}},_editNode:function(h){var m=this.options.editURL;h=h.id;if(m&&h){var n=parseInt(h.split("-").pop());n&&(m=m.replace("[id]",n).replace("%5Bid%5D",
n).split("?")[0],m+="?node="+h+"&hierarchy="+this.options.widgetID,h=S3.uid(),m=d('<iframe id="'+h+'" src='+m+" onload=\"S3.popup_loaded('"+h+'\')" class="loading" marginWidth="0" marginHeight="0" frameBorder="0"></iframe>').appendTo("body"),this._openPopup(m,this.options.editTitle))}},_addNode:function(h){var m=this.options.addURL;h=h.id;if(m&&h){var n=parseInt(h.split("-").pop());n&&(m=m.split("?")[0],m+="?node="+h+"&link_to_parent="+n+"&hierarchy="+this.options.widgetID,h=S3.uid(),m=d('<iframe id="'+
h+'" src='+m+" onload=\"S3.popup_loaded('"+h+'\')" class="loading" marginWidth="0" marginHeight="0" frameBorder="0"></iframe>').appendTo("body"),this._openPopup(m,this.options.addTitle))}},_deleteNode:function(h){if(!confirm(i18n.delete_confirmation))return!1;var m=this.options.deleteURL,n=h.id,u=this.tree;m&&n&&(n=parseInt(n.split("-").pop()))&&(m=m.replace("[id]",n).replace("%5Bid%5D",n).split("?")[0],d.ajaxS3({type:"POST",url:m+"?recursive=1",data:{},dataType:"JSON",success:function(d){u.jstree("delete_node",
h)},error:function(d,h,m){h=d.responseText;if("{"==h[0])try{h=JSON.parse(d.responseText),S3.showAlert(h.message,"error")}catch(n){}}}))},_openPopup:function(h,m){h.dialog({close:function(d,m){if(self.parent){var r=self.parent.$("iframe.ui-dialog-content"),t=r.width();r.width(0);window.setTimeout(function(){r.width(t)},300)}h.remove()},minHeight:480,modal:!0,open:function(m,u){d(".ui-widget-overlay").bind("click",function(){h.dialog("close")})},title:m,minWidth:320,closeText:""})},_bindEvents:function(){return!0},
