...
y=!0:(G=g(u).not(x).get(),G.length&&(y=!0));u=this.input.val(this._stringifySelection(x));this._updateButtonText(C);y&&(u.change(),g(this.element).trigger("select.s3hierarchy"));return!0},_updateButtonText:function(t){var u=this.options,x=0;t&&(x=t.length);if(x)if(1<x)t=u.selectedText.replace("#",x);else{for(var C=jQuery.jstree.reference(g(this.tree)),D=[],y,G,O=u.sep,I=0;I<x;I++){if(O)for(G=t[I],y=g("#"+G+" > a").text().replace(/^\s+|\s+$/g,""),G=C.get_parent(G);"#"!=G;)y=g("#"+G+" > a").text().replace(/^\s+|\s+$/g,
"")+O+y,G=C.get_parent(G);else y=g("#"+t[I]+" > a").text().replace(/^\s+|\s+$/g,"");D.push(y)}t=D.length?D.join(" "):u.noneSelectedText}else t=u.noneSelectedText;this.buttonText.text(t)},_updateAndClose:function(){if(!this._isBulk){this._updateSelectedNodes();var t=this.wrapper,u=this;if(u.options.multiple)t.unbind("mouseleave.hierarchicalopts").one("mouseleave.hierarchicalopts",function(){window.setTimeout(function(){u.closeMenu()},100)});else window.setTimeout(function(){u.closeMenu()},100)}},_selectNode:function(t){var u=
...
this._namespace+"-mpos",function(){x._setMenuPosition(u,t)});u.show();g(this.tree).jstree("set_focus");this._isOpen=!0;t.addClass("ui-state-active");g(this).trigger("open")},closeMenu:function(){g(window).off(this._namespace+"-mpos");g(this.tree).jstree("unset_focus").unbind("click.hierarchicalopts").unbind("mouseleave.hierarchicalopts");g(this.wrapper).fadeOut(50);this._isOpen=!1;g(this.button).removeClass("ui-state-active");g(this).trigger("close")},addNode:function(t,u,x,C){var D=this.tree,y=this.treeID;
...
t=[];return t},_stringifySelection:function(t){if(!this.options.multiple)if(t.length)t=t[0];else return"";return JSON.stringify(t)},_bindEvents:function(){var t=this,u=g(this.wrapper),x=g(this.tree),C=g(this.button),D=this._namespace,y=this.options,G=jQuery.jstree.reference(x),O=g.vakata.context.settings.icons;g(document).bind("context_show.vakata",function(){u.unbind("mouseleave.hierarchicalopts");g.vakata.context.settings.icons=O});x.bind("select_node.jstree",function(I,N){t._selectNode(N.node)}).bind("deselect_node.jstree",
function(I,N){t._deselectNode(N.node)}).bind("open_node.jstree",function(I,N){t._openNode(N.node)});C.bind("click"+D,function(){t._isOpen?t.closeMenu():t.openMenu()}).bind("keyup"+D,function(I){I.preventDefault();switch(I.keyCode){case 27:case 38:case 37:t.closeMenu();break;case 39:case 40:t.openMenu()}}).bind("mouseenter"+D,function(){C.hasClass("ui-state-disabled")||g(this).addClass("ui-state-hover")}).bind("mouseleave"+D,function(){g(this).removeClass("ui-state-hover")}).bind("focus"+D,function(){C.hasClass("ui-state-disabled")||
g(this).addClass("ui-state-focus")}).bind("blur"+D,function(){g(this).removeClass("ui-state-focus")});y.multiple&&y.bulkSelect&&!y.cascadeOptionInTree&&(u.find(".s3-hierarchy-select-all").bind("click"+D,function(I){I.preventDefault();G.select_all();t._updateSelectedNodes()}),u.find(".s3-hierarchy-deselect-all").bind("click"+D,function(I){I.preventDefault();G.deselect_all();t._updateSelectedNodes()}),u.find(".s3-hierarchy-header").bind("click"+D,function(I){I.preventDefault();I.stopPropagation()}).bind("mousedown"+
D,function(I){I.preventDefault();I.stopPropagation()}));g(document).bind("mousedown"+D,function(I){var N=I.target;if(g(".jstree-contextmenu").has(N).length||3==I.which)return u.unbind("mouseleave.hierarchicalopts"),!0;x.is(N)||C.is(N)||0!==x.has(N).length||0!==C.has(N).length||t.closeMenu()});return!0},_unbindEvents:function(){var t=g(this.tree);g(this.button);var u=this._namespace;t.unbind("select_node.jstree").unbind("deselect_node.jstree").unbind("loaded.jstree").unbind("open_node.jstree");g(this.button).unbind(u);
g(document).unbind(u);return!0}})})(jQuery);
...
y=D.width();D.width(0);window.setTimeout(function(){D.width(y)},300)}t.remove()},minHeight:480,modal:!0,open:function(x,C){g(".ui-widget-overlay").bind("click",function(){t.dialog("close")})},title:u,minWidth:320,closeText:""})},_bindEvents:function(){return!0},_unbindEvents:function(){this.tree.jstree("destroy","true");return!0}})})(jQuery);
