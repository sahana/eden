...
message+'<button type="button" class="close" data-dismiss="alert">\u00d7</button></div>'),e.parent().append(d).undelegate(".s3").delegate(".alert","click.s3",function(){$(this).fadeOut("slow").remove();return!1}))});k.on("click.s3",".inline-edt",function(){a=(a=h.val())?parseFloat(a):0;c-=a});n.click(function(){a=(a=h.val())?parseFloat(a):0;c+=a;g.change()});e.change(function(){(b=e.val())?(b=parseFloat(b),$("#inv_adj_item_new_quantity-error").remove()):b=S3.supply.oldQuantity||0;b<c&&(message="Total Quantity reduced to Quantity in Bins",
