...
!1);var p=a(m);var q=$("#"+d+"-ge-data").val();q=q.replace(h,"")}if(k){l=!0;m=f.calendarWidget("getJSDate",!0);var A=a(m);var w=g.replace(h,"")}l&&(a="("+$("#"+d+"-negative").val()+" eq None)",r&&(a+=" or ("+q+' lt "'+p+'")'),k&&(a+=" or ("+w+' gt "'+A+'")'),A=$("#"+d+"-filterby"),A.length&&(A=A.val(),d=$("#"+d+"-filter_opts").val(),a+=" or ("+A+' eq "'+d+'")'),e.push(["$filter",a]))}}else k?(m=f.calendarWidget("getJSDate",d),A=a(m),d&&f.hasClass("end_date")?(d=g.replace(h,""),e.push(["$filter","("+
...
d=c.length;d?$.when(n()).then(function(){for(var b=S3.gis,e=0;e<d;e++){var f=c[e].attributes.id.value;void 0===b.maps[f]&&b.show_map(f,b.options[f])}l(a)},function(a){s3_debug(a)},function(a){s3_debug(a)}):l(a);b.find("table.dataTable.display").each(function(){q(this)})};S3.search.unhide_section=u;S3.search.summary_tabs=function(a,b,c){if(c)for(var d=g($("#"+a)),e=c.split(","),f=0,h=e.length;f<h;f++)c=e[f],p(a,c,d);void 0!=b?($("#summary-tabs").tabs({active:b,activate:function(b,c){b=$(c.newPanel);
b.removeClass("hide");if(c.newTab.length){var d=$(c.newTab).index();c=$("#"+a).find('input.filter-submit-url[type="hidden"]');if(c.length){c=c.first();var e=$(c).val(),f=e.split("?"),g=[];if(1<f.length){e=f[1].split("&");for(var h,k,l=0;l<e.length;l++)h=e[l].split("="),1<h.length&&(k=decodeURIComponent(h[0]),"t"!=k&&g.push(h[0]+"="+h[1]));g.push("t="+d);d=g.join("&");f=f[0];d&&(f=f+"?"+d)}else f=e+"?t="+d;$(c).val(f)}}u(a,b)}}).css({visibility:"visible"}),$('.ui-tabs-panel[aria-hidden="false"]').first().removeClass("hide").find("table.dataTable.display").each(function(){q(this)})):
$("#summary-tabs").css({visibility:"visible"}).find("table.dataTable.display").each(function(){q(this)})};S3.search.summary_maps=function(a){for(var b=$("#summary-common, #summary-sections").find(".map_wrapper"),c=0;c<b.length;c++){var d=b[c];if(!d.hidden){var e=S3.gis;d=d.attributes.id.value;if(void 0===e.maps[d]){var f=e.options[d];void 0!=f&&(e.show_map(d,f),d=g($("#"+a)),e.refreshLayer("search_results",d))}}}};S3.search.s3map=function(a){var b=S3.gis;void 0===a&&(a="default_map");b.show_map(a,
b.options[a]);a=g();b.refreshLayer("search_results",a)};var z=function(a){var b=a.name,c=b.slice(0,-1),d=parseInt(b.slice(-1)),e=$("#"+b);if(e.val()){var f=d+1,g=c+f,h=$("#"+g);h.length||(f=d+2,g=c+f,h=$("#"+g));if(h.length){var k=g.replace(/-/g,"_");S3[k]();h.next(".ui-multiselect").show();k=S3.location_filter_hierarchy;if(void 0!=S3.location_name_l10n){h=!0;var l=S3.location_name_l10n}else h=!1;a.options1=[];a.options2=[];a.options3=[];a.options4=[];a.options5=[];for(b=d;0<=b&&!k.hasOwnProperty("L"+
b);b--);var m=function(b){var d,e;for(d in b)if(b.hasOwnProperty(d)){d&&"null"!=d&&a["options"+f].push(d);var h=b[d];if("object"===typeof h&&(g=f+1,$("#"+c+g).length||(g=f+2),$("#"+c+g).length))for(e in h)h.hasOwnProperty(e)&&e&&"null"!=e&&a["options"+g].push(e)}},n=function(a,b){var e,f,g=$("#"+c+b).val();for(f in a)if(a.hasOwnProperty(f))if(null===g)if(b===d)m(a[f]);else{var h=b+1;$("#"+c+h).length||h++;n(a[f],h)}else for(e in g)g[e]===f&&(b===d?m(a[f]):(h=b+1,$("#"+c+h).length||h++,n(a[f],h)))};
...
b.length&&(b=b.next(".location-filter").next(".ui-multiselect"),b.length&&(b.hide(),b=b.next(".location-filter").next(".ui-multiselect"),b.length&&(b.hide(),b=b.next(".location-filter").next(".ui-multiselect"),b.length&&(b.hide(),b=b.next(".location-filter").next(".ui-multiselect"),b.length&&b.hide())))))}}}}else{for(k=d+1;5>=k;k++)if(p=$("#"+c+k),p.length)if(p.html(""),p.hasClass("groupedopts-filter-widget")&&p.groupedopts("instance"))try{p.groupedopts("refresh")}catch(W){}else p.hasClass("multiselect-filter-widget")&&
...
$(".hierarchy-filter").on("select.s3hierarchy",function(){$(this).closest("form").trigger("optionChanged")});$(".map-filter").each(function(){var a=$(this);$.when(n()).then(function(){var b=a.attr("id"),c=$("#"+b),d,e=S3.gis,f=e.maps[b+"-map"],g=f.s3;g.pointPlaced=function(a){var b={internalProjection:f.getProjectionObject(),externalProjection:e.proj4326};d=(new OpenLayers.Format.WKT(b)).write(a);c.val('"'+d+'"').trigger("change")};g.polygonButtonOff=function(){c.val("").trigger("change")};g.layerRefreshed=
function(){var a=g.polygonButton;a&&"drawpolygonclear-off"==a.getIconClass()&&(g.lastDraftFeature?g.lastDraftFeature.destroy():1<g.draftLayer.features.length&&g.draftLayer.features[0].destroy(),a.control.deactivate(),a.items[0].pressed=!0)};g.polygonButtonLoaded=function(){if(d=c.val()){g.polygonButton.items[0].btnEl.dom.click();var a=OpenLayers.Geometry.fromWKT(d);a.transform(e.proj4326,f.getProjectionObject());a=new OpenLayers.Feature.Vector(a,{},null);var b=g.draftLayer;b.addFeatures([a]);g.lastDraftFeature=
a;f.zoomToExtent(b.getDataExtent())}};g.polygonButtonLoaded()},function(a){s3_debug(a)},function(a){s3_debug(a)})});$(".date-filter").find(".range-picker").each(function(){function a(){for(var a,b,c=[],e=d.data("ts"),f=0;f<e.length;f++)a=e[f],b=moment(a[0]),c.push({x:b,y:a[1]});d.data("slots",c);return[{values:c,key:"",color:"#3b6596",area:!0}]}function b(){Q=O.val();M=S.val();g=new Date(d.data("min"));h=new Date(d.data("max"));R=h-g;Q?(I=moment(Q,e),D=I-g,Q=D/R*100+"%"):Q="0%";M?(J=moment(M,e),D=
J-g,M=D/R*100+"%"):M="100%";F.updatePosition(M,Q)}function c(a){var b=z[a].x;try{var c=z[a+1].x}catch(ea){c=moment(d.data("max"))}var f=Z;Z+=4E3;f=setTimeout(function(){var f=c;d.data("slot",a);O.val(b.format(e));S.val(f.format(e));O.trigger("change")},f);T.push(f)}var d=$(this),e=d.data("fmt"),f=d.data("min");if(f)var g=moment(f);else g=moment().subtract(5,"minutes"),d.data("min",g.format());f=d.data("max");if(f)var h=moment(f);else h=moment().subtract(5,"minutes"),d.data("max",h.format());var k=
d.parent().attr("id"),l=g.format("YYYY"),m=moment(g);f="";for(var n=[{year:l,months:[]}],p=0,q;h>m||m.format("M")===h.format("M");)q=m.format("YYYY"),q!=l&&(n.push({year:q,months:[]}),l=q,p++),n[p].months.push(m.format("MMM YYYY")),m.add(1,"month");for(p=0;p<n.length;p++){l=n[p];m=l.months;l=l.year;f+='<optgroup label="'+l+'">';for(l=0;l<m.length;l++)f+='<option value="'+m[l]+'">'+m[l]+"</option>";f+="</optgroup>"}d.before('<div class="range-coarse"><div class="range-coarse-start"><label for="'+k+
'-cs">From:</label><select id="'+k+'-cs">'+f+'</select></div><div class="range-coarse-end"><label for="'+k+'-ce">to:</label><select id="'+k+'-ce">'+f+"</select></div></div>");f=$("#"+k+"-cs");n=$("#"+k+"-ce");f.val(g.format("MMM YYYY"));n.val(h.format("MMM YYYY"));a();d.before('<a class="button secondary tiny play"><i class="fa fa-play"></i> Play</a><a class="button secondary tiny hide pause"><i class="fa fa-pause"></i> Pause</a><a class="button secondary tiny hide stop"><i class="fa fa-stop"></i> Stop</a>');
