...
b.datepicker.parseDate=function(a,c,d){try{var e=this._base_parseDate(a,c,d)}catch(t){if(0<=t.indexOf(":"))e=this._base_parseDate(a,c.substring(0,c.length-(t.length-t.indexOf(":")-2)),d),b.timepicker.log("Error parsing the date string: "+t+"\ndate string = "+c+"\ndate format = "+a);else throw t;}return e};b.datepicker._base_formatDate=b.datepicker._formatDate;b.datepicker._formatDate=function(a,b,c,d){return(b=this._get(a,"timepicker"))?(b._updateDateTime(a),b.$input.val()):this._base_formatDate(a)};
...
c.push('<option value="">'+(b.msgNoRecords||"-")+"</option>");else{var e=b.fncPrep,f=null;if(e)try{f=e(a)}catch(r){}e=b.lookupField||"id";for(var g=b.fncRepresent,h,k,u,v=0;v<a.length;v++)h=a[v],k=h[e],0===v&&(d=k),u=g?g(h,f):h.name,h=h._tooltip?' title="'+h._tooltip+'"':"",c.push("<option"+h+' value="'+k+'">'+u+"</option>");b.optional&&(d=0,c.unshift('<option value=""></option>'))}return{options:c.join(""),defaultValue:d}},f=function(b,f,h,l,n){var g=!1;if(1<b.length)if("checkbox"==b.first().attr("type")){if(b.first().closest(".checkboxes-widget-s3")){s3_debug("filterOptionsS3 error: checkboxes-widget-s3 not supported, skipping");
...
textStatus:b,xhr:d});Y=x.ajaxProcess.call(z,a);"object"==typeof Y&&null!==Y&&(T=Y.title,Y=Y.content);aa&&(H&&H.call(z,a,b,u,w),w.html(Y))},complete:function(c,d){C&&C.call(z,c,d,u,w);var f=w[0].getElementsByTagName("img");e=f.length;c=0;for(d=f.length;c<d;c++)f[c].complete&&e--;e&&!b.browser.opera?b(f).bind("load.ct error.ct",function(){e--;0===e&&(a.hide(),b(f).unbind(".ct"),aa&&da(l))}):(a.hide(),aa&&da(l))}});c[v]?t(c[v],k):b.ajax(k)}},da=function(a){var c=T||x.showTitle&&"&nbsp;";var d="";var e=
...
c});x.arrows?(/(left|right)/.test(e)&&(e=u.height()-m.height(),d=0<=I&&0<a?F-E-x.dropShadowSteps:0,d=(e>d?d:e)+"px"),m.css({top:d}).show()):m.hide();(l=k(u,x))&&l.length&&l.hide().css({height:W,width:X,zIndex:y.data("cluetip").zIndex-1}).show();u.hide()[x.fx.open](x.fx.openSpeed||0);b.fn.bgiframe&&u.bgiframe();0<x.delayedClose&&(fa=setTimeout(ea,x.delayedClose));x.onShow.call(z,u,w)},ia=function(b){aa=!1;a.hide();if(!x.sticky||/click|toggle/.test(x.activation))ea(),clearTimeout(fa);x.hoverClass&&
...
g.multiselect("instance")&&g.multiselect("refresh"),z(this))}});a.find(".hierarchy-filter:visible").each(function(){g=$(this);c=g.attr("id");p=$("#"+c+"-data").val();d.hasOwnProperty(p)&&(g.is(":visible")||g.hasClass("active")||v(a),n=d[p],g.hasClass("s3-cascade-select")?g.cascadeSelect("set",n):g.hierarchicalopts("set",n))});a.data("noAutoSubmit",0);a.trigger("optionChanged")};var k=function(a,b){void 0===b&&(b=g());a=a.split("?");var c={},d={},e,f=[];b=b.filter(function(a){return"$filter"==a[0]?
(f.push(a),!1):!0});f.length&&b.push(f.reduce(function(a,b){return["$filter","("+a[1]+") and ("+b[1]+")"]}));var h=0;for(e=b.length;h<e;h++){var k=b[h];var l=k[0];k=k[1];null===k?c.hasOwnProperty(l)||(d[l]=!0):(d.hasOwnProperty(l)&&(d[l]=!1),c[l]=!0)}var p=[];if(1!=S3.search.stripFilters&&1<a.length){var m=a[1].split("&");h=0;for(e=m.length;h<e;h++)k=m[h].split("="),1<k.length&&(l=decodeURIComponent(k[0]),d[l]||c[l]||p.push(m[h]))}h=0;for(e=b.length;h<e;h++)k=b[h],l=k[0],k=k[1],c[l]&&p.push(l+"="+
encodeURIComponent(k));b=p.join("&");a=a[0];b&&(a=a+"?"+b);return a};S3.search.filterURL=k;S3.search.ajaxUpdateOptions=function(a,b){var c=$(a);a=c.find("input.filter-ajax-url");a.length&&(a=$(a[0]).val());$.ajax({url:a,dataType:"json"}).done(function(a){c.data("noAutoSubmit",1);for(var d in a){var e=$("#"+d);if(e.length){var f=a[d];if(e.hasClass("options-filter")){if("select"==e[0].tagName.toLowerCase()){var g=e.siblings(".no-options-available");if(f.hasOwnProperty("empty"))e.html(""),e.hasClass("multiselect-filter-widget")&&
e.multiselect("instance")?(e.multiselect("refresh"),e.multiselect("instance").button.hide()):e.hasClass("groupedopts-filter-widget")&&e.groupedopts("instance")?e.groupedopts("refresh"):e.hide(),g.length&&(g.html(f.empty),g.removeClass("hide").show());else{var h=e.val(),k=[],l="",p;if(f.hasOwnProperty("groups")){var m=0;for(p=f.groups.length;m<p;m++){var n=f.groups[m];n.label&&(l+='<optgroup label="'+n.label+'">');for(var q=0,t=n.items.length;q<t;q++){var r=n.items[q];var w=r[0].toString();h&&0<=$.inArray(w,
h)&&k.push(w);l+='<option value="'+w+'"';(w=r[3])&&(l+=' title="'+w+'"');r=r[1];l+=">"+r+"</option>"}n.label&&(l+="</optgroup>")}}else for(m=0,p=f.length;m<p;m++)r=f[m],w=r[0],w=null===w?"None":w.toString(),r=r[1],h&&0<=$.inArray(w,h)&&k.push(w),l+='<option value="'+w+'">'+r+"</option>";e.html(l);k&&e.val(k);g.length&&g.hide();e.hasClass("groupedopts-filter-widget")&&e.groupedopts("instance")?e.groupedopts("refresh"):e.hasClass("multiselect-filter-widget")&&e.multiselect("instance")?(e.multiselect("refresh"),
...
a.DataTable())&&a.responsive&&a.responsive.recalc();else if(a.hasClass("doublescroll"))try{a.dataTableS3("doubleScroll")}catch(w){}},t=function(a,b){var c=b.find(".map_wrapper"),d=c.length;d?$.when(n()).then(function(){for(var b=S3.gis,e=0;e<d;e++){var f=c[e].attributes.id.value;void 0===b.maps[f]&&b.show_map(f,b.options[f])}l(a)},function(a){s3_debug(a)},function(a){s3_debug(a)}):l(a);b.find("table.dataTable.display").each(function(){p(this)})};S3.search.unhide_section=t;S3.search.summary_tabs=function(a,
...
h[1]));g.push("t="+d);d=g.join("&");f=f[0];d&&(f=f+"?"+d)}else f=e+"?t="+d;$(c).val(f)}}t(a,b)}}).css({visibility:"visible"}),$('.ui-tabs-panel[aria-hidden="false"]').first().removeClass("hide").find("table.dataTable.display").each(function(){p(this)})):$("#summary-tabs").css({visibility:"visible"}).find("table.dataTable.display").each(function(){p(this)})};S3.search.summary_maps=function(a){for(var b=$("#summary-common, #summary-sections").find(".map_wrapper"),c=0;c<b.length;c++){var d=b[c];if(!d.hidden){var e=
...
k=S3.location_filter_hierarchy;if(void 0!=S3.location_name_l10n){h=!0;var l=S3.location_name_l10n}else h=!1;a.options1=[];a.options2=[];a.options3=[];a.options4=[];a.options5=[];for(b=d;0<=b&&!k.hasOwnProperty("L"+b);b--);var p=function(b){var d,e;for(d in b)if(b.hasOwnProperty(d)){d&&"null"!=d&&a["options"+f].push(d);var h=b[d];if("object"===typeof h&&(g=f+1,$("#"+c+g).length||(g=f+2),$("#"+c+g).length))for(e in h)h.hasOwnProperty(e)&&e&&"null"!=e&&a["options"+g].push(e)}},m=function(a,b){var e,
f,g=$("#"+c+b).val();for(f in a)if(a.hasOwnProperty(f))if(null===g)if(b===d)p(a[f]);else{var h=b+1;$("#"+c+h).length||h++;m(a[f],h)}else for(e in g)g[e]===f&&(b===d?p(a[f]):(h=b+1,$("#"+c+h).length||h++,m(a[f],h)))};m(k["L"+b],b);for(k=f;5>=k;k++){var n=$("#"+c+k);if(n.length){var q=a["options"+k];q.sort();var t="";for(var r in q)if(q.hasOwnProperty(r)){b=q[r];var z=h?l[b]||b:b;t+='<option value="'+b+'">'+z+"</option>"}n.html(t);if(n.hasClass("groupedopts-filter-widget")&&n.groupedopts("instance"))try{n.groupedopts("refresh")}catch(W){}else n.hasClass("multiselect-filter-widget")&&
n.multiselect("instance")&&n.multiselect("refresh");k===f&&(n.next("button").removeClass("hidden").show(),b=e.next(".ui-multiselect").next(".location-filter").next(".ui-multiselect"),b.length&&(b=b.next(".location-filter").next(".ui-multiselect"),b.length&&(b.hide(),b=b.next(".location-filter").next(".ui-multiselect"),b.length&&(b.hide(),b=b.next(".location-filter").next(".ui-multiselect"),b.length&&(b.hide(),b=b.next(".location-filter").next(".ui-multiselect"),b.length&&b.hide())))))}}}}else{for(k=
d+1;5>=k;k++)if(n=$("#"+c+k),n.length)if(n.html(""),n.hasClass("groupedopts-filter-widget")&&n.groupedopts("instance"))try{n.groupedopts("refresh")}catch(W){}else n.hasClass("multiselect-filter-widget")&&n.multiselect("instance")&&n.multiselect("refresh");b=e.next(".ui-multiselect").next(".location-filter").next(".ui-multiselect");b.length&&(b.hide(),b=b.next(".location-filter").next(".ui-multiselect"),b.length&&(b.hide(),b=b.next(".location-filter").next(".ui-multiselect"),b.length&&(b.hide(),b=
b.next(".location-filter").next(".ui-multiselect"),b.length&&(b.hide(),b=b.next(".location-filter").next(".ui-multiselect"),b.length&&b.hide()))))}},u=function(a){S3.hideAlerts("warning");var b=a.attr("id"),d=a.find('input.filter-submit-url[type="hidden"]').val(),e=g(a);if(a.hasClass("filter-ajax")){a=a.find('input.filter-submit-target[type="hidden"]').val();c[b]={};a=a.split(" ");var f={},h;for(h=0;h<a.length;h++){var l=a[h];var p=$("#"+l);var n=p.is(":visible")?!0:!1;var q=!1;var t=null;p.hasClass("dataTable")?
(q=$("input#"+a[h]+"_configurations"),q.length&&(t=JSON.parse($(q).val()),t=t.ajaxUrl,t="undefined"!=typeof t?k(t,e):null),t?(f[a[h]]=t,q=!1):q=!0):p.hasClass("dl")||p.hasClass("map_wrapper")||p.hasClass("cms_content")||p.hasClass("gi-container")||p.hasClass("pt-container")||p.hasClass("tp-container")||p.hasClass("s3-target")||p.hasClass("s3-organizer")?q=!1:n?(d=k(d,e),window.location.href=d):q=!0;n||(c[b][l]={needs_reload:q,ajaxurl:t,queries:e})}for(h=0;h<a.length;h++)l=a[h],p=$("#"+l),p.is(":visible")&&
(p.hasClass("dataTable")?(b=p.dataTable(),l=f[l],b.fnReloadAjax(l),m(b,e),$("#"+b[0].id+"_dataTable_filterURL").val(l)):p.hasClass("dl")?p.datalist("ajaxReload",e):p.hasClass("map_wrapper")?S3.gis.refreshLayer("search_results",e):p.hasClass("gi-container")?p.groupedItems("reload",null,e):p.hasClass("pt-container")?p.pivottable("reload",null,e):p.hasClass("tp-container")?p.timeplot("reload",null,e):p.hasClass("s3-target")?p.s3Target("reload",k(d,e)):p.hasClass("s3-organizer")&&p.organizer("reload"))}else d=
...
color:"#3b6596",area:!0}]}function b(){Q=O.val();M=S.val();g=new Date(d.data("min"));h=new Date(d.data("max"));R=h-g;Q?(I=moment(Q,e),D=I-g,Q=D/R*100+"%"):Q="0%";M?(J=moment(M,e),D=J-g,M=D/R*100+"%"):M="100%";F.updatePosition(M,Q)}function c(a){var b=v[a].x;try{var c=v[a+1].x}catch(ea){c=moment(d.data("max"))}var f=Z;Z+=4E3;f=setTimeout(function(){var f=c;d.data("slot",a);O.val(b.format(e));S.val(f.format(e));O.trigger("change")},f);T.push(f)}var d=$(this),e=d.data("fmt"),f=d.data("min");if(f)var g=
moment(f);else g=moment().subtract(5,"minutes"),d.data("min",g.format());f=d.data("max");if(f)var h=moment(f);else h=moment().subtract(5,"minutes"),d.data("max",h.format());var k=d.parent().attr("id"),l=g.format("YYYY"),p=moment(g);f="";for(var m=[{year:l,months:[]}],n=0,q;h>p||p.format("M")===h.format("M");)q=p.format("YYYY"),q!=l&&(m.push({year:q,months:[]}),l=q,n++),m[n].months.push(p.format("MMM YYYY")),p.add(1,"month");for(n=0;n<m.length;n++){l=m[n];p=l.months;l=l.year;f+='<optgroup label="'+
l+'">';for(l=0;l<p.length;l++)f+='<option value="'+p[l]+'">'+p[l]+"</option>";f+="</optgroup>"}d.before('<div class="range-coarse"><div class="range-coarse-start"><label for="'+k+'-cs">From:</label><select id="'+k+'-cs">'+f+'</select></div><div class="range-coarse-end"><label for="'+k+'-ce">to:</label><select id="'+k+'-ce">'+f+"</select></div></div>");f=$("#"+k+"-cs");m=$("#"+k+"-ce");f.val(g.format("MMM YYYY"));m.val(h.format("MMM YYYY"));a();d.before('<a class="button secondary tiny play"><i class="fa fa-play"></i> Play</a><a class="button secondary tiny hide pause"><i class="fa fa-pause"></i> Pause</a><a class="button secondary tiny hide stop"><i class="fa fa-stop"></i> Stop</a>');
var t=$("#"+k+" .play"),z=$("#"+k+" .pause"),u=$("#"+k+" .stop"),v=d.data("slots");3>v.length&&t.hide();var R,D,K,F=d.rangepicker({type:"double",startValue:g.format(e),endValue:h.format(e),translateSelectLabel:function(a,b){g=new Date(d.data("min"));h=new Date(d.data("max"));R=h-g;D=a/b*R;K=new Date(+g+parseInt(D));return moment(K).format(e)}});d.before('<div id="'+k+'-chart"><svg></svg></div>');var E=function(a){a=a.point;return'<div class="pt-tooltip"><div class="pt-tooltip-label" style="color:'+
...
X.end;g=new Date(d.data("min"));h=new Date(d.data("max"));R=h-g;D=Q/P*R;I=new Date(+g+parseInt(D));O.val(moment(I).format(e));D=M/P*R;J=new Date(+g+parseInt(D));S.val(moment(J).format(e));d.closest("form").trigger("optionChanged")});f.on("change",function(){g=moment($(this).val(),"MMM YYYY");d.data("min",g.format());I=g.format(e);F.refresh({startValue:I});O.val(I);d.closest("form").trigger("optionChanged")});m.on("change",function(){h=moment($(this).val(),"MMM YYYY").endOf("month");d.data("max",h.format());
J=h.format(e);F.refresh({endValue:J});S.val(J);d.closest("form").trigger("optionChanged")});O.on("change",function(){b()});S.on("change",function(){b()});d.on("clear",function(){F.updatePosition("100%","0%")});d.on("resize",function(a,b,c,f){d.data("min",b);d.data("max",c);F.refresh({startValue:moment(b).format(e),endValue:moment(c).format(e)});d.data("ts",f);F.graph();v=d.data("slots");2<v.length?t.show():t.hide()});var Z=0,T=[];d.data("slot",0);t.on("click",function(){var a=d.data("slot");t.hide();
z.removeClass("hide").show();u.removeClass("hide").show();v=d.data("slots");for(a;a<v.length;a++)c(a)});z.on("click",function(){for(var a=0;a<T.length;a++)clearTimeout(T.pop());Z=0;z.hide();t.show()});u.on("click",function(){for(var a=0;a<T.length;a++)clearTimeout(T.pop());d.data("slot",0);Z=0;z.hide();u.hide();t.show()})});$(".text-filter").keypress(function(a){return 13==a.which?(a.preventDefault(),!1):!0})})})();
