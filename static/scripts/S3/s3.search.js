/**
    ...
    - NB S3Search is being replaced by S3Filter
*/
...
(function () {
     ...
     */
    var saveCurrentSearch = function(event) {
    ...
    var AutocompleteTimer = function() {
        ...
        var fncSearchAutocompleteAjaxArg = function() {
            AutocompleteAjax(selSearchDiv);
        };
        S3.TimeoutVar[$(this).attr('id')] = setTimeout(fncSearchAutocompleteAjaxArg, 400);
    };

    var AutocompleteAjax = function(selSearchDiv) {
    ...
    var CancelEnterPress = function(e) {
        if(e.which === 13){
            return false;
        }
    };
    ...
    $(document).ready(function() {
        ...
        */
        ...
        $('div.simple-form').keyup(AutocompleteTimer)
                            .click(AutocompleteTimer)
                            .keypress(CancelEnterPress);

        $('div.advanced-form').keyup(AutocompleteTimer)
                              .click(AutocompleteTimer)
                              .keypress(CancelEnterPress);
       ...
       $('button#save-search').on('click', saveCurrentSearch);
        ...
        $('input#gis_search_polygon_input').on('change', toggleMapClearButton)
                                           .trigger('change');
    });
     ...
     */
    var clearMapPolygon = function(map_id) {
        var map = S3.gis.maps[map_id];
        if (map.lastDraftFeature) {
            map.lastDraftFeature.destroy();
        }
     ...
     */
    var toggleMapClearButton = function(event) {
