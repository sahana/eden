...
$("hr").attr({style:"display:block"})}},ca=$("#upload-title"),b=$("#upload-container");ca.bind("click",function(g){b.slideDown("fast",function(){ca.html(i18n.upload_image)})});var n=function(g){J&&J.destroy();setTimeout(function(){b.slideUp("fast",function(){ca.html("<a>"+i18n.upload_new_image+"</a>")})},500);var D=new FileReader,Q=(g.target.files||g.originalEvent.dataTransfer.files)[0];fa=Q.name;C=g;"drop"==g.type&&I(g);g=$(".imagecrop-upload");Z(Q)?(D.onload=function(y){S(y.target.result)},D.readAsDataURL(Q)):
alert(i18n.invalid_image+"\n"+i18n.supported_image_formats+": png, jpeg or jpg");g.val("")},f=$("#select-crop-btn"),e=$("#crop-btn"),x=$("#remove-btn"),B=$('input[name="imagecrop-points"]'),G=function(g){J.release();J.disable();e.css({display:"none"});x.css({display:"none"});f.css({display:"inline"})};f.bind("click",function(g){J.enable();var D=J.getBounds();g=D[0]/2;var Q=D[1]/2,y=D[0]/4;D=D[1]/4;J.animateTo([g-y,Q-D,g+y,Q+D]);f.css({display:"none"});e.css({display:"inline"});x.css({display:"inline"})});
e.bind("click",function(g){var D=$(".jcrop-holder");g=parseInt(D.css("width").split("px")[0]);var Q=parseInt(D.css("height").split("px")[0]);D=ba.width/g;Q=ba.height/Q;g=B.val().split(",");var y=g[0],ra=g[1],H=g[3];g=Math.round((g[2]-y)*D);Q=Math.round((H-ra)*Q);J.destroy();G(C);L.attr({width:g,height:Q});z.getContext("2d").drawImage(ba,Math.round(y*D),Math.round(ra*D),g,Q,0,0,g,Q);g=z.toDataURL("image/"+F);S(g)});x.bind("click",G);var M=function(g){B.val(g.x+","+g.y+","+g.x2+","+g.y2)};$("#uploaded-image").bind("load",
...
g=z.toDataURL("image/"+F);S(g)};$(".imagecrop-drag").bind("dragover",I).bind("dragleave",I).bind("drop",n);$(".imagecrop-upload").bind("change",n)}});
