...
q=d(a+"_L5__row"),a=d(a+"_address__row");b?f.hide().after(c).after(g).after(k).after(m).after(h).after(r).after(p).after(q).after(l).after(a).after(e):f.hide().after(c).after(g).after(l).after(a).after(q).after(p).after(r).after(h).after(m).after(k).after(e)}else f.parent().is(".inline-form")?f.show():(d(a+"__row1").hide(),f.hide().after(e),b?k.after(c).after(g):l.after(c).after(g));if(e.length)e.one("click"+this.namespace,function(){var a=d(this);a.fadeOut("slow",function(){a.remove()})})},_lxSelect:function(a,
b,c){var f="#"+this.fieldname,e=this.options,g=d(f+"_L"+a);b?(g.val(b),g.hasClass("multiselect")&&g.multiselect("instance")&&g.multiselect("refresh")):b=parseInt(g.val());if(0===a){var k=this._readLabels(b),l=u.d,m=["1","2","3","4","5"],h,r,p,q;for(p=0;5>p;p++)g=m[p],h=k[g]||l[g],q=f+"_L"+g,g=d(q),r=g.hasClass("required")?"<div>"+h+':<span class="req"> *</span></div>':h+":",d(q+"__row label").html(r),d(q+"__row1 label").html(r),d(q+' option[value=""]').html(i18n.select+" "+h),g.hasClass("multiselect")&&
g.multiselect("instance")&&g.multiselect("refresh")}p=e.hideLx;for(g=a+1;6>g;g++)q=f+"_L"+g,p?(d(q+"__row").hide(),d(q+"__row1").hide()):d(q+" option").remove('[value != ""]'),d(q).val("");if(b)if(q=a+1,e=d(f+"_L"+q+"__row"),e.length){var s;if(d(f+"_L"+a+' option[value="'+b+'"]').hasClass("missing"))h=t[b],h.i=b,l=[h];else{p=!0;for(s in t)if(t[s].f==b){p=!1;break}p&&this._readHierarchy(b,q);l=[];for(s in t)h=t[s],h.f==b&&(h.i=s,l.push(h))}if(a=l.length){l.sort(function(a,b){return a.n.localeCompare(b.n)});
s=f+"_L"+q;k=d(s);m=d(s+' option[value=""]').html();d(s+" option").remove('[value != ""]');s=null;for(p=0;p<a;p++)h=l[p],s=h.i,g=b==s?' selected="selected"':"",r=h.l==q?"":' class="missing"',h='<option value="'+s+'"'+g+r+">"+h.n+"</option>",k.append(h);e.removeClass("hide").show();d(f+"_L"+q+"__row1").removeClass("hide").show();k.hasClass("multiselect")&&(b={header:"",height:300,minWidth:0,selectedList:1,noneSelectedText:m,multiple:!1},k.hasClass("search")?(k.multiselect(b).multiselectfilter({label:"",
