...
k=c(a+"_map_icon__row"),e=c(a+"_L0__row"),f=c(a+"_postcode__row");if(m.is(".control-group, .form-row")){b=c(a+"_L1__row");var q=c(a+"_L2__row"),z=c(a+"_L3__row"),y=c(a+"_L4__row"),t=c(a+"_L5__row"),r=c(a+"_address__row"),n=c(a+"_lat__row"),u=c(a+"_lon__row");a=c(a+"_latlon_toggle__row");d?m.hide().after(h).after(k).after(a).after(u).after(n).after(e).after(b).after(q).after(z).after(y).after(t).after(f).after(r).after(g):m.hide().after(h).after(k).after(a).after(u).after(n).after(f).after(r).after(t).after(y).after(z).after(q).after(b).after(e).after(g)}else m.parent().is(".inline-form")?
...
l.multiselect("refresh")):a=parseInt(l.val(),10);if(0===b){var f=this._readLabels(a),D=e.d,q=["1","2","3","4","5"],n,y,t;f.then(function(a){for(t=0;5>t;t++)k=q[t],n=a[k]||D[k],r=h+"_L"+k,l=c(r),y=m.showLabels?l.hasClass("required")?"<div>"+n+':<span class="req"> *</span></div>':n+":":"",c(r+"__row label").html(y),c(r+"__row1 label").html(y),c(r+' option[value=""]').html(i18n.select+" "+n),l.hasClass("multiselect")&&l.multiselect("instance")&&l.multiselect("refresh")})}f=m.hideLx;for(k=b+1;6>k;k++){var r=
h+"_L"+k;f?(c(r+"__row").hide(),c(r+"__row1").hide()):c(r+" option").remove('[value != ""]');c(r).val("").trigger("change","implicit")}if(a){var p=!1,u=b+1,B=c(h+"_L"+u+"__row");B.length||(p=!0,u++,B=c(h+"_L"+u+"__row"));if(B.length){var v;f=!1;var C=this;if(c(h+"_L"+b+' option[value="'+a+'"]').hasClass("missing")){var w=g[a];w.i=a;var x=[w]}else for(v in x=[],f=!0,g)if(g[v].f==a){f=!1;break}this._readHierarchy(f,a,u,p).then(function(){if(0==x.length)for(v in g)w=g[v],w.f==a&&(w.i=v,x.push(w));var b=
x.length;if(b){x.sort(function(a,b){return a.n.localeCompare(b.n)});var k=h+"_L"+u;var m=c(k),f=c(k+' option[value=""]').html();c(k+" option").remove('[value != ""]');v=null;for(t=0;t<b;t++)w=x[t],v=w.i,k=a==v?' selected="selected"':"",p=w.l==u?"":' class="missing"',k='<option value="'+v+'"'+k+p+">"+w.n+"</option>",m.append(k);B.removeClass("hide").show();c(h+"_L"+u+"__row1").removeClass("hide").show();m.hasClass("multiselect")&&(f={header:"",height:300,minWidth:0,selectedList:1,noneSelectedText:f,
...
b);c.ajaxS3({url:h,dataType:"json",success:function(c){d={};try{for(var h in c)d[h]=c[h];e[b]=d}catch(k){}a.resolve(d)},error:function(b,c,d){(b="UNAUTHORIZED"==d?i18n.gis_requires_login:b.responseText)&&s3_debug(b);a.reject()}})}else a.resolve(d);return a.promise()},_readHierarchy:function(b,a,d,h){var m=""+a+";"+d+";"+h,e=f[m];if(e!==p)return e;var k=c.Deferred();if(b){b="#"+this.fieldname;var A=c(b+"_L"+d),n=!1;A.hide();if(A.hasClass("multiselect")){n=!0;var q=c(b+"_L"+d+"__row button").hide()}var z=
c(b+"_L"+d+"__throbber").removeClass("hide").show();a=h?S3.Ap.concat("/gis/ldata/"+a+"/"+d):S3.Ap.concat("/gis/ldata/"+a);c.ajaxS3({url:a,dataType:"json",success:function(a){for(var b in a)g[b]=a[b];z.hide();n?q.removeClass("hide").show():A.removeClass("hide").show();k.resolve()},error:function(a,b,c){if(a="UNAUTHORIZED"==c?i18n.gis_requires_login:a.responseText)s3_debug(a),S3.showAlert(a,"error");z.hide();n?q.removeClass("hide").show():A.removeClass("hide").show();k.reject()}})}else k.resolve();
