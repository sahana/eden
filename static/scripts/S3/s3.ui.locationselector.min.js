...
f.show():(d(b+"__row1").hide(),f.hide().after(g),m.detach(),f.siblings('[id^="'+a+'"][id$="__row"]').last().after(e).after(m));if(g.length)g.one("click"+this.eventNamespace,function(){var a=d(this);a.fadeOut("slow",function(){a.remove()})})},_lxSelect:function(a,b,c){var e="#"+this.fieldname,f=this.options,g=d(e+"_L"+a);b?(g.val(b),g.hasClass("multiselect")&&g.multiselect("instance")&&g.multiselect("refresh")):b=parseInt(g.val(),10);if(0===a){var m=this._readLabels(b),r=l.d,q=["1","2","3","4","5"],
k,w,t,p;for(t=0;5>t;t++)g=q[t],k=m[g]||r[g],p=e+"_L"+g,g=d(p),w=f.showLabels?g.hasClass("required")?"<div>"+k+':<span class="req"> *</span></div>':k+":":"",d(p+"__row label").html(w),d(p+"__row1 label").html(w),d(p+' option[value=""]').html(i18n.select+" "+k),g.hasClass("multiselect")&&g.multiselect("instance")&&g.multiselect("refresh")}t=f.hideLx;for(g=a+1;6>g;g++)p=e+"_L"+g,t?(d(p+"__row").hide(),d(p+"__row1").hide()):d(p+" option").remove('[value != ""]'),d(p).val("");if(b)if(p=a+1,f=d(e+"_L"+
p+"__row"),f.length){var u;if(d(e+"_L"+a+' option[value="'+b+'"]').hasClass("missing"))k=h[b],k.i=b,r=[k];else{t=!0;for(u in h)if(h[u].f==b){t=!1;break}t&&this._readHierarchy(b,p);r=[];for(u in h)k=h[u],k.f==b&&(k.i=u,r.push(k))}if(a=r.length){r.sort(function(a,b){return a.n.localeCompare(b.n)});u=e+"_L"+p;m=d(u);q=d(u+' option[value=""]').html();d(u+" option").remove('[value != ""]');u=null;for(t=0;t<a;t++)k=r[t],u=k.i,g=b==u?' selected="selected"':"",w=k.l==p?"":' class="missing"',k='<option value="'+
...
"\u00b0"!=c[0]||c[1]&&"'"!=c[1]||c[2]&&'"'!=c[2])return null;f=a[0];g=a[1];m=a[2]}f=f+g/60+m/3600;b&&(f*=b);return f}}return null},_bindEvents:function(){var h=this,l=this.eventNamespace;this.dmsInput.find("input").bind("change"+l,function(){var a=h._validateDMS();null!==a?d(h.element).val(a).change():d(h.element).val(h.defaultValue).change()});this.decimalInput.bind("change"+l,function(){var a=h._validateDecimal();null!==a?(d(h.element).val(a).change(),h.decimalInput.val(a)):d(h.element).val(h.defaultValue).change()});
d(this.element).bind("setvalue"+l,function(){h.refresh()});return!0},_unbindEvents:function(){var h=this.eventNamespace;this.input&&this.input.find("input").unbind(h);d(this.element).unbind(h);return!0}})})(jQuery);
