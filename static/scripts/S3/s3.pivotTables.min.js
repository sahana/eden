...
g){var t=this.options.ajaxURL.split("?"),q=[],e=!1,k;1<t.length?(k=t[1],k=k.split("&")):k=[];var u,n,v;if(a)for(u in a)v=a[u],n=u+"="+v,"totals"==u?this.options.showTotals=v?!0:!1:e||-1!=H.inArray(n,k)||(e=!0),q.push(n);u={};v={};var B,A,F;if(g)for(B=0,A=g.length;B<A;B++)n=g[B],F=n[0],n=n[1],null===n?u[F]||(v[F]=!0):(v[F]&&(v[F]=!1),n=F+"="+encodeURIComponent(n),u[F]?u[F].push(n):u[F]=[n]);B=0;for(A=k.length;B<A;B++)n=k[B].split("="),1<n.length&&(F=decodeURIComponent(n[0]),n=decodeURIComponent(n[1]),
v[F]?e=!0:u[F]?e||-1!=H.inArray(F+"="+n,u[F])||(e=!0):"aggregate"!=F&&(a&&a.hasOwnProperty(F)||q.push(k[B])));for(F in u)for(B=0,A=u[F].length;B<A;B++)e||-1!=H.inArray(u[F][B],k)||(e=!0),q.push(u[F][B]);q=q.join("&");t=t[0];q&&(t=t+"?"+q);this.options.ajaxURL=t;return e},reload:function(a,g,t){t="undefined"!=typeof t?t:!0;"undefined"==typeof g&&(g=this._getFilters());var q=this,e=this.element,k,u=e.find('input[type="hidden"][name="pivotdata"]');if(u.length){e.find(".pt-throbber").show();if(a||g)k=
this._updateAjaxURL(a,g);k||t?(a=this.options.ajaxURL,g=H.ajaxS3,H.searchS3!==v&&(g=H.searchS3),e.find(".pt-empty").hide(),g({url:a,dataType:"json",type:"GET",success:function(a){u.first().val(JSON.stringify(a));q.refresh()},error:function(a,b,e){console.log("UNAUTHORIZED"==e?i18n.gis_requires_login:a.responseText)}})):q.refresh()}},_bindEvents:function(){var a=this,g=H(this.element),t=this.data,q="#"+g.attr("id");H(q+"-options legend").click(function(){H(this).siblings().toggle();H(this).children().toggle()});
H(q+"-filters legend").click(function(){H(this).siblings().toggle();H(this).children().toggle()});g.find(".pt-hide-table").click(function(){a.table_options.hidden=!0;g.find(".pt-table").hide();H(this).siblings(".pt-show-table").show();H(this).hide()});g.find(".pt-show-table").click(function(){a.table_options.hidden=!1;g.find(".pt-table").show();H(this).siblings(".pt-hide-table").show();H(this).hide()});H(q+"-totals").click(function(){var e=H(this).is(":checked");a.options.showTotals!=e&&a.reload({totals:e},
null,!1)});H(q+"-rows,"+q+"-cols,"+q+"-fact").on("change.autosubmit",function(){H(q+"-pt-form").trigger("optionChanged")});if(this.options.autoSubmit){var e=this.options.autoSubmit;H(q+"-pt-form").on("optionChanged",function(){var g=H(this);if(!g.data("noAutoSubmit")){var q=g.data("autoSubmitTimeout");q&&clearTimeout(q);q=setTimeout(function(){var e=a._getOptions(),g=a._getFilters();a.reload(e,g,!1)},e);g.data("autoSubmitTimeout",q)}})}else H(q+"-pt-form input.pt-submit").click(function(){var e=a._getOptions(),
g=a._getFilters();a.reload(e,g,!1)});H(q+" div.pt-table div.pt-cell-zoom").click(function(a){a=H(a.currentTarget);var b=a.closest(".pt-cell-value"),e=b.find(".pt-cell-records");if(0<e.length)e.remove(),a.removeClass("opened");else{for(var g=b.data("keys"),e=b.data("fact"),b=t.lookups[t.facts[e][0]],e=H("<div/>").addClass("pt-cell-records"),q=H("<ul/>"),v=0;v<g.length;v++)q.append("<li>"+b[g[v]]+"</li>");e.append(q);a.addClass("opened").after(e)}});H(q+"-pchart-rows").click(function(){a._renderChart({type:"piechart",
axis:"rows"})});H(q+"-vchart-rows").click(function(){a._renderChart({type:"barchart",axis:"rows"})});H(q+"-schart-rows").click(function(){a._renderChart({type:"spectrum",axis:"rows"})});H(q+"-hchart-rows").click(function(){a._renderChart({type:"breakdown",axis:"rows"})});H(q+"-pchart-cols").click(function(){a._renderChart({type:"piechart",axis:"cols"})});H(q+"-vchart-cols").click(function(){a._renderChart({type:"barchart",axis:"cols"})});H(q+"-schart-cols").click(function(){a._renderChart({type:"spectrum",
axis:"cols"})});H(q+"-hchart-cols").click(function(){a._renderChart({type:"breakdown",axis:"cols"})});g.find(".pt-hide-chart").click(function(){a._renderChart(!1)})},_unbindEvents:function(){var a=H(this.element),g="#"+a.attr("id");H(g+" div.pt-table div.pt-cell-zoom").unbind("click");H(g+"-options legend").unbind("click");H(g+"-filters legend").unbind("click");H(g+"-totals").unbind("click");H(g+"-rows,"+g+"-cols,"+g+"-fact").unbind("change.autosubmit");H(g+"-pt-form").unbind("optionChanged");a.find("input.pt-submit").unbind("click");
H(g+"-pchart-rows,"+g+"-vchart-rows,"+g+"-schart-rows,"+g+"-hchart-rows,"+g+"-pchart-cols,"+g+"-vchart-cols,"+g+"-schart-cols,"+g+"-hchart-cols").unbind("click");a.find(".pt-hide-table").unbind("click");a.find(".pt-show-table").unbind("click");a.find(".pt-hide-chart").unbind("click")}})})(jQuery);
