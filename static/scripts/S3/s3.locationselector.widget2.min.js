...
y&&q.data("specific",y);q.data("hide_lx",a);c&&k(b,0,c);e&&k(b,1,e);g&&k(b,2,g);f&&k(b,3,f);m&&k(b,4,m);u&&k(b,5,u);F(b);x.show();a=$(h+"_lat").val();d=$(h+"_lon").val();c=$(h+"_wkt").val();a||d||c?s(b):$(h+"_map_icon").click(function(){s(b)});$(h+"_L0").change(function(){k(b,0)});$(h+"_L1").change(function(){k(b,1)});$(h+"_L2").change(function(){k(b,2)});$(h+"_L3").change(function(){k(b,3)});$(h+"_L4").change(function(){k(b,4)});$(h+"_L5").change(function(){k(b,5)})};var k=function(b,a,d){var c=
"#"+b,e=$(c).data("hide_lx");d?$(c+"_L"+a).val(d):d=$(c+"_L"+a).val();if(d){for(var g=a+1;6>g;g++){var f=c+"_L"+g;e?$(f+"__row").hide():$(f+" option").remove('[value != ""]');$(f).val("")}p(b);a+=1;e=$(c+"_L"+a+"__row");if(e.length){e.show();var e=!0,m;for(m in l)l[m].f==d&&(e=!1);e&&G(b,a,d);e=[];for(m in l)v=l[m],v.l==a&&v.f==d&&(v.i=m,e.push(v));e.sort(H);var u,g=$(c+"_L"+a);$(c+"_L"+a+" option").remove('[value != ""]');m=0;for(a=e.length;m<a;m++)f=e[m],c=f.i,u=d==c?' selected="selected"':"",c=
'<option value="'+c+'"'+u+">"+f.n+"</option>",g.append(c)}}else for(0===a?d="d":(d=$(c+"_L"+(a-1)).val())||(d="d"),p(b),g=a+1;6>g;g++)f=c+"_L"+g,e?$(f+"__row").hide():$(f+" option").remove('[value != ""]'),$(f).val("");w(b,d)},H=function(b,a){b=b.n;var d=[b,a.n];d.sort();return d[0]==b?-1:1},G=function(b,a,d){b="#"+b;var c=$(b+"_L"+a);c.hide();var e=$(b+"_L"+a+"__throbber");e.show();a=S3.Ap.concat("/gis/ldata/"+d);$.ajax({async:!1,url:a,dataType:"script"}).done(function(a){for(var b in n)l[b]=n[b];
n=null;e.hide();c.show()}).fail(function(a,b,c){msg="UNAUTHORIZED"==c?i18n.gis_requires_login:a.responseText})},t=function(b){b="#"+b+"_L";for(var a,d=5;-1<d;d--)if(a=$(b+d).val())return a;return l.d.id},p=function(b){var a="#"+b,d=$(a+"_parent"),c=$(a);if(c.data("specific"))c=t(b),d.val(c);else{var e=$(a+"_address").val(),g=$(a+"_postcode").val(),f=$(a+"_lat").val(),m=$(a+"_lon").val(),a=$(a+"_wkt").val();e||g||f||m||a?(c.val("dummy"),c=t(b),d.val(c)):(b=t(b),c.val(b),d.val(""))}},F=function(b){var a=
