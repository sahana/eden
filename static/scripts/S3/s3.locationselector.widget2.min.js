(function(){S3.gis.locationselector=function(b,a,e,c,f,m,g,r,q,p){var d="#"+b,k=$(d),s=$(d+"__row"),C=k.next(".error_wrapper"),D=$(d+"_L0__row"),E=$(d+"_map_icon__row"),w=$(d+"_map_icon__row .map_wrapper").attr("id",b+"_map_wrapper"),u=s.hasClass("control-group");if(u){var F=$(d+"_L1__row"),z=$(d+"_L2__row"),B=$(d+"_L3__row"),H=$(d+"_L4__row"),I=$(d+"_L5__row"),N=$(d+"_address__row"),O=$(d+"_postcode__row");e?s.hide().after(w).after(E).after(D).after(F).after(z).after(B).after(H).after(I).after(O).after(N).after(C):
s.hide().after(w).after(E).after(O).after(N).after(I).after(H).after(B).after(z).after(F).after(D).after(C)}else $(d+"__row1").hide(),s.hide().after(C),e?D.after(w).after(E):$(d+"_postcode__row").after(w).after(E);p&&k.data("specific",p);k.data("hide_lx",a);a=$(d+"_lat").val();e=$(d+"_lon").val();p=$(d+"_wkt").val();(a||e||p)&&k.data("manually_geocoded",!0);var K=$(d+"_L0");if(D.length){var s=[],x;for(x in l)v=l[x],0==v.l&&(v.i=x,s.push(v));s.sort(P);x=0;for(C=s.length;x<C;x++)F=s[x],w=F.i,w='<option value="'+
w+'">'+F.n+"</option>",K.append(w);D.removeClass("hide").show();u||$(d+"_L0__row1").removeClass("hide").show()}c&&t(b,0,c);f&&t(b,1,f);m&&t(b,2,m);g&&t(b,3,g);r&&t(b,4,r);q&&t(b,5,q);Q(b);E.removeClass("hide").show();a||e||p?L(b):$(d+"_map_icon").click(function(){L(b);return!1});K.change(function(){t(b,0)});var M=$(d+"_L1");M.change(function(){t(b,1)});var J=$(d+"_L2");J.change(function(){t(b,2)});var G=$(d+"_L3");G.change(function(){t(b,3)});var A=$(d+"_L4");A.change(function(){t(b,4)});var y=$(d+
...
m){var g=S3.Ap.concat("/gis/hdata/"+e);$.ajaxS3({async:!1,url:g,dataType:"script",success:function(a){m={};try{for(var b in n)m[b]=n[b];h[e]=m;n=null}catch(c){}},error:function(a,b,c){msg="UNAUTHORIZED"==c?i18n.gis_requires_login:a.responseText;s3_debug(msg)}})}for(var r=h.d,q,p,d=["1","2","3","4","5"],k,g=0;5>g;g++)q=d[g],p=m[q]||r[q],k=c+"_L"+q,$(k).hasClass("required")?($(k+"__row label").html("<div>"+p+':<span class="req"> *</span></div>'),$(k+"__row1 label").html("<div>"+p+':<span class="req"> *</span></div>')):
($(k+"__row label").html(p+":"),$(k+"__row1 label").html(p+":")),$(k+' option[value=""]').html(i18n.select+" "+p)}if(e){for(q=a+1;6>q;q++)k=c+"_L"+q,f?($(k+"__row").hide(),$(k+"__row1").hide()):$(k+" option").remove('[value != ""]'),$(k).val("");u(b);a+=1;f=$(c+"_L"+a+"__row");if(f.length){f.removeClass("hide").show();$(c+"_L"+a+"__row1").removeClass("hide").show();f=!0;for(g in l)if(l[g].f==e){f=!1;break}f&&R(b,a,e);f=[];for(g in l)r=l[g],r.l==a&&r.f==e&&(r.i=g,f.push(r));f.sort(P);var r=f.length,
s;q=$(c+"_L"+a);$(c+"_L"+a+" option").remove('[value != ""]');for(g=0;g<r;g++)c=f[g],s=c.i,p=e==s?' selected="selected"':"",c='<option value="'+s+'"'+p+">"+c.n+"</option>",q.append(c);if(1==r){t(b,a,s);return}}else $(c+"_geocode button").length&&H(b)}else for(u(b),q=a+1;6>q;q++)k=c+"_L"+q,f?($(k+"__row").hide(),$(k+"__row1").hide()):$(k+" option").remove('[value != ""]'),$(k).val("");B(b)},P=function(b,a){b=b.n;var e=[b,a.n];e.sort();return e[0]==b?-1:1},R=function(b,a,e){b="#"+b;var c=$(b+"_L"+a);
...
"#"+b,e=$(a+"_parent"),c=$(a);if(c.data("specific"))c=z(b),e.val(c);else{var f=$(a+"_address").val(),m=$(a+"_postcode").val(),g=$(a+"_lat").val(),r=$(a+"_lon").val(),a=$(a+"_wkt").val();f||m||g||r||a?(c.val("dummy"),c=z(b),e.val(c)):(b=z(b),c.val(b),e.val(""))}},Q=function(b){var a="#"+b;$(a+"_address__row").removeClass("hide").show();$(a+"_address__row1").removeClass("hide").show();$(a+"_postcode__row").removeClass("hide").show();$(a+"_postcode__row1").removeClass("hide").show();$(a+"_geocode button").length&&
...
$(e+"_map_wrapper").removeClass("hide").show();$.when(U()).then(function(a){a="location_selector_"+b;var f=S3.gis;if(!f.maps[a]){var m=f.show_map(a),g=$(e);$(e+"_parent");B(b);var r=$(e+"_lat"),q=$(e+"_lon"),p=$(e+"_wkt");a=r.val();var d=q.val(),k=p.val();if(a||d||k)void 0!=k?(a={internalProjection:m.getProjectionObject(),externalProjection:f.proj4326},a=(new OpenLayers.Format.WKT(a)).read(k)):(a=new OpenLayers.Geometry.Point(parseFloat(d),parseFloat(a)),a.transform(f.proj4326,m.getProjectionObject()),
a=new OpenLayers.Feature.Vector(a)),m.s3.draftLayer.addFeatures([a]),u(b);var s;a=m.controls;for(var d=0,t=a.length;d<t;d++)if("OpenLayers.Control.DrawFeature"==a[d].CLASS_NAME){s=a[d];break}s&&(m.s3.pointPlaced=function(a){$(e+"_geocode .geocode_fail").hide();$(e+"_geocode .geocode_success").hide();var c=a.geometry;"OpenLayers.Geometry.Point"==c.CLASS_NAME?(a=c.getBounds().getCenterLonLat(),a.transform(m.getProjectionObject(),f.proj4326),r.val(a.lat),q.val(a.lon),g.data("manually_geocoded",!0),S(b)):
