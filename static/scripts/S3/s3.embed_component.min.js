$(function(){function d(){$(".embedded > td > input").removeAttr("disabled");$(".embedded > td > select").removeAttr("disabled");$(".embedded > td > textarea").removeAttr("disabled")}function l(){$(".embedded > td > input").attr("disabled",!0);$(".embedded > td > select").attr("disabled",!0);$(".embedded > td > textarea").attr("disabled",!0)}$("#select_from_registry_row").attr("controller");var h=$("#select_from_registry_row").attr("component"),m=$("#select_from_registry_row").attr("url"),e=$("#select_from_registry_row").attr("value"),
k=$("#select_from_registry_row").attr("field"),c="#"+k,a="#dummy_"+k,f=$(c).val();$(a).addClass("hide");0<f&&($("#clear_form_link").removeClass("hide"),$("#edit_selected_link").removeClass("hide"),l());$("#select_from_registry").click(function(){$("#select_from_registry_row").addClass("hide");$("#component_autocomplete_row").removeClass("hide");$(a).removeClass("hide");$("#component_autocomplete_label").removeClass("hide");$(a).focus()});$(a).focusout(function(){f=$(c).val();$(a).addClass("hide");
...
$("#clear_form_link").addClass("hide");$("#load_throbber").removeClass("hide");clear_component_form();$.getJSONS3(m+a+".s3json?show_ids=true",function(b){try{var a=b["$_"+h][0];l();$(c).val(a["@id"]);var d=/^[$|@].*/,e=/^[$]k_.*/;for(j in a){if(j.match(d))if(j.match(e))var g="#"+h+"_"+j.slice(3);else continue;else g="#"+h+"_"+j;try{if(b=a[j],b.hasOwnProperty("@id")){var f=eval(b["@id"]);$(g).val(f)}else if(b.hasOwnProperty("@value")){try{val=JSON.parse(b["@value"])}catch(k){val=b["@value"]}$(g).val(val)}else $(g).val(b)}catch(m){}}}catch(n){$(c).val("")}$("#load_throbber").addClass("hide");
