...
$("#clear_form_link").addClass("hide");$("#load_throbber").removeClass("hide");clear_component_form();$.getJSONS3(m+a+".s3json?show_ids=true",function(b){try{var a=b["$_"+h][0];l();$(c).val(a["@id"]);var d=/^[$|@].*/,e=/^[$]k_.*/,g;for(j in a){if(j.match(d))if(j.match(e))g="#"+h+"_"+j.slice(3);else continue;else g="#"+h+"_"+j;try{b=a[j];if(b.hasOwnProperty("@id")){var f=eval(b["@id"]);$(g).val(f)}else if(b.hasOwnProperty("@value")){var k;try{k=JSON.parse(b["@value"])}catch(i){k=b["@value"]}$(g).val(k)}else $(g).val(b)}catch(m){}}}catch(n){$(c).val("")}$("#load_throbber").addClass("hide");
