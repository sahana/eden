...
c.attr("prefix"),m=a.attr("class").replace("-","_"),b=S3.Ap.concat("/",i,"/",k,"/search.acjson?",m,"=True&",b),b=b.replace(f,k);void 0!=j&&(b+="&get_fieldname="+j);S3.JSONRequest[a.attr("id")]=$.ajax({url:b,dataType:"json",success:function(a){if(a.length==1){var d=$(a[0].represent).text();l.val(d);e.val(a[0].id);e.change()}else{d='<UL id = "'+f+'_result_list"class = "search_autocomplete_result_list" style = "display:inline;">';if(a.length==0)d=d+('<LI class = "search_autocomplete_result_item" style = "border:1px solid;list-style-type:none;">'+
S3.i18n.no_match+"</LI>");else{for(var b=0;b<a.length;b++)var d=d+('<LI id="'+a[b].id+'"class = "search_autocomplete_result_item" style = "border:1px solid;list-style-type:none;">'),g=$(a[b].represent),d=d+('<SPAN style = "cursor:pointer;">'+g.text()+'</SPAN><A href="'+g.attr("href")+'" target = "blank" style = "font-size:0.8em;margin-right:10px;float:right;">Details</A></LI>');a.length>10&&(d=d+('<LI class = "search_autocomplete_result_item" style = "border:1px solid;list-style-type:none;"><I>'+
S3.i18n.ac_widget_more_results+"</I></LI>"))}c.append(d+"</UL>")}h.remove();$('[name ^= "'+f+'_search_simple"]').blur()}})}else h.remove(),$("#"+f+"_result_list").remove()}function CancelEnterPress(a){if(13===a.which)return!1}
...
$(".simple-lnk").click(function(a){a.preventDefault();a=$('.search_form[fieldname="'+$(this).attr("fieldname")+'"]');a.length==0&&(a=$(".search_form"));a.find(".advanced-form").hide();a.find(".simple-form").show();return false})));$(".search_select_letter_label").click(function(){$("#"+$(this).attr("id").replace("label","widget")).slideToggle();$(this).toggleClass("expanded")});$("div.simple-form").keyup(fncSearchAutocompleteTimer).click(fncSearchAutocompleteTimer).keypress(CancelEnterPress);$("div.advanced-form").keyup(fncSearchAutocompleteTimer).click(fncSearchAutocompleteTimer).keypress(CancelEnterPress);
$(".search_autocomplete_result_list li span").live("click",function(){var a=$(this).parent(),g=a.parent(),c=g.parent(),e=c.parent().find(".hidden_input"),c=c.attr("fieldname"),a=a.attr("id");if(a!=void 0){e.val(a);e.change();e=$('[name ^= "'+c+'_search_simple"]');a=$(this).text();e.val(a);g.remove()}})});
