...
t=d*m*m,e=m*(n-e),n=6378137*(15*g/256+45*f/1024),m=-223234795*f/3072,k=6378137*(1-b/4-3*g/64-5*f/256)*k+-6378137*(3*b/8+3*g/32+45*f/1024)*Math.sin(2*k)+n*Math.sin(4*k)+m*Math.sin(6*k),m=1-a+t,s=5-18*a+p+72*t-58*d,n=Math.pow(e,5);x=.9996*h*(e+m*Math.pow(e,3)/6+s*n/120)+5E5;m=(5-a+9*t+4*t*t)*Math.pow(e,4)/24;y=.9996*(k+h*c*(e*e/2+m+n*e/720*(61-58*a+p+600*t-330*d)));return{utm_zone:l,utm_easting:x,utm_northing:y}};this.invProj=function(l,c,a){l=-(6*(30-l)+3)*r;var e=Math.sqrt(1-b),e=(1-e)/(1+e),k=e*
e,n=e*k;c-=5E5;a=a/.9996/(6378137*(1-b/4-g/64*3-f/256*5));k=1.3125*k-1.71875*k*k;a=a+(1.5*e-.84375*n)*Math.sin(2*a)+k*Math.sin(4*a)+151*n/96*Math.sin(6*a);var h=Math.sin(a),e=Math.cos(a),n=h/e,k=6378137/Math.sqrt(1-b*h*h),m=n*n,p=d*e*e,h=1-b*h*h,h=6378137*(1-b)/Math.sqrt(h*h*h);c/=.9996*k;var t=c*c,s=c*t,w=t*t,z=m*m,v=p*p;temp2=5+3*m+10*p-4*v-9*d;temp4=61+90*m+298*p+45*z-252*d-3*v;temp5=(1+2*m+p)*s/6;temp6=5-2*p+28*m-3*v+8*d+24*z;lat=180*(a-k*n/h*(t/2-w/24*temp2+temp4*s*s/720))/Math.PI;lon=180*(l+
(c-temp5+temp6*c*w/120)/e)/Math.PI;return{lon:lon,lat:lat}}};utm_proj=new this.UTM}
