...
(function($, undefined) {
  ...
  $.widget('ech.multiselect', {
    ...
    options: {
        ...
        position: {},
        appendTo: 'body',
        // S3 extension to create new entries
        create: {}
    },
    ...
    refresh: function(init) {
        ...
        if (!$.isEmptyObject(o.create)) {
            // Add an option to create new entries
            var opts = o.create;
            var url = S3.Ap.concat('/', opts.c, '/', opts.f, '/create?format=popup&caller=', id);
            if (opts.parent) {
                url += '&parent=' + opts.parent;
            }
            if (opts.child) {
                url += '&child=' + opts.child;
            }
            var label = opts.label;
            html += '<li><a class="s3_modal" target="top" title="' + label + '" href="' + url + '"><label class="ui-corner-all"><span><i>' + label + '</i></span></label></a></li>';
        }
        ...
        if (!$.isEmptyObject(o.create)) {
            // Ensure the click is handled properly
            S3.addModals();
        }
