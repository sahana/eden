...
def facebook_outbox():
    ...
    # CRUD Strings
    s3.crud_strings[tablename] = Storage(
        title_display = T("Facebook Details"),
        title_list = T("Posts to facebook"),
        label_list_button = T("View posts"),
        label_delete_button = T("Delete posts"),
    ...
    def prep(r):
        if r.interactive:
            ...
            if not db(table.id > 0).select(table.id,
                                               limitby=(0, 1)).first():
                    # Update results
                    result = msg.poll("msg_facebook_channel", channel_id)
                    if not result:
                        session.error = T("Need to configure Facebook Authentication")
                        redirect(URL(f="facebook_channel", args=[1, "update"]))
        return True
    s3.prep = prep

    s3db.configure("msg_facebook",
                    insertable=False,
                    editable=False,
                    list_fields = ["id",
                                  "from_address",
...
def facebook_inbox():
    ...
    s3.crud_strings[tablename] = Storage(
        title_display = T("Facebook Details"),
        title_list = T("Facebook InBox"),
        label_list_button = T("View post InBox"),
        label_delete_button = T("Delete Facebook"),
        msg_record_deleted = T("Post deleted"),
        msg_list_empty = T("No posts currently in InBox")
    )
...
def facebook_channel():
    ...
    # CRUD Strings
    s3.crud_strings[tablename] = Storage(
        title_update = T("Edit Facebook account"),
        msg_record_modified = T("Facebook account updated"),
    )
...
def process_twitter_outbox():
    ...
    msg.process_outbox(contact_method = "FACEBOOK")
