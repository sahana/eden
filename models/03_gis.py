...
def gis_config_form_setup():
    ...
    table.osm_oauth_consumer_key.comment = DIV(
        ...
        _title="%s|%s|%s" % (
            ...
            T("In order to be able to edit OpenStreetMap data from within %(name_short)s, you need to register for an account on the OpenStreet server.") % dict(name_short=deployment_settings.get_system_name_short()),
...
s3mgr.configure("gis_location",
                ...
                deduplicate=gis_location_duplicate,
                list_fields = ["id",
                               "name",
                               "level",
                               "parent",
